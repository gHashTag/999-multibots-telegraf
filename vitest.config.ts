...
export default defineConfig({
  ...
  test: {
    globals: true,
    /*
     * СРОК ТЕСТА — ТРИДЦАТЬ СЕКУНД, А НЕ ПЯТЬ. ЭТО ПОЧИНКА, А НЕ ПОСЛАБЛЕНИЕ.
     *
     * В этом репозитории десятки СТОРОЖЕЙ, которые читают весь `src` целиком:
     * граф импортов для денежных маршрутов, перепись мест списания, сверка
     * арифметики в комментариях, поиск голых импортов встроенных модулей.
     * Такая работа честно стоит секунд.
     *
     * С умолчанием vitest в 5000 мс они падали ПО ВРЕМЕНИ на любой занятой
     * машине — и падали неотличимо от настоящей находки. Замер 07.09.2026:
     * при полном прогоне рядом со сборкой Xcode «падал» то один сканер, то
     * другой, каждый раз с разным составом. Сообщение при этом звучало как
     * «набор мест, способных списать деньги, изменился» — то есть тревога,
     * которой не было.
     *
     * Сторож, падающий от нагрузки, обесценивается дважды: его перестают
     * читать и начинают перезапускать, а вместе с шумом однажды спишут и
     * настоящую поломку. Тридцать секунд — с запасом выше самого медленного
     * замеренного сканера (7,8 с) и всё ещё далеко от «тест завис».
     */
    testTimeout: 30_000,
    hookTimeout: 30_000,
