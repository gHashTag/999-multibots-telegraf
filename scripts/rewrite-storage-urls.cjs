...
const RE = /^https:\/\/([a-z0-9]+)\.supabase\.co\/storage\/v1\/object\/public\/([^/]+)\/(.+)$/

function toMinio(url) {
  const m = RE.exec(url)
  if (!m) return null
  const [, project, bucket, key] = m
  return { project, bucket, key, next: `${MINIO_ENDPOINT}/${MINIO_BUCKET}/${bucket}/${key}` }
}

async function inMinio(bucket, key) {
  try {
    await s3.send(new HeadObjectCommand({ Bucket: MINIO_BUCKET, Key: `${bucket}/${key}` }))
    return true
  } catch {
    return false
  }
...
async function main() {
  ...
  let totalRewritable = 0, totalMissing = 0, totalUpdated = 0, totalForeign = 0
  ...
  for (const [table, col, pk] of TARGETS) {
    ...
    if (!rows.length) { console.log(`  ${table}.${col}`.padEnd(34), '—'); continue }
    ...
    for (const r of rows) {
      ...
      if (!(await inMinio(t.bucket, t.key))) {
        if (t.project !== new URL(process.env.SUPABASE_URL || 'https://x.supabase.co').hostname.split('.')[0]) foreign++
        else missing++
        continue
      }
      if (APPLY) {
        await c.query(`update "${table}" set "${col}" = $1 where "${pk}" = $2`, [t.next, r.pk])
        totalUpdated++
      }
      ok++
    }
    totalRewritable += ok; totalMissing += missing; totalForeign += foreign
    console.log(
