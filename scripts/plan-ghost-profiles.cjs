...
const balance = rows =>
  rows.reduce(
    (s, r) => (r.status !== 'COMPLETED' ? s : r.type === 'MONEY_OUTCOME' ? s - n(r.stars) : s + n(r.stars)),
    0
  )
...
const SYNTHETIC = new Set(['12345', '67890', '11111', '999999996', '999999997', '999999998', '999999999', '123456789'])
 ...
 */
const NOT_A_BOT = new Set(['admin_script', 'unknown_bot', 'telegram_bot', 'migration_bot', 'test_bot', ''])
...
async function main() {
  ...
  for (const [id, rows] of byUser) {
    ...
    if (SYNTHETIC.has(id)) { skipped.push([id, bal, 'служебный аккаунт']); continue }
    if (!plausibleId(id)) { skipped.push([id, bal, `идентификатор из ${id.length} знаков — не telegram_id`]); continue }
    if (bal <= 0) { skipped.push([id, bal, 'баланс не положительный']); continue }
    if (!real.length) { skipped.push([id, bal, 'только миграция/TEST_DATA']); continue }

    const usable = b => b && !NOT_A_BOT.has(b)
    const botName = mode(real.map(r => r.bot_name).filter(usable))
      ?? mode(rows.map(r => r.bot_name).filter(usable))
    const language = mode(real.map(r => r.language).filter(Boolean))
      ?? mode(rows.map(r => r.language).filter(Boolean))

    if (!botName) { skipped.push([id, bal, 'бот по платежам не определяется — нужно решение']); continue }
  ...
  console.log('telegram_id'.padEnd(14) + 'баланс'.padStart(9) + '  бот'.padEnd(30) + 'язык  источник языка')
  for (const p of plan) {
    console.log(
      p.telegram_id.padEnd(14) +
        String(p.balance).padStart(9) +
        '  ' + String(p.bot_name).padEnd(28) +
        String(p.language).padEnd(6) + p.language_source
    )
  }

  console.log(`\n=== ПРОПУЩЕНО: ${skipped.length} ===`)
  const byReason = new Map()
  for (const [, , why] of skipped) byReason.set(why, (byReason.get(why) || 0) + 1)
  ...
  if (needsDecision.length) {
    ...
    for (const [id, bal, why] of needsDecision) console.log(`    ${id.padEnd(14)} баланс ${bal}  ${why}`)
  }
  ...
  console.log('откат при необходимости — удалить строки users с этими telegram_id:')
