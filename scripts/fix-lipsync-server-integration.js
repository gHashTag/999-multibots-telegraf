...
export async function generateAiServerLipSync(
  ...
  try {
    ...
    return {
      ...
      error: result.error,
      urls: {
        get: \`\${process.env.SERVER_API_URL || 'https://ai-server-production-production-8e2d.up.railway.app'}/api/lipsync/\${result.id}\`,
        cancel: \`\${process.env.SERVER_API_URL || 'https://ai-server-production-production-8e2d.up.railway.app'}/api/lipsync/\${result.id}/cancel\`
      }
