...
async function main() {
  ...
  const users = await fetchAll('users', 'telegram_id,username,first_name,last_name,bot_name')
  ...
  console.log(`затронутых пользователей: ${new Set(td.map(r => String(r.telegram_id))).size}`)
  console.log(`звёзд в TEST_DATA: ${r2(td.reduce((s, r) => s + n(r.stars), 0))}\n`)
  ...
  for (const [uid, rows] of groups) {
    ...
    if (after < 0) victims.push({ uid, now: r2(now), after: r2(after), debt: r2(-after), rows: rows.length })
  }

  victims.sort((a, b) => b.debt - a.debt)

  console.log(`=== УЙДУТ В МИНУС: ${victims.length} человек ===`)
  console.log(`Суммарный минус: ${r2(victims.reduce((s, v) => s + v.debt, 0))} звёзд`)
  console.log(`(это услуги, которыми люди уже воспользовались за тестовые звёзды)\n`)
  console.log(
  ...
  for (const v of victims) {
    ...
    const who = u
      ? (u.username ? '@' + u.username : [u.first_name, u.last_name].filter(Boolean).join(' ') || '—')
      : 'НЕТ В users'
    console.log(
      v.uid.padEnd(14) +
  ...
  const touched = [...groups.entries()].filter(([, rows]) => rows.some(isTestData))
