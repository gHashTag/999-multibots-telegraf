...
# Скрипт для исправления переменной API_SERVER_URL в .env файле
...
# Проверяем наличие переменной API_SERVER_URL в файле .env
if grep -q "API_SERVER_URL=" "$ENV_FILE"; then
    current_url=$(grep "API_SERVER_URL=" "$ENV_FILE" | cut -d "=" -f2)
    echo -e "${BLUE}Текущее значение API_SERVER_URL:${NC} $current_url"
    
    # Проверяем, пустое ли значение или указан example.com
    if [ -z "$current_url" ] || [ "$current_url" = "https://example.com" ] || [ "$current_url" = "" ]; then
        echo -e "${YELLOW}API_SERVER_URL имеет неправильное или пустое значение. Обновляем...${NC}"
        need_update=true
    else
        echo -e "${GREEN}API_SERVER_URL уже имеет допустимое значение. Проверяем доступность...${NC}"
    ...
    echo -e "${YELLOW}API_SERVER_URL не найден в файле .env. Добавляем...${NC}"
    need_update=true
fi

# Если нужно обновление, добавляем или обновляем переменную
if [ "$need_update" = true ]; then
    # Новое значение для API_SERVER_URL
    ...
    # Проверяем, существует ли строка API_SERVER_URL в файле
    if grep -q "API_SERVER_URL=" "$ENV_FILE"; then
        # Если существует, обновляем значение
        sed -i "s|API_SERVER_URL=.*|API_SERVER_URL=$NEW_ELESTIO_URL|" "$ENV_FILE"
    else
        # Если не существует, добавляем новую строку
        echo "" >> "$ENV_FILE"
        echo "API_SERVER_URL=$NEW_ELESTIO_URL" >> "$ENV_FILE"
    fi
    
    echo -e "${GREEN}API_SERVER_URL успешно обновлен до $NEW_ELESTIO_URL${NC}"
