...
async function checkUserBalance(telegramId) {
  ...
  try {
    ...
    console.log(
      `✅ Balance retrieved: ${stars.toLocaleString('ru-RU')} ⭐ stars`
    )
    ...
    const { data: payments, error: paymentsError } = await supabase
      .from('payments_v2')
      .select(
        'type, stars, amount, currency, payment_date, description, status'
      )
      .eq('telegram_id', telegramId)
    ...
    if (!paymentsError && payments && payments.length > 0) {
      ...
      payments.forEach((p, idx) => {
        console.log(
          `   ${idx + 1}. ${formatDate(p.payment_date)} | ${p.type} | ${p.stars || 0}⭐ | ${p.status} | ${p.description || 'N/A'}`
        )
      })
    }
  ...
  try {
    ...
    for (const subscriptionType of subscriptionPriority) {
      ...
      if (subscriptionType === 'NEUROTESTER') {
        ...
        console.log(
          `   ✅ NEUROTESTER subscription is PERMANENT (always active)`
        )
      } else {
        ...
        if (isActive) {
          console.log(
            `   ✅ ${subscriptionType} subscription is ACTIVE (expires ${formatDate(expirationDate)})`
          )
        } else {
          console.log(
            `   ❌ ${subscriptionType} subscription EXPIRED (expired ${formatDate(expirationDate)})`
          )
        }
      }
    ...
    if (activeSubscription) {
      ...
      if (allSubs && allSubs.length > 0) {
        ...
        allSubs.forEach((s, idx) => {
          console.log(
            `   ${idx + 1}. ${s.subscription_type} | ${formatDate(s.payment_date)} | ${s.status}`
          )
        })
      }
...
async function grantNeurovideoSubscription(
  ...
  try {
    ...
    const subscriptionData = {
      ...
      description:
        'Manual NEUROVIDEO subscription grant by admin for user 5781166218',
      payment_date: now,
      created_at: now,
    }
    ...
    if (subscriptionCheck.hasActiveSubscription) {
      ...
      if (sub.type === 'NEUROVIDEO' || sub.type === 'NEUROTESTER') {
        ...
        if (sub.type === 'NEUROTESTER') {
          console.log(
            '   - Note: NEUROTESTER includes all features (NEUROVIDEO + NEUROPHOTO)'
          )
        }
...
async function generateReport(results) {
  ...
  if (results.userCheck.exists) {
    ...
    console.log(
      `   - Name: ${results.userCheck.data.first_name || ''} ${results.userCheck.data.last_name || ''}`
    )
  ...
  if (results.subscriptionBefore.hasActiveSubscription) {
    ...
    console.log(
      `   - Start: ${formatDate(results.subscriptionBefore.subscription.startDate)}`
    )
    if (results.subscriptionBefore.subscription.expirationDate) {
      console.log(
        `   - Expires: ${formatDate(results.subscriptionBefore.subscription.expirationDate)}`
      )
    } else {
  ...
  if (results.grantAction.success) {
    ...
    console.log(
      '   ℹ️  No action needed - user already has active subscription'
    )
  } else {
  ...
  if (results.verification.hasAccess) {
    ...
    console.log(
      `   - Subscription Type: ${results.verification.subscriptionType}`
    )
...
async function main() {
  ...
  console.log(
    '║         USER MANAGEMENT SCRIPT - TELEGRAM ID: 5781166218       ║'
  )
  console.log(
