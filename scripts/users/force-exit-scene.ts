...
function getSecretSafe(secretName: string): string | undefined {
  try {
    ...
    console.warn(`⚠️ Секрет "${secretName}" не найден в Infisical, пропускаем...`)
    return undefined
  }
}
...
async function initializeInfisical() {
  ...
  try {
    ...
    const botTokens = [
      'BOT_TOKEN_1', 'BOT_TOKEN_2', 'BOT_TOKEN_3', 'BOT_TOKEN_4',
      'BOT_TOKEN_5', 'BOT_TOKEN_6', 'BOT_TOKEN_7', 'BOT_TOKEN_8',
      'BOT_TOKEN_9', 'BOT_TOKEN_TEST_1', 'BOT_TOKEN_TEST_2'
    ]
...
async function forceExitScene(telegramId: string): Promise<void> {
  try {
    console.log(`🔄 Начинаю процесс вывода пользователя ${telegramId} из сцены...`)

    // 1. Получаем информацию о пользователе
    const user = await getUserByTelegramId(telegramId)
    if (!user) {
      console.error(`❌ Пользователь с ID ${telegramId} не найден в базе данных`)
      process.exit(1)
    }
...
Это вернет вас в главное меню бота.`
    ...
    try {
      ...
      if (response.data.ok) {
        ...
        console.error(`❌ Ошибка отправки сообщения:`, JSON.stringify(response.data, null, 2))
      }
    } catch (error: any) {
      if (error.response?.data) {
        const errorData = error.response.data
        if (errorData.error_code === 403) {
          console.error(`❌ Пользователь заблокировал бота или не может получать сообщения`)
        } else {
          console.error(`❌ Ошибка при отправке сообщения:`, JSON.stringify(errorData, null, 2))
        }
      } else {
    ...
    console.log(`   2. Если пользователь все еще застрял, попросите его перезапустить Telegram`)
...
if (!telegramId) {
  ...
  console.error('Использование: bun run scripts/users/force-exit-scene.ts <telegram_id>')
