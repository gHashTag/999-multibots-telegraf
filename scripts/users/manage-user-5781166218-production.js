...
const isProduction =
  process.env.NODE_ENV === 'production' || process.cwd().includes('/root/')
...
// Supabase service_role key — только из окружения, в коде его быть не должно.
// Пустая строка вместо ключа даёт невнятную ошибку от сервера, поэтому падаем сразу.
...
if (isProduction) {
  ...
  try {
    ...
    console.log(
      '✅ Loaded Supabase from production dist/core/supabase/index.js'
    )
  } catch (err) {
    ...
    const SUPABASE_SERVICE_ROLE_KEY = requireServiceRoleKey() // secret-guard-ok: значение приходит из process.env, литерала в коде нет
  ...
  try {
    ...
    if (payments && payments.length > 0) {
      ...
      payments.forEach((p, idx) => {
        console.log(
          `   ${idx + 1}. ${formatDate(p.payment_date)} | ${p.type} | ${p.stars || 0}⭐ | ${p.status}`
        )
      })
    }
...
async function checkSubscriptionStatus(telegramId) {
  ...
  for (const subscriptionType of subscriptionPriority) {
    ...
    console.log(
      ...
      formatDate(subData.payment_date)
    )
    ...
    if (subscriptionType === 'NEUROTESTER') {
      ...
      if (isActive) {
        console.log(
          `   ✅ ${subscriptionType} is ACTIVE (expires ${formatDate(expirationDate)})`
        )
      } else {
        console.log(
          `   ❌ ${subscriptionType} EXPIRED (${formatDate(expirationDate)})`
        )
      }
    }
...
async function main() {
  ...
  try {
    ...
    console.log(
      '📊 User Status:',
      results.userCheck.exists ? '✅ Found' : '❌ Not found'
    )
    console.log(
      '💰 Balance:',
      results.balance.balance.toLocaleString('ru-RU'),
      '⭐ stars'
    )
    console.log(
      '📋 Subscription:',
      results.verification.hasAccess ? '✅ Active' : '❌ Inactive'
    )
    console.log(
      '🎯 NEUROVIDEO Access:',
      results.verification.hasAccess ? '✅ GRANTED' : '❌ DENIED'
    )
