...
async function checkBotPrecise(botName) {
  ...
  try {
    ...
    const incomeMethods = {}
    realIncomes.forEach(t => {
      incomeMethods[t.payment_method] =
        (incomeMethods[t.payment_method] || 0) + 1
    })
    ...
    if (suspicious.length > 0) {
      ...
      suspicious.slice(0, 3).forEach((t, i) => {
        console.log(
          `   ${i + 1}. [${t.type}] ${t.currency} ${t.amount} - ${t.payment_method}`
        )
...
async function analyzeAllBotsPrecise() {
  ...
  console.log(
    '│    БОТ      │ ТРАНЗАКЦ.│ ДОХОД XTR│ДОХОД STARS│ ДОХОД RUB│ ИТОГО ДОХ│ ПРИБЫЛЬ  │'
  )
  console.log(
    '├─────────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┤'
  )
  ...
  results.forEach(result => {
    ...
    console.log(
      `│ ${status} ${name}  │${transactions}│${incomeXTR}│${incomeSTARS}│${incomeRUB}│${totalIncome}│${profit}│`
    )
  })
  ...
  const grandTotalIncome = (totalIncomeXTR + totalIncomeSTARS + totalIncomeRUB)
    .toFixed(0)
    .padStart(8)
  const totalProfitStr = totalProfit.toFixed(0).padStart(8)

  console.log(
    `│ ИТОГО:      │${totalTransactionsStr}│${totalIncomeXTRStr}│${totalIncomeSTARSStr}│${totalIncomeRUBStr}│${grandTotalIncome}│${totalProfitStr}│`
  )
  console.log(
    '└─────────────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────────┘'
  )
  ...
  console.log(`\n📈 СТАТИСТИКА:`)
  console.log(
    `   Прибыльные боты: ${profitableBots.length} из ${results.length}`
  )
  ...
  if (noPaymentsBots.length > 0) {
    ...
    noPaymentsBots.forEach(bot => {
      console.log(
        `   - ${bot.botName}: ${bot.totalTransactions} транзакций (все системные)`
      )
    })
  }
