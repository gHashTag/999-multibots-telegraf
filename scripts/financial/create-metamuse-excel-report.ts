/**
 * СОЗДАЕМ EXCEL ОТЧЕТ ПО METAMUSE_MANIFEST_BOT
 * Из LIVE Supabase данных
 */
...
async function createMetaMuseExcelReport() {
  console.log('📊 СОЗДАНИЕ EXCEL ОТЧЕТА ПО METAMUSE_MANIFEST_BOT');
  ...
  try {
    ...
    for (let i = 2; i <= 4; i++) {
      summarySheet.getRow(i).font = { size: 11 };
      summarySheet.getRow(i).alignment = { vertical: 'middle' };
    }
    ...
    // Форматирование RUB
    rubSheet.eachRow((row, rowNumber) => {
    ...
    starsSheet.eachRow((row, rowNumber) => {
      if (rowNumber === 1) return;
      row.font = { size: 10 };
      row.alignment = { vertical: 'middle', wrapText: true };
    });
    ...
    console.log(`   Лист 3: STARS_расходы (${starsOutcomeSample.length} из ${starsOutcome.length} строк)`);
    console.log(`\n💎 ИТОГО:`);
    console.log(`   1️⃣ РАСХОДЫ (STARS): ${Math.round(starsOutcomeSum).toLocaleString()}⭐`);
    console.log(`   2️⃣ ДОХОДЫ В РУБЛЯХ: ${Math.round(rubIncomeSum).toLocaleString()}₽`);
