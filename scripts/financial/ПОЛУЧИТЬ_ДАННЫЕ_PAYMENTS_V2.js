...
async function getRealData() {
  ...
  for (const botName of botNames) {
    ...
    try {
      ...
      console.log(
        `   💰 Реальные доходы: XTR ${realIncomeXTR}, STARS ${realIncomeSTARS}, RUB ${realIncomeRUB}`
      )
      console.log(
        `   📦 Системные доходы: XTR ${systemIncomeXTR}, STARS ${systemIncomeSTARS}, RUB ${systemIncomeRUB}`
      )
      console.log(
        `   💸 Расходы: XTR ${realExpenseXTR}, STARS ${realExpenseSTARS}, RUB ${realExpenseRUB}`
      )
      console.log()
    } catch (err) {
  ...
  const fs = require('fs')
  fs.writeFileSync(
    'real_bot_data_payments_v2.json',
