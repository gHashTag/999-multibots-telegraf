...
async function getAllDataWithPagination(botName) {
  ...
  while (true) {
    ...
    console.log(`   📊 Получено: ${transactions.length}, общий итог: ${allTransactions.length}`);
    ...
    transactions.forEach(t => {
      ...
      if (type === 'MONEY_INCOME') {
        ...
        const isSystem = description.includes('System Grant') || description.includes('Manual') ||
                        description.includes('BONUS') || description.includes('refund') ||
                        description.includes('Refund') || category === 'BONUS' || paymentMethod === 'SYSTEM';
...
async function getAllBotsData() {
  ...
  console.log('💡 Пагинация: получаем по 1000 записей за раз для обработки больших объемов\n');
  ...
  // Сохраняем данные
  const fs = require('fs');
  fs.writeFileSync('all_bots_data_payments_v2.json', JSON.stringify(results, null, 2));
  ...
  results.forEach(bot => {
    ...
    console.log(`  Реальные доходы: XTR ${bot.realIncome.xtr.toFixed(2)}, STARS ${bot.realIncome.stars.toFixed(2)}, RUB ${bot.realIncome.rub.toFixed(2)}`);
    console.log(`  Системные доходы: XTR ${bot.systemIncome.xtr.toFixed(2)}, STARS ${bot.systemIncome.stars.toFixed(2)}, RUB ${bot.systemIncome.rub.toFixed(2)}`);
    console.log(`  Расходы: XTR ${bot.realExpense.xtr.toFixed(2)}, STARS ${bot.realExpense.stars.toFixed(2)}, RUB ${bot.realExpense.rub.toFixed(2)}`);
  });
