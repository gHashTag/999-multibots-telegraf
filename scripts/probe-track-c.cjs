...
async function main() {
  const users = await fetchAll('users', 'telegram_id,username,bot_name,created_at,updated_at,level,vip,voice_id')
  ...
  const pay = await fetchAll('payments_v2', 'telegram_id,stars,type,status,service_type,description,payment_date')
  ...
  console.log(`самая свежая запись в базе: ${new Date(NOW).toISOString().slice(0, 10)}`)
  console.log(`users ${users.length}, model_trainings ${mt.length}, payments ${pay.length}\n`)
  ...
  for (const [id, c] of dups.slice(0, 15)) {
    ...
    console.log(`    ${id} x${c}  боты: ${[...new Set(rows.map(r => r.bot_name))].join(', ')}`)
  }
  ...
  const noId = users.filter(u => u.telegram_id === null || u.telegram_id === undefined || String(u.telegram_id) === '')
  ...
  for (const p of pay) {
    ...
    payers.set(k, (payers.get(k) || 0) + (p.type === 'MONEY_OUTCOME' ? -Number(p.stars || 0) : Number(p.stars || 0)))
  }
  const ghosts = [...payers.keys()].filter(k => !known.has(k))
  console.log(`\n=== платящие без записи в users: ${ghosts.length} ===`)
  console.log('   (баланс у них считается, а профиля нет — бот не знает языка, бота, голоса)')
  for (const g of ghosts.slice(0, 20)) {
    const rows = pay.filter(p => String(p.telegram_id) === g)
    console.log(`    ${g.padEnd(14)} операций ${String(rows.length).padStart(4)}  баланс ${Math.round(payers.get(g) * 100) / 100}`)
  }
  if (ghosts.length > 20) console.log(`    ... ещё ${ghosts.length - 20}`)

  // --- 4. model_trainings: статусы --------------------------------------
  console.log('\n=== model_trainings: статусы ===')
  for (const [s, c] of tally(mt, m => m.status)) console.log(`    ${String(c).padStart(5)}  ${s}`)

  const stuck = mt.filter(m => {
    const s = String(m.status || '').toLowerCase()
    if (['succeeded', 'success', 'completed', 'failed', 'canceled', 'cancelled'].includes(s)) return false
  ...
  for (const m of okNoUrl.slice(0, 15)) {
    console.log(`    id=${String(m.id).padStart(5)}  uid=${String(m.telegram_id).padEnd(12)} ${m.model_name || '—'}  ${String(m.created_at).slice(0, 10)}`)
  }
  ...
  const trainPays = pay.filter(
    p =>
      ...
      /train/i.test(String(p.service_type || '') + ' ' + String(p.description || ''))
  )
  const trained = new Set(mt.map(m => String(m.telegram_id)))
  const paidNoRow = [...new Set(trainPays.map(p => String(p.telegram_id)))].filter(id => !trained.has(id))
  ...
  for (const id of paidNoRow.slice(0, 20)) {
    ...
    console.log(`    ${id.padEnd(14)} платежей ${String(rows.length).padStart(3)}  на ${Math.round(sum * 100) / 100} звёзд`)
  }
  if (paidNoRow.length > 20) console.log(`    ... ещё ${paidNoRow.length - 20}`)
  console.log(`  всего оплат за обучение: ${trainPays.length} на ${Math.round(trainPays.reduce((s, r) => s + Number(r.stars || 0), 0) * 100) / 100} звёзд`)
}
