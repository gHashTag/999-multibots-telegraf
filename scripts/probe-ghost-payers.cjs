...
const show = (title, pairs, lim = 12) => {
  console.log(`\n  ${title}`)
  for (const [k, c] of pairs.slice(0, lim)) console.log(`    ${String(c).padStart(6)}  ${k}`)
...
async function main() {
  ...
  const users = await fetchAll('users', 'telegram_id,username,bot_name,created_at')
  ...
  console.log(`из них с положительным балансом: ${withMoney.length}, суммарно ${totalStars} звёзд`)

  // --- Откуда они? -------------------------------------------------------
  show('способ оплаты', tally(gRows, r => r.payment_method))
  show('бот', tally(gRows, r => r.bot_name))
  show('тип операции', tally(gRows, r => r.type))
  show('описание', tally(gRows, r => String(r.description || '').slice(0, 60)), 15)
  show('по месяцам', tally(gRows, r => String(r.payment_date).slice(0, 7)).sort((a, b) => (a[0] < b[0] ? -1 : 1)), 30)
  ...
  for (const [id, rows] of weird) {
    console.log(`  ${id.padEnd(16)} знаков ${String(id).length}  операций ${rows.length}  баланс ${r2(balance(rows))}`)
    console.log(`      ${String(rows[0].payment_date).slice(0, 10)}  ${rows[0].payment_method}  "${String(rows[0].description || '').slice(0, 50)}"`)
  }
  ...
  const knownNum = new Set(users.map(u => Number(u.telegram_id)).filter(x => !Number.isNaN(x)))
  ...
  // --- Топ по деньгам ----------------------------------------------------
  const top = ghosts
    .map(([id, rows]) => ({ id, bal: r2(balance(rows)), ops: rows.length, first: String(rows[0].payment_date).slice(0, 10) }))
    .sort((a, b) => b.bal - a.bal)
    .slice(0, 20)
  console.log('\n=== 20 самых «богатых» без профиля ===')
  console.log('  id'.padEnd(18) + 'баланс'.padStart(12) + 'операций'.padStart(10) + '  первая')
  for (const t of top) {
    console.log(`  ${t.id.padEnd(16)}${String(t.bal).padStart(12)}${String(t.ops).padStart(10)}  ${t.first}`)
  }

  // --- Сравнение с теми, у кого профиль ЕСТЬ -----------------------------
  const withProfile = [...byUser.entries()].filter(([id]) => known.has(id))
  const avg = arr => (arr.length ? r2(arr.reduce((s, x) => s + x, 0) / arr.length) : 0)
  console.log('\n=== призраки против обычных ===')
  console.log(`  операций на человека:  призраки ${avg(ghosts.map(([, r]) => r.length))}  vs  с профилем ${avg(withProfile.map(([, r]) => r.length))}`)
  console.log(`  баланс на человека:    призраки ${avg(ghosts.map(([, r]) => balance(r)))}  vs  с профилем ${avg(withProfile.map(([, r]) => balance(r)))}`)
}
