...
const MINIO_ENDPOINT = process.env.AWS_ENDPOINT_URL_S3 || 'https://bucket-production-8259.up.railway.app'
const MINIO_BUCKET = process.env.BUCKET_NAME || 'vibee-assets'

const BUCKETS = ['images', 'landingpage', 'leelachakra', 'dev', 'ai-training', 'sync-labs']
...
async function listAll(bucket, prefix = '') {
  ...
  for (;;) {
    ...
    for (const it of page) {
      ...
      if (it.id === null || it.id === undefined) out.push(...(await listAll(bucket, key)))
      else out.push({ key, size: it.metadata?.size ?? 0, type: it.metadata?.mimetype })
    }
...
async function main() {
  ...
  let total = 0, bytes = 0, copied = 0, skipped = 0, failed = 0
  ...
  for (const bucket of BUCKETS) {
    ...
    bytes += b
    console.log(`  ${bucket.padEnd(14)} ${String(objects.length).padStart(4)} objects  ${(b / 1048576).toFixed(1)} MB`)

    if (!APPLY) continue

    for (const o of objects) {
      const destKey = `${bucket}/${o.key}`
      if (await existsInMinio(destKey)) { skipped++; continue }

      const src = `${SUPABASE_URL}/storage/v1/object/public/${bucket}/${o.key}`
      try {
        const r = await fetch(src, { headers: { apikey: KEY, Authorization: `Bearer ${KEY}` } })
        if (!r.ok) { failed++; console.log(`    MISS ${r.status} ${destKey}`); continue }
        const body = Buffer.from(await r.arrayBuffer())
        await s3.send(new PutObjectCommand({
          Bucket: MINIO_BUCKET, Key: destKey, Body: body,
          ContentType: o.type || r.headers.get('content-type') || 'application/octet-stream',
        }))
        copied++
      } catch (e) {
        failed++
  ...
  if (APPLY) console.log(`copied ${copied}, already present ${skipped}, failed ${failed}`)
