...
async function main() {
  ...
  try {
    ...
    log.info('');
    await addSecret(infisicalClient, 'INNGEST_EVENT_KEY', KEYS.INNGEST_EVENT_KEY);

    // 4. Добавляем INNGEST_SIGNING_KEY
    await addSecret(infisicalClient, 'INNGEST_SIGNING_KEY', KEYS.INNGEST_SIGNING_KEY);
    ...
    console.log(`   - INNGEST_EVENT_KEY: ${KEYS.INNGEST_EVENT_KEY.substring(0, 30)}...`);
    console.log(`   - INNGEST_SIGNING_KEY: ${KEYS.INNGEST_SIGNING_KEY.substring(0, 30)}...`);
    console.log('');
    process.exit(1);
  }
}
