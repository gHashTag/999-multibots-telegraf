...
async function addKeysToInfisical() {
  ...
  try {
    ...
    const result1 = await addSecret(accessToken, 'INNGEST_EVENT_KEY', PREDEFINED_KEYS.INNGEST_EVENT_KEY);
    ...
    const result2 = await addSecret(accessToken, 'INNGEST_SIGNING_KEY', PREDEFINED_KEYS.INNGEST_SIGNING_KEY);
    ...
    console.log('');
    console.log(`${colors.green}1. Откройте: https://app.infisical.com/${colors.reset}`);
    console.log(`${colors.green}2. Проект: fd763fa3-35d5-4045-93bd-1795c5f00fc3${colors.reset}`);
...
async function verifyFix() {
  ...
  try {
    ...
    if (logs.includes('INNGEST_SIGNING_KEY')) {
      ...
      log.warn('INNGEST_SIGNING_KEY не найден в логах (подождите ещё 30 секунд)');
    }
...
function finalInstructions() {
  ...
  log.success('=======================================');
  console.log('');
  console.log(`${colors.green}✅${colors.reset} INNGEST_EVENT_KEY добавлен в Infisical`);
  console.log(`${colors.green}✅${colors.reset} INNGEST_SIGNING_KEY добавлен в Infisical`);
