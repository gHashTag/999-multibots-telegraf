...
// Анализ ЖИВОЙ базы: createClient падает с «Invalid URL» на заглушечном
// SUPABASE_URL из общего тестового окружения. Запускаем только при настоящем
// адресе (http/https), иначе честно отмечаем пропуск.
const HAS_REAL_SUPABASE = /^https?:\/\//.test(process.env.SUPABASE_URL || '')
...
async function loadDataFromDatabase() {
  ...
  try {
    ...
    if (expensesError) {
      throw new Error(
        `Ошибка загрузки business_expenses: ${expensesError.message}`
      )
    }

    console.log(`   ✅ Загружено: ${payments?.length || 0} платежей (чистых)`)
    console.log(
      `   ✅ Загружено: ${businessExpenses?.length || 0} бизнес-расходов\n`
    )
...
function analyzeDataQuality(data: any[]): {
  ...
  const totalProblems =
    ...
    anomalies.typeErrors +
    anomalies.zeroAmounts +
    anomalies.negativeAmounts
...
describe.skipIf(!HAS_REAL_SUPABASE)(
  '📊 INVESTMENT ANALYTICS - COMPLETE SUITE',
  () => {
    test('Should generate complete investment analysis with CORRECT LOGIC', async () => {
      console.log('\n' + '='.repeat(80))
      console.log('📊 ЗАПУСК ИНВЕСТИЦИОННОГО АНАЛИЗА')
      console.log(
        '   С ПРАВИЛЬНОЙ ЛОГИКОЙ РАСЧЕТОВ (ПОЛЯ amount/stars/currency)'
      )
      ...
      if (dbData) {
        ...
        console.log(
          `   ✅ После фильтрации: ${rawData.length} записей (из ${dbData.payments.length})`
        )
      } else {
        // Fallback: используем JSON файлы
        console.log('📁 Используем JSON файлы...')
        const allData = JSON.parse(
          fs.readFileSync('payments_data.json', 'utf-8')
        )
        ...
        const {
          ...
          TEST_BOTS,
          FAKE_BOTS,
        } = require('../constants/bots.js')

        console.log(`   📊 PRODUCTION_BOTS: ${PRODUCTION_BOTS.length} ботов`)
        console.log(
          `   📊 TEST_BOTS: ${TEST_BOTS.length} ботов (clip_maker_neuro_bot - основной)`
        )
        console.log(
          `   📊 FAKE_BOTS: ${FAKE_BOTS.length} ботов (исключаются)\n`
        )
        ...
        rawData = allData.filter((row: any) => {
          ...
          if (
            FAKE_BOTS.some(fakeBot =>
              botName.toLowerCase().includes(fakeBot.toLowerCase())
            )
          ) {
            return false
          }
        ...
        const businessExpensesFile = path.join(
          process.cwd(),
          'business_expenses_data.json'
        )
        if (fs.existsSync(businessExpensesFile)) {
          businessExpenses = JSON.parse(
            fs.readFileSync(businessExpensesFile, 'utf-8')
          )
        }
        ...
        rawData = uniqueData
        console.log(
          `   ✅ После фильтрации: ${rawData.length} записей (из ${allData.length})`
        )
      ...
      if (businessExpenses.length > 0) {
        ...
        const totalBusinessCosts = businessExpenses.reduce(
          (sum: number, exp: any) => {
            const amount = Math.abs(parseFloat(exp.amount) || 0)
            const rate =
              exp.currency === 'XTR' || exp.currency === 'STARS' ? 1.8 : 1.0
        ...
        console.log(
          `   💰 Общая сумма расходов: ${Math.round(totalBusinessCosts).toLocaleString()}₽\n`
        )
      }
      ...
      console.log(
        `   💡 Используется: ${cleanedData.length} записей (из ${rawData.length})\n`
      )
      ...
      const businessCostsInRub = businessExpenses.reduce(
        (sum: number, exp: any) => {
          const amount = Math.abs(parseFloat(exp.amount) || 0)
      ...
      Object.entries(revenueByCurrency).forEach(([currency, amount]) => {
        const symbol =
          currency === 'RUB' ? '₽' : currency === 'STARS' ? '⭐' : ''
        console.log(
          `      ${currency}: ${Math.round(amount).toLocaleString()} ${symbol}`
        )
      })
      ...
      console.log(
        `   💼 Бизнес-расходы: ${Math.round(businessCostsInRub).toLocaleString()}₽ (из STARS)`
      )
      ...
      const uniqueUsers = new Set(
        cleanedData.map((row: any) => row.telegram_id)
      ).size
      ...
      cleanedData
        ...
        .forEach((row: any) => {
          ...
          const rate =
            row.currency === 'XTR' || row.currency === 'STARS' ? 1.8 : 1.0
          const revenue = absAmount * rate

          userRevenueMap.set(
            userId,
            (userRevenueMap.get(userId) || 0) + revenue
          )
          userOperationsMap.set(
            userId,
            (userOperationsMap.get(userId) || 0) + 1
          )
      ...
      const botMetrics = [...PRODUCTION_BOTS, ...TEST_BOTS].map(botName => {
        ...
        const botRevenue = cleanedData
          .filter(
            (row: any) =>
              row.bot_name === botName && row.type === 'MONEY_INCOME'
          )
        ...
        const botCosts = cleanedData
          .filter(
            (row: any) =>
              row.bot_name === botName && row.type === 'MONEY_OUTCOME'
          )
          .reduce((sum: number, row: any) => {
            const amount =
              row.currency === 'STARS'
      ...
      const providerMetrics = AI_PROVIDERS.map(provider => {
        ...
        return {
          ...
          percentage:
            totalCosts > 0 ? Math.round((providerCosts / totalCosts) * 100) : 0,
        }
      }).filter(p => p.cost > 0)
      ...
      const currencyBreakdown = currencies
        .map(currency => {
          const currencyRevenue = cleanedData
            .filter(
              (row: any) =>
                row.currency === currency && row.type === 'MONEY_INCOME'
            )
            .reduce((sum: number, row: any) => {
              ...
              const absAmount = Math.abs(amount)
              const rate =
                currency === 'XTR' || currency === 'STARS' ? 1.8 : 1.0
          ...
          return {
            ...
            percentage:
              totalRevenue > 0
                ? Math.round((currencyRevenue / totalRevenue) * 100)
                : 0,
          }
        })
      ...
      console.log(
        `   ✅ Рассчитано: ${uniqueUsers} пользователей, ${botMetrics.length} ботов, ${providerMetrics.length} AI провайдеров\n`
      )
      ...
      summarySheet.getCell('A1').value =
        '📊 ИНВЕСТИЦИОННЫЙ АНАЛИЗ - ИСПОЛНИТЕЛЬНОЕ РЕЗЮМЕ'
      ...
      cleanedData.forEach((row: any) => {
        ...
        if (
          desc.includes('test_data') ||
          desc.includes('system/bonus/testing')
        ) {
          return
        }
      ...
      qualitySheet.getCell('A1').font = {
        size: 16,
        bold: true,
        color: { argb: 'FFFFFF' },
      }
      qualitySheet.getCell('A1').fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'DC2626' },
      }
      ...
      qualitySheet.addRow([
        'Записей в payments_v2',
        cleaningReport.originalCount.toLocaleString(),
        '📊',
      ])
      qualitySheet.addRow([
        'Качество данных',
        `${cleaningReport.qualityScoreBefore.toFixed(1)}/100`,
        cleaningReport.qualityScoreBefore > 90
          ? '✅ ОТЛИЧНО'
          : cleaningReport.qualityScoreBefore > 70
            ? '🟡 ХОРОШО'
            : '⚠️ НУЖНА РАБОТА',
      ])
      ...
      qualitySheet.addRow([
        'Бизнес-расходы',
        businessExpenses.length.toLocaleString(),
        '💼',
      ])
      qualitySheet.addRow([
        'Дубликаты',
        cleaningReport.removedDuplicates.toLocaleString(),
        dbData ? '✅ Уже удалены' : '⚠️ Требуется очистка',
      ])
      qualitySheet.addRow([
        'Нулевые суммы',
        cleaningReport.removedZeroAmounts.toLocaleString(),
        dbData ? '✅ Удалены' : '⚠️ Требуется очистка',
      ])
      ...
      if (dbData) {
        ...
        qualitySheet.addRow([
          'Таблица payments_v2',
          `${dbData.payments.length} записей`,
          'Чистые данные',
        ])
        qualitySheet.addRow([
          'Таблица business_expenses',
          `${dbData.businessExpenses.length} записей`,
          'Реальные расходы',
        ])
      } else {
        qualitySheet.addRow([
...
${currencyBreakdown
  .map(
    curr =>
      `- **${curr.currency}**: ${curr.revenue.toLocaleString()}₽ (${curr.percentage}%)`
  )
  .join('\n')}
...
${topUsers
  ...
  .map(
    (user, i) =>
      `${i + 1}. ID: \`${user.id}\` - ${user.revenue.toLocaleString()}₽ (${user.operations} операций)`
  )
  .join('\n')}
...
`
      ...
      console.log(`\n📊 СОЗДАНО:`)
      console.log(
        `   📄 Excel файл: INVESTMENT_ANALYSIS_REPORT.xlsx (11 листов):`
      )
      ...
      console.log(`   💸 Расходы: ${Math.round(totalCosts).toLocaleString()}₽`)
      console.log(
        `      └─ Из business_expenses: ${Math.round(businessCostsInRub).toLocaleString()}₽`
      )
      ...
      console.log(
        `   🏦 Продакшн: ${PRODUCTION_BOTS.length} | 🧪 Тестовых: ${TEST_BOTS.length}`
      )
      ...
      console.log(`\n🔍 КАЧЕСТВО ДАННЫХ:`)
      console.log(
        `   📊 В payments_v2: ${cleaningReport.originalCount.toLocaleString()} записей (Quality: ${cleaningReport.qualityScoreBefore.toFixed(1)}/100)`
      )
      console.log(
        `   ✅ Очищено в БД: ${dbData ? 'ДА' : 'НЕТ (используем JSON)'}`
      )
