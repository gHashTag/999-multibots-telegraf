...
describe('📊 DATA QUALITY ANALYSIS', () => {
  test('Should perform comprehensive data quality analysis', async () => {
    ...
    console.log(`   ✅ Типы: ${JSON.stringify(fields.map(f => typeof sample[f]))}\n`);
    ...
    const duplicateGroups = Array.from(fullDuplicates.values()).filter(group => group.length > 1);
    const totalFullDuplicates = duplicateGroups.reduce((sum, group) => sum + (group.length - 1), 0);
    const duplicateExamples = duplicateGroups.slice(0, 5).map(group => rawData[group[0]]);
    ...
    console.log(`      ⚠️  Найдено: ${totalFullDuplicates} дубликатов в ${duplicateGroups.length} группах\n`);
    ...
    const partialGroups = Array.from(partialDuplicates.values()).filter(group => group.length > 1);
    const totalPartialDuplicates = partialGroups.reduce((sum, group) => sum + (group.length - 1), 0);
    const partialExamples = partialGroups.slice(0, 5).map(group => rawData[group[0]]);
    ...
    console.log(`      ⚠️  Найдено: ${totalPartialDuplicates} частичных дубликатов\n`);
    ...
    console.log(`      ⚠️  Найдено: ${typeAmountErrors.length} логических ошибок\n`);

    // АНОМАЛИЯ 4: НУЛЕВЫЕ СУММЫ
    console.log('   4️⃣  Поиск нулевых сумм...');
    const zeroAmounts = rawData.filter((row: any) => parseFloat(row.amount) === 0);
    ...
    const negativeAmounts = rawData.filter((row: any) => parseFloat(row.amount) < 0);
    ...
    console.log(`      ⚠️  Найдено: ${negativeAmounts.length} отрицательных сумм\n`);

    // АНОМАЛИЯ 6: НЕИЗВЕСТНЫЕ ТИПЫ
    console.log('   6️⃣  Поиск неизвестных типов операций...');
    const validTypes = ['MONEY_INCOME', 'MONEY_OUTCOME'];
    const unknownTypes = rawData.filter((row: any) => !validTypes.includes(row.type));
    ...
    console.log(`      ⚠️  Найдено: ${unknownTypes.length} записей с неизвестными типами\n`);

    // АНОМАЛИЯ 7: НЕКОРРЕКТНЫЕ ВАЛЮТЫ
    console.log('   7️⃣  Поиск некорректных валют...');
    const validCurrencies = ['XTR', 'STARS', 'RUB'];
    const invalidCurrencies = rawData.filter((row: any) => !validCurrencies.includes(row.currency));
    ...
    console.log(`      ⚠️  Найдено: ${invalidCurrencies.length} записей с некорректными валютами\n`);

    // АНОМАЛИЯ 8: ОЧЕНЬ БОЛЬШИЕ СУММЫ
    console.log('   8️⃣  Поиск аномально больших сумм...');
    const maxAmount = Math.max(...rawData.map((row: any) => Math.abs(parseFloat(row.amount) || 0)));
    const largeThreshold = 10000;
    const largeAmounts = rawData.filter((row: any) => Math.abs(parseFloat(row.amount) || 0) > largeThreshold);
    ...
    anomalies.forEach(anomaly => {
      const weight = anomaly.severity === 'CRITICAL' ? criticalWeight :
                    anomaly.severity === 'HIGH' ? highWeight :
                    anomaly.severity === 'MEDIUM' ? mediumWeight : lowWeight;
      totalPenalty += anomaly.count * weight;
    });

    const maxPossiblePenalty = rawData.length * criticalWeight;
    const qualityScore = Math.max(0, 100 - (totalPenalty / maxPossiblePenalty * 100));
    ...
    console.log(`   ❌ Problematic Records: ${qualityReport.problematicRecords}`);
    console.log(`   📈 Quality Score: ${qualityReport.dataQualityScore}/100`);

    // Статус качества
    let status = 'EXCELLENT';
    let statusIcon = '🟢';
    if (qualityScore < 50) { status = 'POOR'; statusIcon = '🔴'; }
    else if (qualityScore < 70) { status = 'FAIR'; statusIcon = '🟡'; }
    else if (qualityScore < 85) { status = 'GOOD'; statusIcon = '🟠'; }
    ...
    console.log(`   📈 Quality Score: ${qualityReport.dataQualityScore}/100`);
    console.log(`   📊 Общие записи: ${qualityReport.totalRecords.toLocaleString()}`);
    console.log(`   ✅ Корректные: ${qualityReport.validRecords.toLocaleString()}`);
    console.log(`   ❌ Проблемные: ${qualityReport.problematicRecords.toLocaleString()}`);
    console.log(`   🚨 Критические аномалии: ${anomalies.filter(a => a.severity === 'CRITICAL').length}`);
