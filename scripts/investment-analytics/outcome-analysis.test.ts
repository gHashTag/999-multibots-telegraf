...
describe.skipIf(!HAS_PAYMENTS_DUMP)('📊 АНАЛИЗ РАСХОДОВ ПО БОТАМ', () => {
  test('Should analyze all MONEY_OUTCOME records in detail', async () => {
    ...
    const allOutcomes = rawData.filter(
      (row: any) => row.type === 'MONEY_OUTCOME'
    )
    console.log(`   📊 Всего расходов: ${allOutcomes.length}`)
    console.log(
      `   📈 Доля от общего: ${((allOutcomes.length / rawData.length) * 100).toFixed(1)}%\n`
    )
    ...
    allOutcomes.forEach((outcome: any) => {
      ...
      // Валютная разбивка
      analysis.currencyBreakdown[outcome.currency] =
        (analysis.currencyBreakdown[outcome.currency] || 0) + 1
      ...
      if (
        desc.toLowerCase().includes('generation') ||
        desc.toLowerCase().includes('генераци')
      ) {
        category = 'AI_GENERATION'
      } else if (
        desc.toLowerCase().includes('training') ||
        desc.toLowerCase().includes('обучен')
      ) {
        category = 'MODEL_TRAINING'
      } else if (
        desc.toLowerCase().includes('lip') ||
        desc.toLowerCase().includes('синхрон')
      ) {
        category = 'LIP_SYNC'
      } else if (
        desc.toLowerCase().includes('video') ||
        desc.toLowerCase().includes('видео')
      ) {
        category = 'VIDEO_PROCESSING'
      } else if (
        desc.toLowerCase().includes('image') ||
        desc.toLowerCase().includes('изображен')
      ) {
        category = 'IMAGE_PROCESSING'
      } else if (
        desc.toLowerCase().includes('subscription') ||
        desc.toLowerCase().includes('подпис')
      ) {
        category = 'SUBSCRIPTION'
      } else if (
        desc.toLowerCase().includes('refund') ||
        desc.toLowerCase().includes('возврат')
      ) {
        category = 'REFUND'
      } else if (
        desc.toLowerCase().includes('system') ||
        desc.toLowerCase().includes('системн')
      ) {
        category = 'SYSTEM'
      } else if (
        desc.toLowerCase().includes('test') ||
        desc.toLowerCase().includes('тест')
      ) {
        category = 'TEST_DATA'
      }
    ...
    botAnalysis.forEach(analysis => {
      analysis.avgAmount =
        analysis.totalOutcomes > 0
          ? analysis.totalAmount / analysis.totalOutcomes
          : 0
    })
    ...
    const sortedBots = Array.from(botAnalysis.values()).sort(
    ...
    sortedBots.forEach(bot => {
      ...
      console.log(
        `   💰 Общая сумма: ${Math.round(bot.totalAmount).toLocaleString()}`
      )
      console.log(
        `   📊 Средний расход: ${Math.round(bot.avgAmount).toLocaleString()}`
      )

      console.log(`\n   💱 Валюты:`)
      Object.entries(bot.currencyBreakdown).forEach(([curr, count]) => {
        console.log(
          `      ${curr}: ${count} (${((count / bot.totalOutcomes) * 100).toFixed(1)}%)`
        )
      })
      ...
      Object.entries(bot.categories)
        ...
        .forEach(([cat, count]) => {
          console.log(
            `      ${cat}: ${count} (${((count / bot.totalOutcomes) * 100).toFixed(1)}%)`
          )
        })
      ...
      console.log(
        `      Подозрительные описания: ${bot.anomalies.suspiciousDescriptions}`
      )

      console.log(`\n   ✅ Реальные расходы: ${bot.realOutcomes.length}`)
      console.log(`   ❌ Фейковые расходы: ${bot.fakeOutcomes.length}`)
      console.log(
        `   📈 Доля фейка: ${((bot.fakeOutcomes.length / bot.totalOutcomes) * 100).toFixed(1)}%`
      )
      ...
      if (bot.fakeOutcomes.length > 0) {
        ...
        bot.fakeOutcomes.slice(0, 3).forEach((outcome, i) => {
          console.log(
            `      ${i + 1}. [${outcome.currency}] ${outcome.amount} - ${outcome.description.substring(0, 60)}...`
          )
        })
      }
    })
    ...
    const totalAmount = allOutcomes.reduce(
      (sum, o) => sum + Math.abs(parseFloat(o.amount) || 0),
      0
    )
    ...
    console.log(
      `   💰 Общая сумма: ${Math.round(totalAmount).toLocaleString()}`
    )
    console.log(
      `   ✅ Реальные расходы (отриц. суммы): ${realCount.toLocaleString()}`
    )
    console.log(
      `   ❌ Фейковые расходы (положит./нулевые): ${fakeCount.toLocaleString()}`
    )
    console.log(
      `   📊 Доля фейка: ${((fakeCount / totalOutcomes) * 100).toFixed(1)}%\n`
    )
    ...
    Object.entries(Object.fromEntries(allCategories))
      ...
      .forEach(([cat, count]) => {
        console.log(
          `      ${cat}: ${count} (${((count / totalOutcomes) * 100).toFixed(1)}%)`
        )
      })
    ...
    const problematicBots = sortedBots.filter(
      bot => bot.fakeOutcomes.length / bot.totalOutcomes > 0.5
    )

    if (problematicBots.length > 0) {
      problematicBots.forEach(bot => {
        console.log(
          `   ❌ ${bot.botName}: ${((bot.fakeOutcomes.length / bot.totalOutcomes) * 100).toFixed(1)}% фейка`
        )
        console.log(
          `      ${bot.fakeOutcomes.length} из ${bot.totalOutcomes} расходов - подозрительные`
        )
      })
    } else {
    ...
    if (realOutcomes.length > 0) {
      realOutcomes.slice(0, 10).forEach((outcome, i) => {
        console.log(
          `   ${i + 1}. [${outcome.bot_name}] ${outcome.amount} ${outcome.currency}`
        )
    ...
    console.log(
      '❌ ПРИМЕРЫ ФЕЙКОВЫХ РАСХОДОВ (положительные суммы с типом MONEY_OUTCOME):\n'
    )
    ...
    console.log(`   💸 Всего расходов: ${totalOutcomes.toLocaleString()}`)
    console.log(
      `   ✅ Реальных: ${realCount.toLocaleString()} (${((realCount / totalOutcomes) * 100).toFixed(1)}%)`
    )
    console.log(
      `   ❌ Фейковых: ${fakeCount.toLocaleString()} (${((fakeCount / totalOutcomes) * 100).toFixed(1)}%)`
    )
