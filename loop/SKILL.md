...
27. **zsh: переменная `path` связана с PATH** (цикл №66): цикл `for path in …` в скрипте молча затирает PATH, последующие команды падают «command not found» с пустыми кодами. В zsh-скриптах не называть локальные переменные path/PATH-дубликатами (path, PATH, fignore и т.п.) — использовать нейтральные имена (fpath занят zsh-функциями!).
