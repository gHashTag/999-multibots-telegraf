...
32. **CSS-фиксы живут в ЛЕНИВЫХ чанках** (цикл №159): Vite выносит стили страниц в Feed-*.css/Chat-*.css рядом с JS-чанками — проверка деплоя по index-*.css даёт ложную «стагнацию» (2 ложных зонда-PR). Правильная цепочка: имя чанка из графа index-*.js (`grep -o 'Feed-[A-Za-z0-9_-]*\.css'`) → fetch именно его. index.html при этом обновляется напрямую (маркер в исходнике проходит даже без пересборки ассетов).
33. **Кириллица vs латиница в регулярках** (цикл №215): матч по заголовку «РЕТРОСПЕКТИВА КОНВЕЙЕРА» писал как «RETROSPECTIVE КОНВЕЙЕРА» (первое слово латиницей) — регэксп молча не совпадал, секция тихо пропадала. Правило: якорь регулярки по русскому тексту — одно короткое кириллическое слово без латинских дублей («КОНВЕЙЕРА», не «RETROSPECTIVE КОНВЕЙЕРА»), и проверять матч живым прогоном, а не «код выглядит правильно».
