...
15. Merge ветки цикла с main конфликтует в LOOP_REPORT.md (append-only файлы): дописывай разделы ТОЛЬКО в конец и проверяй пункт плана после каждого merge.
...
23. **Доставка фиксов мимо чужой активной ветки** (24.08): временный `git worktree` + симлинк node_modules туда (иначе related-tests валится на UNRESOLVED_IMPORT) → cherry-pick → push своей ветки. related-tests сравнивает с базой падений: URL-объект вместо строки в fetch ломает тест, ждущий строку — читай тест до смены формы аргумента. Крон-харнесс копит невыполенные вызовы: консолидировать в один виток.
