...
28. **Кассовые env тянутся при старте процесса** (цикл №70): локальный 3333, поднятый до установки TOKENS_PAYMENT_BOT_TOKEN, честно говорит «касса не настроена» — инвойс/пакеты проверять на проде (или рестартовать локаль). Вебхук кассира на @t27ai_bot легален: прод-бэкенд держит в polling только боты 1..10 (BOT_TOKEN_11/12 свободны); слёты вебхука ранее были разовыми, лечение — повторный setWebhook + наблюдение витками. Уточнение (виток №75): слёт событийный — рестарт сервисов при деплое общего main; рендер теперь самовосстанавливает вебхук через 30с после старта (PR #669).
29. **Сердцебиение продуктового лупа** (из продуктового лупа ZCode): перед началом витка проверь loop/.zcode-heartbeat — если последний виток был <14 мин назад, это дубликат из очереди крона: обнови файл и выйди no-op (не делай работу и отчёт). Крон-харнесс копит невыполненные вызовы; витки с поллингом рендера/PR длиннее 15-мин интервала. Долгие операции — в фон, виток обязан укладываться в интервал.
30. **macOS: команды `timeout` не существует** (цикл №91): ни timeout, ни gtimeout в системном шелле — обёртка `timeout N cmd` даёт «command not found» и ПУСТУЮ строку на stdout, что выглядит как «API пуст»/«доступ отозван». Для лимитов использовать curl -m/--max-time, а в шелле — проверять существование команды. Перед выводом «доступ потерян» — до 6 ретраев с ростом паузы (8→15с; виток №103: 3×8с не хватило, 6×15с добыло ключ) — флуктуация гостевой сессии бывает длинной. Греп регресса обязан включать FAIL-паттерн: ранний exit скрипта иначе молча «проходит» фильтр ❌|РЕГРЕСС.
31. **Метки времени в отчётах — только от `date -u`** (цикл №124): несколько витков подряд я ставил UTC-время «по ощущению» и уехал на ~4 часа вперёд — LOOP_STATE от автопилота был точен, а мои заголовки нет. Правило: заголовок цикла = вывод `date -u +%Y-%m-%dT%H:%M:%SZ` в момент фиксации, никакой памяти о «сколько прошло». Логи — первый слой правды; Charity Majors 2026: AI требует больше дисциплины, не меньше.
