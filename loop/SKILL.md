...
28. **Кассовые env тянутся при старте процесса** (цикл №70): локальный 3333, поднятый до установки TOKENS_PAYMENT_BOT_TOKEN, честно говорит «касса не настроена» — инвойс/пакеты проверять на проде (или рестартовать локаль). Вебхук кассира на @t27ai_bot легален: прод-бэкенд держит в polling только боты 1..10 (BOT_TOKEN_11/12 свободны); слёты вебхука ранее были разовыми, лечение — повторный setWebhook + наблюдение витками.
