...
- [x] **`note` больше не затирает отказ нигде** — ПОЧИНЕНО. `lastAct`
      ...
      было вовсе.
- [ ] ВЫСОКОЕ. Окно отрицания в 24 знака неверно в ОБЕ стороны: «не надо, и
      оплачивать я не буду» → buy; «Нет, хочу купить сейчас» → buy пропадает.
- [ ] ВЫСОКОЕ. Альбом С ПОДПИСЬЮ отвечается дважды: гвард стоит только в
      ветке «без текста».
