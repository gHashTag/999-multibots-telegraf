...
- [ ] СЛЕДУЮЩИЙ ШАГ (первым делом): разобраться с красным в main —
      это разблокирует и деньги, и всех остальных. Смотреть `chat-memory.ts`
      (правило «offer/deliver → wait, если писали меньше 3 дней назад») и
      фикстуру теста в main.
