...
- [ ] СРЕДНЕЕ. Заготовки ответов на медиа — жёсткий русский текст
      (`businessBotService.ts` MEDIA_KINDS). Текстовый путь развели по языку
      клиента 15.09, этот забыли. Здесь слов клиента нет вовсе, так что
      единственный доступный признак — `language_code` из апдейта; на
      текстовом пути он НЕ годится, а тут годится, и это надо объяснить
      в комментарии.
