...
- [ ] **ВЫСОКОЕ. Ответ ИИ уходит клиенту ПОВЕРХ перехвата владельцем.**
      Флаг takeover ставится, но ход, уже летящий к модели, его не смотрит.
      Владелец пишет клиенту сам — и бот отвечает поверх него.
      `src/services/businessBotService.ts` (ветка takeover и место отправки).
- [ ] **ВЫСОКОЕ. Отправка фото в личку не попадает в зеркало**, и
      `dm-delivery.test.ts` этого не видит (слепой сторож).
- [ ] СРЕДНЕЕ. «Платил ли человек» считается по архиву `payments_v2` —
      покупки после 09.09.2026 не видны (`crm-tools.ts:206-211`).
...
- [ ] СРЕДНЕЕ. Отказывает чужому не `crm_offer` — а он шлёт фото и тратит.
...
- [ ] НИЗКОЕ. `crm-owner-gate.test.ts` слеп ровно для `crm_offer`.
