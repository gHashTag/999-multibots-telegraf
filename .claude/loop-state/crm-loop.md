...
- цикл 7 записан целиком — см. раздел «Цикл 7» в конце файла
- 16.09.2026 ~01:xx: ИДЁТ аудит слабых мест (workflow `ws9ccnsff`, 12 агентов,
  6 линз + скептики). Первые находки уже есть и они серьёзные:
  - `stageOf` получает ОДНО касание, а правило про отказ читает всю историю
  - клиент, которому бот уже ответил, в очереди `ours` может застрять навсегда
  - касание `kind: 'note'` ломает стадию — а я сегодня начал писать именно
    такие (`crm-subscription.ts`, `managed-bots`), значит мог сделать хуже
  - механизм отмены касания (`reverts_id`, `effectiveTouches`) собран целиком
    и НЕ ИМЕЕТ НИ ОДНОГО ПИСАТЕЛЯ — это форма 20 из blind-guards, и построил
    её я
- СЛЕДУЮЩИЙ ВИТОК: дождаться вердиктов скептиков, составить план по выжившим,
  чинить начиная с касания `note` (оно уже в проде ветки)
