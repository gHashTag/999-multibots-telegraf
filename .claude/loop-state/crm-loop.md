...
- [ ] СРЕДНЕЕ. `recordedQuestion` угадывает по исключению; рендер пишет
      вопрос ДО агента, поэтому при обрыве потока вопрос дублируется.
- [ ] СРЕДНЕЕ. `redeemInvoiceFor` не отличает подписку от разового счёта.
