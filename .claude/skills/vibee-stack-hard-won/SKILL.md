...
Отдельно названа цена другого решения: `catch` в той же функции возвращает
`true`, то есть при падении проверки баланса платная функция становится
бесплатной для всех на время сбоя. Обратный выбор блокировал бы платящих из-за
чужой аварии. Это решение владельца продукта, а не рефакторинг.
