...
Счёт 1230/576 не меняется НИ ПРИ КАКОЙ конфигурации. Опровергнуты ПЯТЬ гипотез:

4. «дело в самом импорте» — минимальный файл с
   `import { vi, Mock } from 'vitest'` и `f as Mock` под конфигом проекта
   компилируется БЕЗ ЕДИНОЙ ошибки;
5. «виноваты 28 файлов, тянущих типы jest» — исключил их, общий счёт упал
   1230 → 1185, а TS2709 остались ровно 576.

САМОЕ ВАЖНОЕ, что удалось установить: файл `robokassaWebhook.test.ts`,
скомпилированный В ОДИНОЧКУ под тем же конфигом, даёт 6 ошибок и НИ ОДНОЙ
TS2709. То есть это эффект ВСЕЙ ПРОГРАММЫ: что-то среди сотни тестовых файлов
вносит глобальный `Mock`, перекрывающий импортированный тип во всех остальных.

Бисекция ПРОВЕДЕНА и дала результат:

    только 152 тестовых файла + подопытный  → TS2709: 0
    весь src/**/*.ts + подопытный           → TS2709: 576
    весь src/**/*.ts БЕЗ тестов             → TS2709: 4

То есть эффект вносят НЕ тесты, а продакшн-дерево. И все 4 ошибки в чистом
прогоне — в одном файле: `src/inngest_app/test/utils/test-helpers.ts`, строки
45-48, объявление `interface MockLogger { info: Mock; error: Mock; ... }`.

Файл импортирует ТОЛЬКО из vitest (`import type { Mock } from 'vitest'`, стр.11),
типы jest в программу не попадают вовсе. При этом минимальный файл с тем же
импортом компилируется чисто. Значит различие в чём-то, что вносит остальное
дерево `src/`.

Цель сузилась с 576 ошибок в 54 файлах до 4 в одном. Но бисекция по каталогам
дальше НЕ ПОШЛА: ни `src/**/*.ts`, ни `src/inngest_app/**`, ни любой отдельный
каталог с этим файлом не воспроизводят TS2709 — везде 0. Воспроизводит только
сам `tsconfig.test.json` целиком.

⚠️ ЛОВУШКА, В КОТОРУЮ Я ПОПАЛ ЗДЕСЬ ЖЕ. Я решил, что причина в `typeRoots`, и
«подтвердил» это замером: с ним 576, без него 0. Замер был ложным. Скрипт,
который должен был сгенерировать конфиг без `typeRoots`, УПАЛ (в
`tsconfig.test.json` есть комментарии, `json.load` их не ест), и `tsc`
прочитал ОСТАВШИЙСЯ ОТ ПРЕДЫДУЩЕГО ШАГА `tsconfig.b.json` с узким `include`.
Ноль пришёл оттуда. Убрал `typeRoots` по-настоящему — счёт не изменился,
1230/576.

Правило: **если шаг, готовящий вход, упал — результат следующего шага
недействителен, даже если он выглядит осмысленно.** Временные файлы удаляй
сразу после использования, иначе следующий прогон прочитает вчерашний.

ИСЧЕРПАНО СЕМЬ ГИПОТЕЗ. Не тратьте цикл на повтор:
1. форма импорта (`import type`) — 54 файла переписаны, счёт тот же;
2. `typeRoots` — убран по-настоящему, счёт тот же;
3. `moduleResolution: bundler` вместо node — тот же;
4. 28 файлов с типами jest — исключены, TS2709 те же 576;
5. минимальный файл с тем же импортом — компилируется ЧИСТО;
6. ~~бисекция по каталогам — везде 0~~ — ЭТОТ РЕЗУЛЬТАТ НЕДЕЙСТВИТЕЛЕН.
   Перепроверка показала: тот же `include: ["src/**/*.ts"]` даёт 576, а не 0.
   Вчерашние нули пришли оттого, что shell-функция, писавшая `tsconfig.b.json`,
   срабатывала не каждый раз, и `tsc` читал ПЕРВЫЙ, узкий конфиг. Та же
   ловушка со устаревшим временным файлом — и я попал в неё повторно, в тот же
   день, когда её записал;
7. явный include `node_modules/vitest/globals.d.ts` — без него 576 остаются,
   а общий счёт РАСТЁТ до 1340 (теряются глобальные describe/it).

`--traceResolution` показывает корректное разрешение:
`vitest` → `node_modules/vitest/dist/index.d.ts@3.2.4+@types/debug@4.1.12+@types/node@20.19.43`.

Бисекция по КОНФИГУ проведена и дала однозначное: **576 воспроизводятся на
минимальном конфиге** — `extends: ./tsconfig.json`, `include: src/**/*.ts`,
`exclude: [dist, apps]`. Ни `types`, ни `typeRoots`, ни включение
`globals.d.ts` роли не играют: базовый `tsconfig.json` уже содержит
`types: ["node"]`.

Значит причина в САМОМ базовом конфиге в сочетании с тем, что тестовые файлы
попадают в программу (1335 файлов). Следующий заход — бисекция по свойствам
`tsconfig.json`, а не по надстройке над ним.

И главное правило этого захода: **после КАЖДОЙ записи временного конфига
проверяй, что записалось именно то.** Два раза подряд молчаливый сбой генератора
дал правдоподобные, но ложные результаты.
