...
Цель сузилась с 576 ошибок в 54 файлах до 4 ошибок в одном. Следующий заход:
бисекция уже по НЕтестовому `src` — половина каталогов, потом четверть, — и
`tsc --traceResolution` на `vitest` из этого конкретного файла.
