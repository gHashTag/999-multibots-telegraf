...
6. бисекция по каталогам — ни один каталог не воспроизводит, везде 0;
...
Воспроизводит ТОЛЬКО полный `tsconfig.test.json`; ни один его отдельный элемент
— нет. Значит дело в сочетании, а не в одном параметре. Следующий заход:
собирать конфиг с нуля минимальными шагами, добавляя по одному свойству, пока
576 не появятся. Это и есть настоящая бисекция здесь — по КОНФИГУ, а не по
файлам.
