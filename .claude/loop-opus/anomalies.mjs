...
const новые = стало.filter(k => !прежние.includes(k))
const ушедшие = прежние.filter(k => !стало.includes(k))
...
if (!новые.length && !ушедшие.length) {
  console.log(
    история.length ? '   набор аномалий не менялся' : '   первый запуск, сравнивать не с чем'
  )
}
