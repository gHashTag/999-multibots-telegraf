...
# Resource usage
ssh -i ~/.ssh/selectel root@185.161.67.53 'docker stats 999-multibots --no-stream'

# Critical errors (last 500 lines)
ssh -i ~/.ssh/selectel root@185.161.67.53 'docker logs 999-multibots --tail 500 2>&1 | grep -E "(Error|Exception|TypeError|ReferenceError|failed|ECONNREFUSED|ETIMEDOUT|UnhandledPromiseRejection|Fatal|Crash|Terminated)" | head -20'

# AI Photoshop functionality check
ssh -i ~/.ssh/selectel root@185.161.67.53 'docker logs 999-multibots 2>&1 | grep -i "wasAllModels\|all_models.*restored" | tail -10'

# Recent meaningful activity
ssh -i ~/.ssh/selectel root@185.161.67.53 'docker logs 999-multibots 2>&1 | tail -100 | grep -v "Запрос к Supabase\|Нет уведомлений" | head -20'
```
