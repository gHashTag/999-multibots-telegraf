...
describe('generateNeuroImage', () => {
  ...
  beforeEach(() => {
    ...
    ctx = {
      session: { prompt: 'sess prompt', userModel: 'murl' },
    ...
    generateNeuroImage =
      require('@/services/generateNeuroImage').generateNeuroImage
  })
  ...
  it('throws error when numImages is zero', async () => {
    await expect(
      generateNeuroImage(prompt, model_url, 0, telegram_id, ctx, botName)
  ...
  it('handles axios error, logs and replies', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      'Ошибка при генерации нейроизображения:',
      err
    )
