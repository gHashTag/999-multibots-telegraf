...
import { rublePaymentScene } from '../../src/scenes/paymentScene/rublePaymentScene' // ОТНОСИТЕЛЬНЫЙ ПУТЬ
import { MyContext, MySession, UserModel, ModelUrl, ModeEnum } from '../../src/interfaces' // ОТНОСИТЕЛЬНЫЙ ПУТЬ
...
jest.mock('../../src/price/helpers', () => ({
  ...
  // Добавь другие нужные моки, если они используются в rublePaymentScene
}));
...
describe('Ruble Payment Scene', () => {
  ...
  beforeEach(() => {
    ...
    mockUpdate = { // Присваиваем значение здесь
      update_id: 2,
      callback_query: {
        id: 'cb1',
        from: {
    ...
    mockUpdate.message = {
        message_id: 1, date: Date.now(),
        from: { id: 456, is_bot: false, first_name: 'Test2', language_code: 'ru' },
        chat: { id: 456, type: 'private', first_name: 'Test2' }, text: ''
    };
    ...
    ctx.session = {
      ...
      userModel: { model_name: 'm1', trigger_word: 't1', model_url: 'o/r:v' as ModelUrl },
      email: undefined,
      steps: undefined,
  ...
  it('should enter the scene and call handleSelectRubAmount', async () => {
    ...
    expect(handlers.handleSelectRubAmount).toHaveBeenCalledWith({ ctx, isRu: true })
    expect(ctx.scene.leave).not.toHaveBeenCalled()
  })

  it('should handle error during enterMiddleware and leave the scene', async () => {
    // Мокаем конкретную функцию из handlers
    (handlers.handleSelectRubAmount as jest.Mock).mockRejectedValueOnce(enterError); // Используем enterError
    ...
    expect(ctx.reply).toHaveBeenCalledWith('Произошла ошибка. Попробуйте позже.', undefined)
  ...
  it('should handle selecting a ruble amount via action callback', async () => {
    ...
    // Мокаем функции явно
    (priceHelpers.getInvoiceId as jest.Mock).mockResolvedValue(invoiceUrl); // Мокаем конкретную функцию
    (supabase.setPayments as jest.Mock).mockResolvedValue({
        ...setPaymentsSuccessResponse,
        data: [{ ...setPaymentsSuccessResponse.data![0], inv_id: invId, out_sum: amountRub.toString(), stars: stars }],
    });
    ...
    // Проверяем мок явно
    expect(priceHelpers.getInvoiceId).toHaveBeenCalledWith( // Проверяем конкретную функцию
      config.MERCHANT_LOGIN,
      amountRub,
      expect.any(Number),
      expect.stringContaining(`Пополнение баланса на ${stars} звезд`), // Убираем String()
      config.PASSWORD1
    )
    expect(supabase.setPayments).toHaveBeenCalledWith(
      expect.objectContaining({
        telegram_id: '456',
  ...
  it('should handle error when Robokassa invoice generation fails', async () => {
    const amountRub = 200
    // const invoiceError = new Error('Invoice generation failed') // Уже объявлена выше
    (priceHelpers.getInvoiceId as jest.Mock).mockRejectedValue(invoiceError); // Мокаем конкретную функцию и используем invoiceError
    ...
    ctx.updateType = 'callback_query';
    // @ts-ignore
    ctx.update.callback_query = { ...mockUpdate.callback_query, data: `top_up_rub_${amountRub}` };
     // @ts-ignore
    ctx.match = [`top_up_rub_${amountRub}`, amountRub.toString()];
    ...
    expect(ctx.answerCbQuery).toHaveBeenCalledTimes(1)
    expect(priceHelpers.getInvoiceId).toHaveBeenCalledTimes(1) // Проверяем конкретную функцию
    expect(supabase.setPayments).not.toHaveBeenCalled()
    expect(ctx.reply).toHaveBeenCalledWith(
    ...
    expect(ctx.scene.leave).toHaveBeenCalledTimes(1)
    expect(mockedLogger.error).toHaveBeenCalledWith(
        expect.stringContaining(`Error processing callback top_up_rub_${amountRub}`),
        expect.objectContaining({ error: invoiceError.message, telegram_id: 456 }) // Используем invoiceError.message
    )
  })
  ...
  it('should handle selecting an invalid ruble amount option', async () => {
    ...
    ctx.updateType = 'callback_query';
    // @ts-ignore
    ctx.update.callback_query = { ...mockUpdate.callback_query, data: `top_up_rub_${invalidAmount}` };
     // @ts-ignore
    ...
    expect(ctx.answerCbQuery).toHaveBeenCalledTimes(1)
    expect(priceHelpers.getInvoiceId).not.toHaveBeenCalled() // Проверяем конкретную функцию
    expect(supabase.setPayments).not.toHaveBeenCalled()
    expect(ctx.reply).toHaveBeenCalledWith(
