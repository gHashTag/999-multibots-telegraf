...
describe('getAiFeedbackFromSupabase - input validation and env', () => {
  ...
  it('throws if assistant_id is missing', async () => {
    // @ts-ignore
    await expect(
      getAiFeedbackFromSupabase({ ...validParams, assistant_id: '' })
  ...
  it('throws if report is missing', async () => {
    // @ts-ignore
    await expect(
      getAiFeedbackFromSupabase({ ...validParams, report: '' })
  ...
  it('throws if language_code is missing', async () => {
    // @ts-ignore
    await expect(
      getAiFeedbackFromSupabase({ ...validParams, language_code: '' })
  ...
  it('throws if full_name is missing', async () => {
    // @ts-ignore
    await expect(
      getAiFeedbackFromSupabase({ ...validParams, full_name: '' })
    ).rejects.toThrow('Full name is not set')
  })

  it('throws if OPENAI_API_KEY is not set', async () => {
    delete process.env.OPENAI_API_KEY
    await expect(getAiFeedbackFromSupabase(validParams)).rejects.toThrow(
