...
const mockOrder = jest.fn<() => Promise<{ data: Payment[] | null; error: any }>>()
...
describe('getPaymentsInfoByTelegramId', () => {
  ...
  it('returns empty array and logs error when query fails', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith('Ошибка при получении информации о платежах пользователя:', fakeError)
