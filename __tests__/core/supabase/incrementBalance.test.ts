describe('incrementBalance', () => {
  ...
  beforeEach(() => {
    ...
    incrementBalance =
      require('@/core/supabase/incrementBalance').incrementBalance
  })

  it('throws when select errors or no data', async () => {
    // simulate select error
    // simulate select error
    builderSelect.single.mockResolvedValue({
      data: null,
      error: { message: 'fail' },
    })
    await expect(incrementBalance({ telegram_id, amount })).rejects.toThrow(
      'Не удалось получить текущий баланс'
    )
  })
  ...
  it('resolves when update succeeds', async () => {
    ...
    await expect(
      incrementBalance({ telegram_id, amount })
