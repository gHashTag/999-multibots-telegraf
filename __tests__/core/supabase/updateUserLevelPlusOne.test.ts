describe('updateUserLevelPlusOne', () => {
  ...
  beforeEach(() => {
    ...
    updateUserLevelPlusOne =
      require('@/core/supabase/updateUserLevelPlusOne').updateUserLevelPlusOne
  })
  ...
  it('increments level and logs success when no error', async () => {
    ...
    await expect(
      updateUserLevelPlusOne(telegram_id, level)
  ...
  it('logs error when update returns error', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      'Ошибка обновления уровня пользователя:',
      err
    )
  })
