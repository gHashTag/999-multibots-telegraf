...
describe('getSubtitles', () => {
  ...
  it('returns parsed JSON when response has valid JSON content', async () => {
    const data = { subtitles: [{ startTime: '00:00:01,000', endTime: '00:00:02,000', text: 'Hello' }] }
  ...
  it('throws error when content is null', async () => {
    ...
    await expect(getSubtitles('test', 5)).rejects.toThrow('Received null content from OpenAI')
  })
})