...
describe('answerAi', () => {
  ...
  const userData = { username: 'u', first_name: 'f', last_name: 'l', company: 'c', position: 'p', designation: 'd' }
  const prompt = 'hello'
  ...
  it('returns content when GPT provides a message', async () => {
    ...
    const result = await answerAi(model, userData, prompt, languageCode, systemPrompt)
