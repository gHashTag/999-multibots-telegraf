...
describe('Additional OpenAI modules', () => {
  ...
  it('getSubtitles parses JSON content', async () => {
    const arr = ['a', 'b']
    createSpy.mockResolvedValue({ choices: [{ message: { content: JSON.stringify(arr) } }] })
  ...
  it('getTriggerReel returns string content', async () => {
    createSpy.mockResolvedValue({ choices: [{ message: { content: 'reel text' } }] })
  ...
  it('getSlides parses JSON content', async () => {
    ...
    createSpy.mockResolvedValue({ choices: [{ message: { content: JSON.stringify(slides) } }] })
  ...
  it('upgradePrompt returns content string', async () => {
    createSpy.mockResolvedValue({ choices: [{ message: { content: 'upgraded' } }] })
