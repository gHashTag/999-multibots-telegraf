...
describe('getSlides', () => {
  ...
  it('returns parsed JSON when response has valid JSON content', async () => {
    ...
    const result = await getSlides({ prompt: 'slide prompt', scenesCount: 1, isDescription: false })
    expect(result).toEqual(data)
  })

  it('throws error when content is null', async () => {
    mockCreate.mockResolvedValue({ choices: [{ message: { content: null } }] })
    await expect(getSlides({ prompt: 'test', scenesCount: 2, isDescription: true })).rejects.toThrow('Received null content from OpenAI')
  })
})