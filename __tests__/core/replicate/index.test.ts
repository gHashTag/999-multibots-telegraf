...
describe('core/replicate index', () => {
  ...
  it('initializes Replicate with auth from env', () => {
    ...
    expect(mockReplicateConstructor).toHaveBeenCalledWith({ auth: 'replicate-token' })
    expect((replicateModule.replicate as any).opts).toEqual({ auth: 'replicate-token' })
  })
  ...
  it('exports modelPricing mapping', () => {
    ...
    expect(modelPricing['stability-ai/stable-diffusion-3.5-large-turbo']).toBe('$0.040 / image')
  ...
  it('models.getInput returns correct dimensions for various aspect ratios', () => {
    ...
    expect(in1).toMatchObject({ prompt: 'hello', aspect_ratio: '1:1', width: 1024, height: 1024 })
    // 16:9 aspect ratio
    const in2 = models.flux.getInput('world', '16:9')
    expect(in2).toMatchObject({ prompt: 'world', aspect_ratio: '16:9', width: 1368, height: 768 })
    // 9:16 aspect ratio
    const in3 = models.flux.getInput('test', '9:16')
    expect(in3).toMatchObject({ prompt: 'test', aspect_ratio: '9:16', width: 768, height: 1368 })
    // default aspect ratio (undefined) should use 16:9
    const in4 = models.flux.getInput('auto', undefined as any)
    expect(in4).toMatchObject({ prompt: 'auto', aspect_ratio: '16:9', width: 1368, height: 768 })
    // unknown aspect ratio should default to fallback dimensions
    const in5 = models.flux.getInput('unknown', '4:3')
    expect(in5).toMatchObject({ prompt: 'unknown', aspect_ratio: '4:3', width: 1368, height: 1024 })
  })
