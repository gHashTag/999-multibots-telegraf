...
export const setupHearsHandlers = (bot: Telegraf<MyContext>) => {
  ...
  bot.on('callback_query', async (ctx: MyContext, next) => {
    ...
    logger.debug('🔍 [GLOBAL CALLBACK] Checking wizard:', {
      hasScene: !!ctx.scene,
    ...
    if (ctx.scene?.current?.id) {
      logger.debug('🚨 [GLOBAL CALLBACK] Active wizard detected, passing to wizard:', {
        wizardId: ctx.scene.current.id,
        callback: data,
      })
  ...
  bot.hears(menuButtonTexts, async (ctx: MyContext) => {
    ...
    logger.debug('🎯 [SIMPLE MENU] Button pressed:', text)

    // Передаем кнопку в новую простую функцию
    const handled = await handleMenuButtonPress(ctx, text)

    if (handled) {
      logger.debug('✅ [SIMPLE MENU] Button handled successfully')
    } else {
      logger.debug('⚠️ [SIMPLE MENU] Button not recognized:', text)
    }
  })
}
//
