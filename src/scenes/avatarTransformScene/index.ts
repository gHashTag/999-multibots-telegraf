...
const createMarvelPromptByGender = (
  ...
  if (!heroPrompt) {
    ...
    console.error(`🚨 [HERO VALIDATION ERROR] Hero "${heroName}" is in heroes list but has NO prompt!`, {
      ...
      heroesListLength: maleHeroes.length + femaleHeroes.length
    })
  ...
  async ctx => {
    ...
    if (receivedText === (isRu ? '🎲 Случайный стиль' : '🎲 Random style')) {
      ...
      const buttonToHeroMap: Record<string, string> = {
        ...
        // Fallback для неизвестных героев (для мужских)
        ...
        // ========== ФОЛБЭК МАППИНГИ ДЛЯ ВСЕХ НЕДОСТАЮЩИХ ГЕРОЕВ ==========
        // Games - добавляем стандартные кнопки для Соника и Марио
        '🎨 Соник': 'Соник',
        '🎨 Марио': 'Марио',
        
        // Marvel (missing from translations) 
        ...
        // DC (missing from translations)
        '🎨 Киборг': 'Киборг',
        
        // Slavic (missing from translations)
