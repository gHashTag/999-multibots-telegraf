 ...
 */
export const balanceNotifierScene = new Scenes.BaseScene<MyContext>(
  BALANCE_NOTIFIER_SCENE_ID
)
...
balanceNotifierScene.enter(async ctx => {
  ...
  try {
    ...
    const notificationEnabled =
      ctx.session.balanceNotifications?.enabled || false
    const notificationThreshold =
      ctx.session.balanceNotifications?.threshold || 10
...
balanceNotifierScene.action('toggle_notifications', async ctx => {
  ...
  ctx.session.balanceNotifications.enabled =
    !ctx.session.balanceNotifications.enabled
