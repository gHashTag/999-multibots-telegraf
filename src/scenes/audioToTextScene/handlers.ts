...
import {
  transcribeAudioBasic,
  transcribeLongAudioBasic,
  ...
  try {
    ...
    if (ctx.message) {
      if ('voice' in ctx.message && ctx.message.voice) {
        ...
        if (
          ![...SUPPORTED_AUDIO_FORMATS, ...SUPPORTED_VIDEO_FORMATS].includes(
            fileType
          )
        ) {
          await ctx.reply(
    ...
    // Получаем файл
    await ctx.reply(isRu ? '⏳ Получаю файл...' : '⏳ Getting the file...')
      ...
      Markup.inlineKeyboard([
        [
          Markup.button.callback(
            isRu ? 'Язык: Автоопределение' : 'Language: Auto-detect',
            CALLBACKS.LANG_AUTO
          ),
        ],
        [
          Markup.button.callback(
            isRu ? 'Модель: Whisper Medium' : 'Model: Whisper Medium',
            CALLBACKS.MODEL_MEDIUM
          ),
        ],
        [
          Markup.button.callback(
            isRu ? 'Точность: Средняя' : 'Accuracy: Medium',
            CALLBACKS.ACCURACY_MEDIUM
          ),
        ],
        [
          Markup.button.callback(
            isRu ? 'Начать транскрипцию' : 'Start transcription',
            CALLBACKS.START_TRANSCRIPTION
          ),
        ],
      ])
    )
    ...
    ctx.session.audioToText.transcriptionModel =
      TranscriptionModels.WHISPER_MEDIUM
...
export async function transcriptionHandler(ctx: MyContext) {
  try {
    ...
    if (callbackData === CALLBACKS.LANG_AUTO) {
      ctx.session.audioToText.transcriptionLanguage =
      ...
      await ctx.editMessageReplyMarkup({
        inline_keyboard: [
          ...
          [
            Markup.button.callback(
              isRu ? 'Язык: Русский' : 'Language: Russian',
              CALLBACKS.LANG_RU
            ),
          ],
          [
            Markup.button.callback(
              isRu ? 'Язык: Английский' : 'Language: English',
              CALLBACKS.LANG_EN
            ),
          ],
          [
            Markup.button.callback(
              isRu ? 'Назад к настройкам' : 'Back to settings',
              CALLBACKS.SETTINGS
            ),
          ],
        ],
      ...
      ctx.session.audioToText.transcriptionLanguage =
        TranscriptionLanguages.RUSSIAN
      await ctx.reply(
      ...
      ctx.session.audioToText.transcriptionLanguage =
        TranscriptionLanguages.ENGLISH
      await ctx.reply(
    ...
    if (callbackData === CALLBACKS.MODEL_TINY) {
      ctx.session.audioToText.transcriptionModel =
        TranscriptionModels.WHISPER_TINY
      await ctx.reply(
        isRu
          ? '✅ Выбрана модель: Whisper Tiny'
          : '✅ Selected model: Whisper Tiny'
      )
      return
    } else if (callbackData === CALLBACKS.MODEL_BASE) {
      ctx.session.audioToText.transcriptionModel =
        TranscriptionModels.WHISPER_BASE
      await ctx.reply(
        isRu
          ? '✅ Выбрана модель: Whisper Base'
          : '✅ Selected model: Whisper Base'
      )
      return
    } else if (callbackData === CALLBACKS.MODEL_SMALL) {
      ctx.session.audioToText.transcriptionModel =
        TranscriptionModels.WHISPER_SMALL
      await ctx.reply(
        isRu
          ? '✅ Выбрана модель: Whisper Small'
      ...
      await ctx.reply(
        isRu
          ? '✅ Выбрана модель: Whisper Medium'
          : '✅ Selected model: Whisper Medium'
      )
      return
    } else if (callbackData === CALLBACKS.MODEL_LARGE) {
      ctx.session.audioToText.transcriptionModel =
        TranscriptionModels.WHISPER_LARGE
      await ctx.reply(
        isRu
          ? '✅ Выбрана модель: Whisper Large'
    ...
    if (callbackData === CALLBACKS.START_TRANSCRIPTION) {
      ...
      const settings: TranscriptionSettings = {
        model:
          (ctx.session.audioToText
            .transcriptionModel as unknown as TranscriptionModels) ||
          defaultModel,
        language:
          (ctx.session.audioToText
            .transcriptionLanguage as unknown as TranscriptionLanguages) ||
          defaultLanguage,
        accuracy:
          (ctx.session.audioToText.accuracy as 'low' | 'medium' | 'high') ||
          defaultAccuracy,
      }
