...
export const subscriptionScene = new Scenes.WizardScene<MyContext>(
  ...
  async ctx => {
    logger.info('➡️ [SubScene Step 1] Entered scene', { userId: ctx.from?.id })
    ...
    try {
      ...
      logger.info(
        '➡️ [SubScene Step 1] Leaving scene due to error displaying tariffs...'
      )
      return ctx.scene.leave() // Выходим из сцены при ошибке
    }
    logger.info('➡️ [SubScene Step 1] Moving to next step...')
    return ctx.wizard.next()
  },
  async ctx => {
    logger.info('➡️ [SubScene Step 2] Entered step', { userId: ctx.from?.id })
    ...
    if ('callback_query' in ctx.update && 'data' in ctx.update.callback_query) {
      ...
      if (text === 'neurobase') {
        ...
        logger.info(
          '➡️ [SubScene Step 2 CB] Intending to enter PaymentScene (NeuroBase)...'
        )
        ...
        logger.info(
          '➡️ [SubScene Step 2 CB] Intending to enter PaymentScene (NeuroPhoto)...'
        )
        return ctx.scene.enter(ModeEnum.PaymentScene)
      } else if (text === 'mainmenu') {
        console.log('Selected: 🏠 Главное меню')
        logger.info(
          '➡️ [SubScene Step 2 CB] Intending to enter MainMenu directly...'
        )
        return ctx.scene.enter(ModeEnum.MainMenu)
      } else {
        logger.warn('➡️ [SubScene Step 2 CB] Unknown callback data', { text })
      ...
      logger.info('➡️ [SubScene Step 2 Text] Received non-callback query')
      ...
      // Игнорируем текстовые сообщения или команды внутри этой сцены
      // handleMenu(ctx) // Убираем вызов handleMenu
      await ctx.reply(
        isRussian(ctx)
          ? 'Пожалуйста, выберите один из тарифов или вернитесь в меню с помощью кнопок.'
          : 'Please select a tariff or return to the menu using the buttons.'
      )
      // Остаемся в той же сцене
      return
    }
  }
)
