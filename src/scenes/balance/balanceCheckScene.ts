 ...
 */
export const balanceCheckScene = new Scenes.BaseScene<MyContext>('balance-check');
...
balanceCheckScene.enter(async (ctx) => {
  try {
    ...
    if (!user) {
      await ctx.reply('User not found in database. Please start the bot with /start command.');
      return await ctx.scene.leave();
    }
