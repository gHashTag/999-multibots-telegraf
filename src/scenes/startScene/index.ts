...
const startScene = new Scenes.WizardScene<MyContext>(
  ...
  async (ctx) => {
    ...
    logger.debug('✅ [startScene] Показываем главное меню:', {
      telegramId,
    ...
    try {
      ...
      try {
        ...
        logger.warn('⚠️ Не удалось получить имя бота:', error)
      }
      ...
      try {
        ...
        if (customWelcome) {
          logger.debug('✅ [startScene] Получено уникальное приветствие из avatars:', {
            botName,
            messageLength: customWelcome.length
          })
        }
      } catch (error) {
        logger.warn('⚠️ [startScene] Не удалось получить приветствие из avatars:', error)
      }
      ...
      // Логируем статистику
      logger.debug('✅ [startScene] Меню отправлено успешно')

    } catch (error) {
      logger.error('❌ [startScene] Ошибка при показе меню:', error)
