...
Or, if you prefer to go solo, click "Proceed to Subscription".`
    ...
    // Убираем явный выход из сцены здесь. Сцена должна оставаться активной,
    // чтобы обработать callback_query от инлайн-кнопок.
    // Если пользователь введет другую команду, глобальный обработчик команды
    // должен будет сам позаботиться о выходе из текущей сцены (ctx.scene.leave()).
  }
)
