...
function handleError(error: unknown, ctx: MyContext) {
  logger.error('❌ Ошибка в обработчике', {
    description: 'Error in handler',
  ...
  if (error instanceof Error) {
    errorMessage(error, String(ctx.from?.id), isRussian)
  } else {
    errorMessage(new Error(String(error)), String(ctx.from?.id), isRussian)
  }
}
