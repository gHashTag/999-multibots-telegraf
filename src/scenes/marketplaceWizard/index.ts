...
const mainMenuStep = async (ctx: MyContext) => {
  ...
  await ctx.reply(
    isRu ? '🛒 Маркетплейс\n\nВыберите действие:' : '🛒 Marketplace\n\nChoose an action:',
    Markup.inlineKeyboard([
      [Markup.button.callback(isRu ? '🔍 Каталог' : '🔍 Browse', 'mp_browse')],
      [Markup.button.callback(isRu ? '📦 Мои покупки' : '📦 My Items', 'mp_my_items')],
...
const sellInputStep = async (ctx: MyContext) => {
  ...
  if (!wd.sellDescription) {
    ...
    ctx.session.wizardData = wd
    await ctx.reply(isRu ? '💰 Цена в звездах (число):' : '💰 Price in stars (number):')
    return
  }
  if (!wd.sellPrice) {
    const price = parseInt(text, 10)
    if (isNaN(price) || price < 1) {
      await ctx.reply(isRu ? 'Введите корректное число (минимум 1).' : 'Enter a valid number (min 1).')
      return
    }
    wd.sellPrice = price
    ctx.session.wizardData = wd
  ...
  if (result) {
    ...
    await ctx.reply(
      isRu ? '❌ Не удалось создать товар. Попробуйте позже.' : '❌ Failed to create item. Try later.',
    )
  }
...
marketplaceWizard.action('mp_browse', async (ctx) => {
  ...
  buttons.push([Markup.button.callback(isRu ? '◀️ Назад' : '◀️ Back', 'mp_back_main')])
...
ITEM_TYPES.forEach((itemType) => {
  marketplaceWizard.action(`mp_cat_${itemType.key}`, async (ctx) => {
    ...
    if (items.length === 0) {
      await ctx.editMessageText(
        isRu ? '😔 Пока нет товаров в этой категории.' : '😔 No items in this category yet.',
        Markup.inlineKeyboard([
    ...
    const lines = items.map((it, i) => `${i + 1}. ${it.title} — ${it.price_stars} ⭐`)
    const buttons = items.map((it) => [
      Markup.button.callback(`🛒 ${it.title}`, `mp_buy_${it.id}`),
    ])
    buttons.push([Markup.button.callback(isRu ? '◀️ Назад' : '◀️ Back', 'mp_browse')])
...
marketplaceWizard.action(/^mp_buy_(.+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    Markup.inlineKeyboard([
      [Markup.button.callback(isRu ? '✅ Купить' : '✅ Buy', `mp_confirm_${itemId}`)],
      [Markup.button.callback(isRu ? '◀️ Назад' : '◀️ Back', 'mp_browse')],
    ]),
  )
})
...
marketplaceWizard.action(/^mp_confirm_(.+)$/, async (ctx) => {
  ...
  if (result.success) {
    await ctx.editMessageText(
      isRu ? '✅ Покупка успешна! Вот ваш контент:' : '✅ Purchase successful! Here is your content:',
    )
    ...
    const errorMessages: Record<string, { ru: string; en: string }> = {
      ...
      cannot_buy_own: { ru: '❌ Нельзя купить свой товар.', en: '❌ Cannot buy your own item.' },
      insufficient_balance: { ru: '❌ Недостаточно звезд.', en: '❌ Insufficient stars.' },
    }
    const msg = errorMessages[result.error || ''] || { ru: '❌ Ошибка.', en: '❌ Error.' }
