...
export const SeeDream4InputSchema = z
  .object({
    prompt: z
      .string()
    ...
    image_input: z
      ...
      .max(10, '🚨 Cannot process more than 10 images')
      .optional(),
    aspect_ratio: z
      .string()
      .regex(/^\d+:\d+$/, '🚨 Aspect ratio must be in format "width:height"')
      .optional(),
...
export function getSeeDream4Dimensions(size: SeeDream4Size): {
  width: number
  height: number
} {
  switch (size) {
...
export function validateSeeDream4Input(input: unknown) {
  try {
    ...
    console.error('🚨 [SeeDream4] Input validation FAILED:', {
      ...
      hasTelegramId:
        typeof input === 'object' && input !== null && 'telegram_id' in input,
    })
