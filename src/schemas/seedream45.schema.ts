...
export const SeeDream45InputSchema = z
  .object({
    prompt: z
      .string()
    ...
    width: z
      .number()
      .int()
      .min(1024, '🚨 Width must be at least 1024px')
      .max(4096, '🚨 Width cannot exceed 4096px')
      .optional(),
    height: z
      .number()
      .int()
      .min(1024, '🚨 Height must be at least 1024px')
    ...
    sequential_image_generation:
      SequentialImageGenerationSchema.default('disabled'),
    ...
    max_images: z
      ...
      .max(15, '🚨 Cannot generate more than 15 images')
      .default(1),
    ...
    image_input: z
      ...
      .max(14, '🚨 Cannot process more than 14 images')
      .optional(),
    ...
    telegram_id: z
      ...
      .refine(
        id => /^\d+$/.test(id),
        '🚨 Invalid Telegram ID format - must be numeric'
      )
      .optional(),
...
export type SequentialImageGeneration = z.infer<
  typeof SequentialImageGenerationSchema
>

// Utility function to get dimensions by size (SeeDream 4.5)
export function getSeeDream45Dimensions(size: SeeDream45Size): {
  width: number
  height: number
} {
  switch (size) {
...
export function validateSeeDream45Input(input: unknown) {
  try {
    ...
    if (isZodError && typeof input === 'object' && input !== null) {
      ...
      if (inputObj.size === '1K') {
        suggestions.push(
          'SeeDream 4.5 does NOT support 1K resolution - use 2K or 4K'
        )
      }
    ...
    return {
      success: false,
      error,
      suggestions: isZodError ? 'Check validation requirements' : undefined,
    }
  }
}
