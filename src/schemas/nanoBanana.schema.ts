...
export const NanoBananaInputSchema = z.object({
  ...
  image_input: z.array(z.string().url()).min(1, 'At least one input image is required').max(10, 'Maximum 10 images allowed'),
...
export type ReplicateNanoBananaResponse = z.infer<typeof ReplicateNanoBananaResponseSchema>
...
export function extractImageUrlFromReplicateResponse(output: unknown): string | null {
  try {
