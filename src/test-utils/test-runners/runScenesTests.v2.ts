...
async function runTestModule(
    ...
    for (const test of testModule) {
      if (typeof test === 'function') {
        try {
          ...
          logger.error(
            `Ошибка при выполнении теста из массива ${testName}:`,
            error
          )
          results.push({
            success: false,
  ...
  const testFunctions = Object.entries(testModule).filter(
    ([key, value]) => typeof value === 'function' && key.startsWith('test')
  )
  ...
  if (testFunctions.length > 0) {
    for (const [name, func] of testFunctions) {
      try {
        ...
        logger.error(
          `Ошибка при выполнении функции ${name} из модуля ${testName}:`,
          error
        )
        results.push({
          success: false,
...
export async function runScenesTests(): Promise<TestResult[]> {
  ...
  for (const testFile of TEST_FILES) {
    ...
    try {
      ...
      logger.error(
        `Ошибка при загрузке или выполнении тестового файла ${testFile}:`,
        error
      )
      results.push({
        success: false,
  ...
  console.log(
    `📊 Процент успешных: ${Math.round((successTests / totalTests) * 100)}%`
  )
