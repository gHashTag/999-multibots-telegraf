...
export async function runScenesTests(): Promise<TestResult[]> {
  ...
  await runTestsGroup(
    'Тесты сцены создания пользователя',
    [
  ...
  try {
    ...
    console.log(
      `✅ Text to Video Wizard tests completed: ${textToVideoResults.filter(r => r.success).length} passed, ${textToVideoResults.filter(r => !r.success).length} failed`
    )
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `✅ Image to Video Wizard tests completed: ${imageToVideoResults.filter(r => r.success).length} passed, ${imageToVideoResults.filter(r => !r.success).length} failed`
    )
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `✅ Image to Prompt Wizard tests completed: ${imageToPromptResults.filter(r => r.success).length} passed, ${imageToPromptResults.filter(r => !r.success).length} failed`
    )
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `✅ NeuroPhoto wizard V2 tests completed: ${neuroPhotoWizardV2Results.filter(r => r.success).length} passed, ${neuroPhotoWizardV2Results.filter(r => !r.success).length} failed`
    )
  } catch (error) {
    console.error('❌ Error running neuroPhotoWizardV2 tests:', error)
  }
  ...
  try {
    ...
    console.log(
      `✅ Lip Sync Wizard tests completed: ${lipSyncWizardResults.filter(r => r.success).length} passed, ${lipSyncWizardResults.filter(r => !r.success).length} failed`
    )
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `✅ Help Scene tests completed: ${helpSceneResults.filter(r => r.success).length} passed, ${helpSceneResults.filter(r => !r.success).length} failed`
    )
  } catch (error) {
  ...
  console.log('')
  console.log(
    `📊 Результаты тестирования: Всего ${results.length}, Успех: ${successCount}, Ошибки: ${failCount}`
  )
  logger.info(
    `📊 Результаты тестирования: Всего ${results.length}, Успех: ${successCount}, Ошибки: ${failCount}`
  )
 ...
 */
export async function runTestsGroup(
  tests: Test[],
