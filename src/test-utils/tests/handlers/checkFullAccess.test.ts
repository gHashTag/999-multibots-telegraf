...
const fullAccessTests = fullAccessSubscriptions.map(sub => (
...
const noAccessTests = noAccessSubscriptions.map(sub => (
    async (): Promise<TestResult> => {
...
export async function runCheckFullAccessTests(options: { verbose?: boolean } = {}): Promise<TestResult[]> {
