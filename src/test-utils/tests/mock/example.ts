...
export function example1() {
  ...
  logger.info(
    `Был ли вызван с 'arg1': ${simpleMock.calledWith('arg1', 'arg2')}`
  )
...
export function example2() {
  ...
  logger.info(
    `Аргументы первого вызова: ${JSON.stringify(mockWithImpl.calls[0].args)}`
  )
...
export function example5() {
  ...
  logger.info(
    `fetchUser был вызван с id: ${apiMocks.fetchUser.calls[0].args[0]}`
  )
