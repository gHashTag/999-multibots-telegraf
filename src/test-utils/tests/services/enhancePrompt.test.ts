  ...
  try {
    ...
    const mockOpenAI = mockFunction<MockOpenAI>({
      createChatCompletion: async () => ({
        choices: [{
          message: { 
            content: 'Создай реалистичное изображение пушистого кота с выразительными глазами'
          }
        }]
      })
    });

    const result = await OpenAIService.enhancePrompt(TEST_PROMPT_RU, true, mockOpenAI);
  ...
  try {
    ...
    const mockOpenAI = mockFunction<MockOpenAI>({
      createChatCompletion: async () => ({
        choices: [{
          message: { 
            content: 'Create a photorealistic image of a fluffy cat with expressive eyes'
          }
        }]
      })
    });

    const result = await OpenAIService.enhancePrompt(TEST_PROMPT_EN, false, mockOpenAI);
