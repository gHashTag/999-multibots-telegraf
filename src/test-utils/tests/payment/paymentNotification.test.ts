...
const setupTest = async () => {
  ...
  Object.defineProperty(notificationHelper, 'notifyUserAboutSuccess', { value: mockNotifyUserAboutSuccess, configurable: true })
  ...
  Object.defineProperty(userDb, 'getUserByTelegramIdString', { value: mockGetUserByTelegramIdString, configurable: true })
  // Mock invalidateBalanceCache if necessary (assuming it's in userDb too)
  const mockInvalidateBalanceCache = mockApi.create()
  Object.defineProperty(userDb, 'invalidateBalanceCache', { value: mockInvalidateBalanceCache, configurable: true })
  ...
  Object.defineProperty(userDb, 'createSuccessfulPayment', { value: mockCreateSuccessfulPayment, configurable: true })

  allMocks.push(mockGetUserByTelegramIdString, mockInvalidateBalanceCache, mockCreateSuccessfulPayment)
}
...
export async function testPaymentNotification(): Promise<TestResult> {
  ...
  const testUserData = { id: '1', telegram_id: testUserId, balance: 500, is_ru: true, bot_name: 'test_bot', level: 1, created_at: '', updated_at: '' } // Example user data
  const mockPaymentData = { id: 10, telegram_id: testUserId, amount: 100, stars: 100, type: TransactionType.MONEY_EXPENSE, description: 'Test', bot_name: 'test_bot', operation_id: 'op1' }
  ...
  mockGetUserByTelegramIdString.mockResolvedValueOnce(testUserData) // First call in get-balance step
  ...
  const createPaymentMock = userDb.createSuccessfulPayment as MockedFunction<any> // Get the mocked function
  ...
  try {
    const result = await engine.send({ name: 'payment/process', data: eventData })

    // Verify notification was called
    assert.isTrue(mockNotifyUserAboutSuccess.mock.calls.length > 0, `${testName} - notifyUserAboutSuccess should be called`)
    // Add more specific checks on the arguments if needed
    const notificationArgs = mockNotifyUserAboutSuccess.mock.calls[0][0]
    assert.equal(notificationArgs.telegram_id, Number(testUserId), `${testName} - Check notification telegram_id`)
    assert.equal(notificationArgs.amount, 100, `${testName} - Check notification amount`)
    assert.equal(notificationArgs.newBalance, 400, `${testName} - Check notification newBalance`)

    return { name: testName, success: true, message: 'Notification test passed' }
...
export async function testBalanceTopUp(): Promise<TestResult> {
  ...
  const testUserData = { id: '2', telegram_id: testUserId, balance: initialBalance, is_ru: false, bot_name: 'test_bot', level: 1, created_at: '', updated_at: '' };
  const mockPaymentData = { id: 11, telegram_id: testUserId, amount: topUpAmount, stars: topUpAmount, type: TransactionType.MONEY_INCOME, description: 'Top Up', bot_name: 'test_bot', operation_id: 'op2' };

  // Mock database calls
  mockGetUserByTelegramIdString.mockResolvedValueOnce(testUserData) // Initial balance check
                                .mockResolvedValueOnce({ ...testUserData, balance: finalBalance }); // Balance check after update
  const createPaymentMock = userDb.createSuccessfulPayment as MockedFunction<any>; 
  createPaymentMock.mockResolvedValue(mockPaymentData);
  ...
  try {
    ...
    assert.isTrue(createPaymentMock.mock.calls.length > 0, `${testName} - createSuccessfulPayment called`);
    const createArgs = createPaymentMock.mock.calls[0][0];
    assert.equal(createArgs.type, TransactionType.MONEY_INCOME, `${testName} - Payment type should be INCOME`);
    assert.equal(createArgs.amount, topUpAmount, `${testName} - Payment amount check`);

    // Verify notification was called with correct details
    assert.isTrue(mockNotifyUserAboutSuccess.mock.calls.length > 0, `${testName} - notifyUserAboutSuccess called`);
    const notificationArgs = mockNotifyUserAboutSuccess.mock.calls[0][0];
    assert.equal(notificationArgs.telegram_id, Number(testUserId), `${testName} - Check notification telegram_id`);
    assert.equal(notificationArgs.amount, topUpAmount, `${testName} - Check notification amount`);
    assert.equal(notificationArgs.type, TransactionType.MONEY_INCOME, `${testName} - Check notification type`);
    assert.equal(notificationArgs.newBalance, finalBalance, `${testName} - Check notification newBalance`);
    assert.equal(notificationArgs.currentBalance, initialBalance, `${testName} - Check notification currentBalance`);

    return { name: testName, success: true, message: 'Balance top up test passed' };

  } catch (error: any) {
    const errorMessage = error?.message || String(error);
    const errorValue = error instanceof Error ? error : new Error(String(error));
    return { name: testName, success: false, message: `Test failed: ${errorMessage}`, error: errorValue } as any; // Temporary workaround
  }
}
...
export async function testBalanceDebit(): Promise<TestResult> {
  ...
  const testUserData = { id: '3', telegram_id: testUserId, balance: initialBalance, is_ru: true, bot_name: 'test_bot', level: 1, created_at: '', updated_at: '' };
  const mockPaymentData = { id: 12, telegram_id: testUserId, amount: debitAmount, stars: debitAmount, type: TransactionType.MONEY_EXPENSE, description: 'Debit', bot_name: 'test_bot', operation_id: 'op3' };
  ...
  try {
    ...
    assert.isTrue(createPaymentMock.mock.calls.length > 0, `${testName} - createSuccessfulPayment called`);
    const createArgs = createPaymentMock.mock.calls[0][0];
    assert.equal(createArgs.type, TransactionType.MONEY_EXPENSE, `${testName} - Payment type should be EXPENSE`);
    assert.equal(createArgs.amount, debitAmount, `${testName} - Payment amount check`);

    // Verify notification was called with correct details
    assert.isTrue(mockNotifyUserAboutSuccess.mock.calls.length > 0, `${testName} - notifyUserAboutSuccess called`);
    const notificationArgs = mockNotifyUserAboutSuccess.mock.calls[0][0];
    assert.equal(notificationArgs.telegram_id, Number(testUserId), `${testName} - Check notification telegram_id`);
    assert.equal(notificationArgs.amount, debitAmount, `${testName} - Check notification amount`);
    assert.equal(notificationArgs.type, TransactionType.MONEY_EXPENSE, `${testName} - Check notification type`);
    assert.equal(notificationArgs.newBalance, finalBalance, `${testName} - Check notification newBalance`);
    assert.equal(notificationArgs.currentBalance, initialBalance, `${testName} - Check notification currentBalance`);

    return { name: testName, success: true, message: 'Balance debit test passed' };
...
export async function testInsufficientBalance(): Promise<TestResult> {
  ...
  const testUserData = { id: '4', telegram_id: testUserId, balance: initialBalance, is_ru: false, bot_name: 'test_bot', level: 1, created_at: '', updated_at: '' };
  ...
  try {
    ...
    return { name: testName, success: false, message: 'Test failed: Expected an error for insufficient balance, but none was thrown.' };
    ...
    const errorMessage = error?.message || String(error);
    assert.contains(errorMessage, 'Недостаточно средств', `${testName} - Error message should indicate insufficient funds`);
    assert.contains(errorMessage, `Баланс: ${initialBalance}`, `${testName} - Error message should contain current balance`);
    assert.contains(errorMessage, `требуется: ${debitAmount}`, `${testName} - Error message should contain required amount`);

    // Verify that payment and notification were NOT triggered
    assert.equal(createPaymentMock.mock.calls.length, 0, `${testName} - createSuccessfulPayment should NOT be called`);
    assert.equal(mockNotifyUserAboutSuccess.mock.calls.length, 0, `${testName} - notifyUserAboutSuccess should NOT be called`);

    return { name: testName, success: true, message: 'Insufficient balance error correctly handled' };
  }
}
...
export async function runPaymentNotificationTests(): Promise<TestResult[]> {
  ...
  logger.info(`🏁 Тесты уведомлений о платежах завершены. Выполнено: ${results.length}`)
