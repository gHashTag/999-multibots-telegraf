...
export async function runRobokassaFormTests(): Promise<any> {
  ...
  function testInvalidInvId() {
    ...
    try {
      ...
      logger.info(
        '✅ Тест пройден: некорректные InvId обрабатываются правильно'
      )
      ...
      logger.error(
        '❌ Ошибка при выполнении теста обработки некорректных InvId',
        {
          error: error instanceof Error ? error.message : String(error),
        }
      )
  ...
  function testSignatureGeneration() {
    ...
    try {
      ...
      assert.strictEqual(
        urlMerchantLogin,
        merchantLogin,
        'MerchantLogin должен совпадать'
      )
      assert.strictEqual(
        urlOutSum,
        amount.toString(),
        'OutSum должен совпадать'
      )
      ...
      assert.strictEqual(
        urlSignatureValue,
        expectedSignature,
        'Подпись должна быть корректной'
      )
  ...
  function testTestModeFlag() {
    ...
    try {
      ...
      const urlWithTestMode = generateValidRobokassaUrl(
        amount,
        invId,
        description,
        true
      ...
      assert.strictEqual(
        parsedUrlWithTest.searchParams.get('IsTest'),
        '1',
        'IsTest должен быть равен 1'
      )

      // URL без тестового режима
      const urlWithoutTestMode = generateValidRobokassaUrl(
        amount,
        invId,
        description,
        false
      ...
      assert.isFalse(
        parsedUrlWithoutTest.searchParams.has('IsTest'),
        'IsTest не должен присутствовать'
      )
