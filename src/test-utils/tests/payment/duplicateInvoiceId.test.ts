...
class DuplicateInvoiceIdTester {
  ...
  async testUniqueInvoiceId(): Promise<TestResult> {
    try {
      ...
      assert.isTrue(nowExists, 'Платеж должен существовать в базе после создания')
  ...
  async testDuplicateInvoiceId(): Promise<TestResult> {
    try {
      ...
      try {
        ...
        const isDuplicateKeyError = errorMessage.includes('duplicate key') || 
        ...
        return {
          success: true,
          message: 'Система корректно предотвратила создание дублирующегося платежа'
        }
      }
  ...
  async testPaymentStatusCheck(): Promise<TestResult> {
    try {
      ...
      assert.strictEqual(payment?.status, 'PENDING', 'Статус платежа должен быть PENDING')
      ...
      assert.strictEqual(updatedPayment?.status, 'COMPLETED', 'Статус платежа должен быть COMPLETED')
  ...
  async testPaymentProcessing(): Promise<TestResult> {
    try {
      ...
      assert.strictEqual(count, 1, 'Должна быть только одна запись с данным ID инвойса')
  ...
  async testFallbackInvoiceIdGeneration(): Promise<TestResult> {
    try {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const exists = await this.invoiceExists(invId);
        assert.isTrue(exists, `Платеж ${i+1} с ID ${invId} должен существовать в базе`);
      }
      ...
      logger.error('❌ Ошибка при тестировании запасного алгоритма генерации ID', {
