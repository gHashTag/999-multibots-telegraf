...
import { createDbTestUser, trackTestUserId } from '@/test-utils/helpers/dbTestHelper';
    ...
    try {
      await createDbTestUser({
        ...
        if (error.message.includes('already exists')) {
            logger.warn(`[DuplicateInvoiceIdTest] Test user ${TEST_TELEGRAM_ID} already exists. Proceeding.`);
            // Track the ID even if creation failed due to existence
            trackTestUserId(TEST_TELEGRAM_ID); 
        } else {
            logger.error('[DuplicateInvoiceIdTest] Failed to ensure test user exists:', error);
...
async function invoiceExists(invId: string | number): Promise<boolean> {
  try {
    ...
    if (error) {
        logger.error('[DuplicateInvoiceIdTest] Error checking invoice existence:', { error, invId });
        return false; // Assume not exists on error?
    }
    return !!data;
  } catch (error) {
    logger.error('[DuplicateInvoiceIdTest] Unexpected error checking invoice:', { error, invId });
    return false;
  }
}
...
export async function testUniqueInvoiceId(): Promise<TestResult> {
  ...
  try {
    ...
    assert.isFalse(exists, `${testName} - Invoice ID ${invId} should not exist yet`);
    if (exists) {
      return { success: false, name: testName, message: `Invoice ID ${invId} already exists unexpectedly` };
    }
    ...
    assert.isTrue(result.success, `${testName} - Payment should be created successfully`);

    // Проверяем, что платеж появился в базе
    const nowExists = await invoiceExists(invId);
    assert.isTrue(nowExists, `${testName} - Payment ${invId} should exist in DB after creation`);

    return { success: true, name: testName, message: `Payment with ID ${invId} successfully created` };
  } catch (error: any) {
...
export async function testDuplicateInvoiceId(): Promise<TestResult> {
  ...
  try {
    ...
    assert.isFalse(exists, `${testName} - Invoice ID ${invId} should not exist yet (before first create)`);
     if (exists) {
      return { success: false, name: testName, message: `Invoice ID ${invId} already exists unexpectedly (before first create)` };
    }
    ...
    assert.isTrue(result1.success, `${testName} - First payment should be created successfully`);

    // Убедимся, что он появился
    exists = await invoiceExists(invId);
    assert.isTrue(exists, `${testName} - Invoice ID ${invId} should exist after first create`);
     if (!exists) {
      return { success: false, name: testName, message: `Invoice ID ${invId} did not exist after first create` };
    }
    ...
    try {
      ...
      return { success: false, name: testName, message: 'System allowed creating a duplicate payment ID' };
    } catch (error) {
      // Ожидаем ошибку уникальности ключа от базы данных
      thrownError = error;
    }

    assert.ok(thrownError, `${testName} - Should have thrown an error on duplicate insert`);
    ...
    assert.isTrue(isDuplicateKeyError, `${testName} - Error message should indicate unique constraint violation. Got: ${errorMessage}`);

    return { success: true, name: testName, message: 'System correctly prevented duplicate payment ID' };
...
export async function testPaymentStatusCheck(): Promise<TestResult> {
  ...
  try {
      ...
      assert.isTrue(createResult.success, `${testName} - Failed to create initial pending payment`);
      ...
      assert.isNull(selectError, `${testName} - Error selecting payment status: ${selectError?.message}`);
      assert.isNotNull(payment, `${testName} - Payment should be found`);
      assert.strictEqual(payment?.status, 'PENDING', `${testName} - Initial payment status should be PENDING`);
      ...
      const { error: updateError } = await supabase
        ...
        .eq('inv_id', String(invId));
      assert.isNull(updateError, `${testName} - Error updating payment status: ${updateError?.message}`);
      ...
      assert.isNull(selectUpdatedError, `${testName} - Error selecting updated status: ${selectUpdatedError?.message}`);
      assert.isNotNull(updatedPayment, `${testName} - Updated payment should be found`);
      assert.strictEqual(updatedPayment?.status, 'COMPLETED', `${testName} - Payment status should be COMPLETED`);

      return { success: true, name: testName, message: 'Payment status check and update work correctly' };
  } catch (error: any) {
      logger.error(`[DuplicateInvoiceIdTest] Error in ${testName}:`, { error });
      return { success: false, name: testName, message: error.message, error };
  }
...
export async function testPaymentProcessingDuplicateCheck(): Promise<TestResult> {
  ...
  try {
    ...
    assert.isNull(error, `${testName} - Error checking payment after processing: ${error?.message}`);
    assert.equal(count, 1, `${testName} - Should be only one payment record with inv_id ${invId}. Found: ${count}`);
    assert.isTrue(payments && payments.length === 1, `${testName} - Payment record should exist`);
    assert.equal(payments?.[0]?.status, 'COMPLETED', `${testName} - Payment status should be COMPLETED after processing`);

    return { success: true, name: testName, message: 'Inngest processing handled duplicate check correctly' };
  } catch (error: any) {
    logger.error(`[DuplicateInvoiceIdTest] Error in ${testName}:`, { error });
    return { success: false, name: testName, message: error.message, error };
  }
...
export async function runDuplicateInvoiceIdTests(options: { verbose?: boolean } = {}): Promise<TestResult[]> {
  ...
  for (const test of tests) {
    try {
        ...
        logger.error(`🔥 Критическая ошибка при выполнении теста ${test.name}:`, error);
        results.push({
