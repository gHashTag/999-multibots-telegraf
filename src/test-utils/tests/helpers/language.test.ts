...
const createMockCtx = (languageCode?: string, fromValue?: { id: number, is_bot: boolean, first_name: string, language_code?: string } | undefined): Context => {
    return {
        from: fromValue,
...
export async function testIsRussian_Ru(): Promise<TestResult> {
    ...
    try {
        const ctx = createMockCtx('ru', { id: 123, is_bot: false, first_name: 'Test', language_code: 'ru' });
...
export async function testIsRussian_En(): Promise<TestResult> {
    ...
    try {
        const ctx = createMockCtx('en', { id: 123, is_bot: false, first_name: 'Test', language_code: 'en' });
        const result = isRussian(ctx);
...
export async function testIsRussian_UndefinedLang(): Promise<TestResult> {
    ...
    try {
        const ctx = createMockCtx(undefined, { id: 123, is_bot: false, first_name: 'Test', language_code: undefined });
        const result = isRussian(ctx);
        assert.isFalse(result, `${testName} should return false`);
        return { name: testName, success: true, message: 'Passed' };
    } catch (error: any) {
...
export async function runLanguageHelperTests(options: { verbose?: boolean } = {}): Promise<TestResult[]> {
  const tests = [
    testIsRussian_Ru,
    testIsRussian_En,
