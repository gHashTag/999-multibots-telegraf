...
export async function validatePorts(): Promise<boolean> {
  ...
  for (const config of PORT_CONFIGS) {
    ...
    if (isUsed) {
      ...
      logger.error(`❌ Port ${config.port} (${config.serviceName}) is already in use by process: ${process}`);
      allPortsValid = false;
    } else {
  ...
  if (!allPortsValid) {
    logger.warn('⚠️ Some ports are already in use. This might cause issues with the application.');
    logger.info('💡 Try running: lsof -i :<port> | grep LISTEN to identify the process');
