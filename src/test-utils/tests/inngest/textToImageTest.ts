...
export async function testTextToImage(
  ...
  try {
    ...
    assert.assert(generationResponse.success, "Failed to create image generation task");
    ...
    const generationResult = {
      success: true,
      images: Array(data.num_images).fill(null).map((_, i) => `https://example.com/image-${i + 1}.jpg`),
      processedPrompt: data.prompt
    };
    
    assert.assert(generationResult.success, "Image generation failed");
    assert.assert(generationResult.images.length === data.num_images, 
      `Expected ${data.num_images} images, but got ${generationResult.images.length}`);
    
    if (verbose) {
      logger.info(`[${testName}] Generated ${generationResult.images.length} images`);
    ...
    if (verbose) {
      logger.info(`[${testName}] Successfully stored ${storageResult.storedImages} images`);
    }
 ...
 */
export async function testMidjourney(options: { verbose?: boolean } = {}): Promise<TextToImageTestResult> {
 ...
 */
export async function testDalle(options: { verbose?: boolean } = {}): Promise<TextToImageTestResult> {
  const data: TextToImageData = {
 ...
 */
export async function testInvalidPrompt(options: { verbose?: boolean } = {}): Promise<TextToImageTestResult> {
  const data: TextToImageData = {
    prompt: "", // Empty prompt
    model: "midjourney",
    num_images: 1,
 ...
 */
export async function runTextToImageTests(options: { verbose?: boolean } = {}): Promise<TextToImageTestResult[]> {
  ...
  try {
    ...
    logger.info(`Text-to-Image tests completed: ${passedTests}/${results.length} passed`);
  } catch (error: any) {
