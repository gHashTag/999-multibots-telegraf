 ...
 */
export async function testDefaultMotion(
 ...
 */
export async function testZoomMotion(
  options: { verbose?: boolean } = {}
): Promise<ImageToVideoTestResult> {
  const data: ImageToVideoData = {
 ...
 */
export async function testPanMotion(
  options: { verbose?: boolean } = {}
): Promise<ImageToVideoTestResult> {
  const data: ImageToVideoData = {
 ...
 */
export async function runImageToVideoTests(
  options: { verbose?: boolean } = {}
): Promise<ImageToVideoTestResult[]> {
  logger.info('🎭 Запуск тестов Image-to-Video...', {
    description: 'Starting Image-to-Video tests...',
  })
  ...
  try {
    ...
    logger.info(
      `📊 Тесты Image-to-Video завершены: ${passedTests}/${results.length} успешно`,
      {
    ...
    logger.error(
      `❌ Ошибка при запуске тестов Image-to-Video: ${error.message}`,
      {
