...
export async function runBalanceNotifierTests(options: { verbose?: boolean } = {}): Promise<TestResult[]> {
  logger.info('🚀 Запуск тестов уведомлений о балансе...', { description: 'Starting Balance Notification Tests...' })
  ...
  try {
    ...
    logger.info(`📊 Тесты уведомлений о балансе завершены: ${passedTests}/${results.length} успешно`, {
    ...
    logger.error(`❌ Критическая ошибка при запуске тестов уведомлений о балансе: ${error.message}`, {
...
if (require.main === module) {
  runBalanceNotifierTests({ verbose: true }).then(results => {
    logger.info({
