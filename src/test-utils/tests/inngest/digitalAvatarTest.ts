 ...
 */
export async function testDigitalAvatarBody(
  data?: Partial<DigitalAvatarTrainingData>
): Promise<TestResult> {
  ...
  const mockReplicate =
    mock<(options: any) => Promise<{ id: string; status: string }>>()
  ...
  const mockSupabase =
    mock<
      (data: any) => Promise<{ id: string; replicate_training_id: string }>
    >()
  ...
  const mockSendEvent =
    mock<(name: string, data: any) => Promise<DigitalAvatarTestResult>>()
 ...
 */
export async function testDigitalAvatarBodyV2(
  data?: Partial<DigitalAvatarTrainingData>
): Promise<TestResult> {
  const defaultData: DigitalAvatarTrainingData = {
    bot_name: 'test_bot',
  ...
  logger.info({
    message:
      '🧪 Тест функции создания цифрового аватара V2 (Digital Avatar Body V2)',
