...
export async function testVoiceToText(
  ...
  try {
    ...
    assert.assert(transcriptionResponse.success, "Failed to create transcription task");
    
    if (verbose) {
      logger.info(`[${testName}] Task created with ID: ${transcriptionResponse.taskId}`);
      logger.info(`[${testName}] Estimated time: ${transcriptionResponse.estimatedTime}`);
    }
    ...
    if (data.is_ru || data.language === 'ru') {
      transcriptionText = "Привет, это тестовое аудио для проверки функции преобразования голоса в текст.";
    } else {
      transcriptionText = "Hello, this is a test audio for checking the voice-to-text transcription function.";
    }
      ...
      segments: [
        {
          ...
          text: data.is_ru ? "Привет, это тестовое аудио" : "Hello, this is a test audio",
          confidence: 0.92
        },
        {
          start: 2.1,
          end: 5.2,
          text: data.is_ru ? "для проверки функции преобразования голоса в текст." : "for checking the voice-to-text transcription function.",
          confidence: 0.87
        }
      ]
    };
    ...
    if (verbose) {
      logger.info(`[${testName}] Transcribed text: "${transcriptionResult.text}"`);
      logger.info(`[${testName}] Confidence: ${transcriptionResult.confidence}`);
      logger.info(`[${testName}] Detected language: ${transcriptionResult.language_detected}`);
      logger.info(`[${testName}] Audio duration: ${transcriptionResult.duration} seconds`);
    }
 ...
 */
export async function testEnglishAudio(options: { verbose?: boolean } = {}): Promise<VoiceToTextTestResult> {
 ...
 */
export async function testRussianAudio(options: { verbose?: boolean } = {}): Promise<VoiceToTextTestResult> {
  const data: VoiceToTextData = {
 ...
 */
export async function testAutoDetectLanguage(options: { verbose?: boolean } = {}): Promise<VoiceToTextTestResult> {
  const data: VoiceToTextData = {
 ...
 */
export async function testInvalidAudio(options: { verbose?: boolean } = {}): Promise<VoiceToTextTestResult> {
  const data: VoiceToTextData = {
    audio_url: "", // Empty URL
    telegram_id: 123456789,
    username: "testuser",
 ...
 */
export async function runVoiceToTextTests(options: { verbose?: boolean } = {}): Promise<VoiceToTextTestResult[]> {
  logger.info("🎤 Запуск тестов Voice-to-Text...", { description: "Starting Voice-to-Text tests..." });
  ...
  try {
    ...
    logger.info(`📊 Тесты Voice-to-Text завершены: ${passedTests}/${results.length} успешно`, {
      description: `Voice-to-Text tests completed: ${passedTests}/${results.length} passed`
    });
  } catch (error: any) {
    logger.error(`❌ Ошибка при запуске тестов Voice-to-Text: ${error.message}`, {
