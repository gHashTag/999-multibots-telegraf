...
export async function testCreateVoiceAvatar(
  ...
  logger.info(`[${testName}] Starting test with voice sample: "${data.voice_sample_url}"`);
  ...
  try {
    ...
    assert.assert(validationResult.duration >= 30, "Voice sample is too short (minimum 30 seconds)");
    
    if (verbose) {
      logger.info(`[${testName}] Sample duration: ${validationResult.duration} seconds`);
      logger.info(`[${testName}] Sample quality: ${validationResult.quality}`);
      logger.info(`[${testName}] Detected language: ${validationResult.language}`);
    }
    ...
    assert.assert(creationResponse.success, "Failed to start voice model creation");
    
    if (verbose) {
      logger.info(`[${testName}] Task created with ID: ${creationResponse.taskId}`);
      logger.info(`[${testName}] Estimated time: ${creationResponse.estimatedTime}`);
    }
    ...
    if (verbose) {
      ...
      logger.info(`[${testName}] Preview audio: ${creationResult.previewAudioUrl}`);
      logger.info(`[${testName}] Similarity score: ${creationResult.similarityScore}`);
    ...
    if (verbose) {
      logger.info(`[${testName}] Successfully saved voice model for user ${saveResult.userId}`);
    }
 ...
 */
export async function testEnglishVoice(options: { verbose?: boolean } = {}): Promise<CreateVoiceAvatarTestResult> {
 ...
 */
export async function testRussianVoice(options: { verbose?: boolean } = {}): Promise<CreateVoiceAvatarTestResult> {
  const data: CreateVoiceAvatarData = {
 ...
 */
export async function testShortSample(options: { verbose?: boolean } = {}): Promise<CreateVoiceAvatarTestResult> {
  const data: CreateVoiceAvatarData = {
 ...
 */
export async function testInvalidSample(options: { verbose?: boolean } = {}): Promise<CreateVoiceAvatarTestResult> {
  const data: CreateVoiceAvatarData = {
  ...
  try {
    ...
    return {
      ...
      passed: true,
      name: testName,
      message: "Successfully validated that empty voice sample URLs are rejected",
      category: TestCategory.Inngest,
      details: {
        error: error.message
      }
 ...
 */
export async function runVoiceAvatarTests(options: { verbose?: boolean } = {}): Promise<CreateVoiceAvatarTestResult[]> {
  logger.info("🎙️ Запуск тестов Create Voice Avatar...", { description: "Starting Create Voice Avatar tests..." });
  ...
  try {
    ...
    logger.info(`📊 Тесты Create Voice Avatar завершены: ${passedTests}/${results.length} успешно`, {
      description: `Create Voice Avatar tests completed: ${passedTests}/${results.length} passed`
    });
  } catch (error: any) {
    logger.error(`❌ Ошибка при запуске тестов Create Voice Avatar: ${error.message}`, {
