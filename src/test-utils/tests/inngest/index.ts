...
export async function runInngestTests(
  ...
  logger.info('🚀 Запуск тестов Inngest Functions...', {
  ...
  try {
    ...
    logger.info(
      `📊 Тесты Inngest завершены: ${passedTests}/${results.length} успешно`,
      {
    ...
    logger.error(
      `❌ Критическая ошибка при запуске тестов Inngest: ${error.message}`,
      {
