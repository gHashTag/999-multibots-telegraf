...
export async function testTextToSpeech(
  ...
  logger.info(`[${testName}] Starting test with text: "${data.text.substring(0, 30)}${data.text.length > 30 ? '...' : ''}"`);
  ...
  try {
    ...
    assert.assert(generationResponse.success, "Failed to create speech generation task");
    ...
    if (verbose) {
      ...
      logger.info(`[${testName}] Duration: ${generationResult.duration.toFixed(2)} seconds`);
 ...
 */
export async function testDefaultVoice(options: { verbose?: boolean } = {}): Promise<TextToSpeechTestResult> {
 ...
 */
export async function testCustomVoice(options: { verbose?: boolean } = {}): Promise<TextToSpeechTestResult> {
  const data: TextToSpeechData = {
 ...
 */
export async function testLongText(options: { verbose?: boolean } = {}): Promise<TextToSpeechTestResult> {
  // Create a longer text to test
  const longText = "This is a long paragraph of text that will be used to test the text-to-speech system with a substantial amount of content. "
 ...
 */
export async function testInvalidVoice(options: { verbose?: boolean } = {}): Promise<TextToSpeechTestResult> {
  const data: TextToSpeechData = {
 ...
 */
export async function runTextToSpeechTests(options: { verbose?: boolean } = {}): Promise<TextToSpeechTestResult[]> {
  logger.info("🔊 Запуск тестов Text-to-Speech...", { description: "Starting Text-to-Speech tests..." });
  ...
  try {
    ...
    logger.info(`📊 Тесты Text-to-Speech завершены: ${passedTests}/${results.length} успешно`, {
      description: `Text-to-Speech tests completed: ${passedTests}/${results.length} passed`
    });
  } catch (error: any) {
    logger.error(`❌ Ошибка при запуске тестов Text-to-Speech: ${error.message}`, {
