...
export async function testImageToPrompt(
  ...
  try {
    ...
    assert.assert(generationResponse.success, "Failed to create prompt generation task");
    ...
    if (verbose) {
      logger.info(`[${testName}] Generated prompt: "${generationResult.prompt}"`);
      logger.info(`[${testName}] Confidence score: ${generationResult.confidence}`);
    }
 ...
 */
export async function testSimpleMode(options: { verbose?: boolean } = {}): Promise<ImageToPromptTestResult> {
 ...
 */
export async function testDetailedMode(options: { verbose?: boolean } = {}): Promise<ImageToPromptTestResult> {
  const data: ImageToPromptData = {
 ...
 */
export async function testInvalidImage(options: { verbose?: boolean } = {}): Promise<ImageToPromptTestResult> {
  const data: ImageToPromptData = {
    image_url: "", // Empty URL
    telegram_id: 123456789,
    username: "testuser",
 ...
 */
export async function runImageToPromptTests(options: { verbose?: boolean } = {}): Promise<ImageToPromptTestResult[]> {
  logger.info("🖼️ Запуск тестов Image-to-Prompt...", { description: "Starting Image-to-Prompt tests..." });
  ...
  try {
    ...
    logger.info(`📊 Тесты Image-to-Prompt завершены: ${passedTests}/${results.length} успешно`, {
      description: `Image-to-Prompt tests completed: ${passedTests}/${results.length} passed`
    });
  } catch (error: any) {
    logger.error(`❌ Ошибка при запуске тестов Image-to-Prompt: ${error.message}`, {
