 ...
 */
export async function testGen2(options: { verbose?: boolean } = {}): Promise<TextToVideoTestResult> {
 ...
 */
export async function testRunway(options: { verbose?: boolean } = {}): Promise<TextToVideoTestResult> {
  const data: TextToVideoData = {
 ...
 */
export async function testInvalidPrompt(options: { verbose?: boolean } = {}): Promise<TextToVideoTestResult> {
  const data: TextToVideoData = {
    prompt: "", // Empty prompt
    duration: 5,
    model: "gen-2",
 ...
 */
export async function runTextToVideoTests(options: { verbose?: boolean } = {}): Promise<TextToVideoTestResult[]> {
  logger.info("🎬 Запуск тестов Text-to-Video...", { description: "Starting Text-to-Video tests..." });
  ...
  try {
    ...
    logger.info(`📊 Тесты Text-to-Video завершены: ${passedTests}/${results.length} успешно`, {
      description: `Text-to-Video tests completed: ${passedTests}/${results.length} passed`
    });
  } catch (error: any) {
    logger.error(`❌ Ошибка при запуске тестов Text-to-Video: ${error.message}`, {
