...
import { testRussianTranslations, testEnglishTranslations, testLocalizationKeysConsistency } from './translationTests'
...
export function runTranslationTests(): TestResult[] {
  logger.info('Запуск тестов переводов', { description: 'Starting translation tests' })

  try {
    // Запускаем тесты для русских переводов
    logger.info('Тестирование русских переводов', { description: 'Testing Russian translations' })
    const russianResult = testRussianTranslations()
    
    // Запускаем тесты для английских переводов
    logger.info('Тестирование английских переводов', { description: 'Testing English translations' })
    const englishResult = testEnglishTranslations()
    
    // Проверяем согласованность ключей локализации
    logger.info('Проверка согласованности ключей локализации', { description: 'Checking localization keys consistency' })
    const keysConsistencyResult = testLocalizationKeysConsistency()
    
    // Общая проверка перевода
    logger.info('Общая проверка перевода', { description: 'General translation check' })
