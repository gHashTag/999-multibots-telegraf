...
const createTestContext = (
  options: { language?: string; callbackData?: string } = {}
) => {
 ...
 */
const assertInlineKeyboardContains = (
  replyMock: any,
  ...
  expectedButtons.forEach(expectedButton => {
    if (!allButtons.some((button: string) => button.includes(expectedButton))) {
      throw new Error(
        `${errorMessage}: Кнопка "${expectedButton}" не найдена в клавиатуре`
      )
    }
  })
}
...
export async function testLanguageScene_EnterScene(): Promise<TestResult> {
  ...
  try {
    ...
    assertReplyContains(
      replyMock,
      'Language Selection',
      'Заголовок выбора языка не найден в сообщении'
    )

    // Проверяем наличие кнопок языка
    assertInlineKeyboardContains(
      replyMock,
      ['English', 'Русский'],
      'Кнопки выбора языка не найдены'
    )
...
export async function testLanguageScene_ChangeToRussian(): Promise<TestResult> {
  ...
  try {
    ...
    const languageActionHandler = actionHandlers.find(
      handler =>
        handler.value instanceof RegExp &&
        handler.value.toString() === '/^language:(.+)$/'
    )
    ...
    if (setUserLanguageCalls[0][1] !== 'ru') {
      throw new Error(
        `Неверный язык: ожидался 'ru', получен '${setUserLanguageCalls[0][1]}'`
      )
    }
    ...
    if (!answerCbQueryCalls[0][0].includes('Русский')) {
      throw new Error(
        'Сообщение о смене языка не содержит упоминания русского языка'
      )
    }
...
export async function testLanguageScene_ChangeToEnglish(): Promise<TestResult> {
  ...
  try {
    ...
    if (setUserLanguageCalls[0][1] !== 'en') {
      throw new Error(
        `Неверный язык: ожидался 'en', получен '${setUserLanguageCalls[0][1]}'`
      )
    }
    ...
    if (!answerCbQueryCalls[0][0].includes('English')) {
      throw new Error(
        'Сообщение о смене языка не содержит упоминания английского языка'
      )
    }
...
export async function testLanguageScene_UnsupportedLanguage(): Promise<TestResult> {
  ...
  try {
    ...
    if (mockSetUserLanguage.mock.calls.length > 0) {
      throw new Error(
        'Метод setUserLanguage был вызван для неподдерживаемого языка'
      )
    }
    ...
    if (!answerCbQueryCalls[0][0].includes('Unsupported')) {
      throw new Error(
        'Сообщение не содержит информации о неподдерживаемом языке'
      )
    }
    ...
    if (editMessageTextMock.mock.calls.length > 0) {
      throw new Error(
        'Метод editMessageText был вызван для неподдерживаемого языка'
      )
    }
...
export async function testLanguageScene_BackToMenu(): Promise<TestResult> {
  ...
  try {
    ...
    const actionHandlers = languageScene.actions
    const backToMenuHandler = actionHandlers.find(
      handler => handler.value === 'back_to_menu'
    )
    ...
    if (enterCalls[0][0] !== 'menuScene') {
      throw new Error(
        `Неверная сцена: ожидалась 'menuScene', получена '${enterCalls[0][0]}'`
      )
    }
...
export async function testLanguageScene_CurrentLanguageIndicator(): Promise<TestResult> {
  ...
  try {
    ...
    const russianButton = allButtons.find((button: string) =>
      button.includes('Русский')
    )
    const englishButton = allButtons.find((button: string) =>
      button.includes('English')
    )
    ...
    if (englishButton.includes('✓')) {
      throw new Error(
        'Кнопка английского языка содержит индикатор выбора, хотя не должна'
      )
    }
