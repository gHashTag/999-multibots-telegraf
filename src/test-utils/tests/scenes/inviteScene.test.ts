  ...
  try {
    ...
    if ((ctx as any).replies && (ctx as any).replies.length > 0) {
      ...
      if (
        ...
        linkText.includes(
          `https://t.me/${TEST_BOT_USERNAME}?start=${TEST_USER_ID}`
        )
      ) {
        return {
          ...
          success: true,
          message:
            'inviteScene правильно показывает информацию о приглашениях на русском языке',
        }
      } else {
        return {
          name: 'Вход в сцену inviteScene (русский)',
    ...
    if ((ctx as any).replies && (ctx as any).replies.length > 0) {
      ...
      if (
        ...
        return {
          ...
          success: true,
          message:
            'inviteScene правильно показывает информацию о приглашениях на английском языке',
        }
      } else {
        return {
          name: 'Вход в сцену inviteScene (английский)',
  ...
  try {
    ...
    if (sceneEnterCalled && sceneEnterCalledWithMenuScene) {
      return {
        ...
        success: true,
        message:
          'inviteScene правильно перенаправляет в главное меню после показа информации',
      }
    } else {
      return {
        name: 'Переход в главное меню из inviteScene',
    ...
    if ((ctx as any).replies && (ctx as any).replies.length > 0) {
      ...
      if (
        ...
        return {
          ...
          success: true,
          message:
            'inviteScene правильно обрабатывает ошибки при получении данных о рефералах',
        }
      } else {
        return {
          name: 'Обработка ошибок в inviteScene',
...
export async function runInviteSceneTests(): Promise<TestResult[]> {
  ...
  try {
    ...
    console.log(
      `✅ Тесты сцены приглашений: ${passCount}/${results.length} успешно`
    )
    return results
  } catch (error: any) {
