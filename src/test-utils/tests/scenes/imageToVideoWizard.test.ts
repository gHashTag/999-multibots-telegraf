...
const mockedValidateAndCalculateVideoModelPrice =
  createMockFunction<typeof validateAndCalculateVideoModelPrice>()
...
export async function testImageToVideoWizard_SelectModel(): Promise<TestResult> {
  try {
    ...
    expect(mockedValidateAndCalculateVideoModelPrice.mock.calls[0][0]).toBe(
      TEST_VIDEO_MODEL.toLowerCase()
    )
    ...
    return {
      ...
      success: true,
      message:
        'Successfully selected video model and progressed to image upload step',
    }
  } catch (error) {
...
export async function testImageToVideoWizard_UploadImage(): Promise<TestResult> {
  return runTest(
    async () => {
      ...
      return {
        message:
          'Successfully processed image upload and progressed to text prompt step',
      }
    },
    {
  ...
  return runTest(
    async () => {
      ...
      try {
        ...
        const { imageToVideoWizard } = await import(
          '@/scenes/imageToVideoWizard'
        )
        ...
        return {
          message:
            'Successfully processed text prompt and sent generation request',
        }
      } finally {
    ...
    toHaveBeenCalledWith: (...args: any[]) => {
      ...
      for (let i = 0; i < args.length; i++) {
        if (lastCall[i] !== args[i]) {
          throw new Error(
            `Expected function to have been called with ${JSON.stringify(args)} but was called with ${JSON.stringify(lastCall)}`
          )
        }
      }
    },
