...
import { MockFunction, createMockFunction } from '@/test-utils/mocks/mockFunction'
...
describe('promptEnhancerScene Tests', () => {
  ...
  async function testPromptEnhancerScene_Enter(): Promise<TestResult> {
    try {
      ...
      console.error('❌ Ошибка в тесте входа в сцену:', error instanceof Error ? error.message : String(error));
      return {
        name: 'testPromptEnhancerScene_Enter',
        category: 'promptEnhancer',
        success: false,
  ...
  async function testPromptEnhancerScene_EnterEnglish(): Promise<TestResult> {
    try {
      ...
      console.error('❌ Ошибка в тесте входа в сцену (EN):', error instanceof Error ? error.message : String(error));
      return {
        name: 'testPromptEnhancerScene_EnterEnglish',
        category: 'promptEnhancer',
        success: false,
  ...
  async function testPromptEnhancerScene_EnhancePrompt(): Promise<TestResult> {
    try {
      ...
      console.error('❌ Ошибка в тесте улучшения промпта:', error instanceof Error ? error.message : String(error));
      return {
        name: 'testPromptEnhancerScene_EnhancePrompt',
        category: 'promptEnhancer',
        success: false,
  ...
  async function testPromptEnhancerScene_HandleError(): Promise<TestResult> {
    try {
      ...
      console.log('✅ Тест успешно завершен: обработка ошибки улучшения промпта');
      return {
      ...
      console.error('❌ Ошибка в тесте обработки ошибки:', error instanceof Error ? error.message : String(error));
      return {
        name: 'testPromptEnhancerScene_HandleError',
        category: 'promptEnhancer',
        success: false,
