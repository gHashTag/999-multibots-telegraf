...
const mockedGetReferalsCountAndUserData = mockFunction<typeof getReferalsCountAndUserData>();
...
function setupTest() {
  ...
  mockedGetTranslation.mockReturnValue(Promise.resolve({
...
export async function testMenuSceneEnter(): Promise<TestResult> {
  try {
    ...
    // Проверки
    expect(mockedGetReferalsCountAndUserData).toHaveBeenCalledWith(TEST_USER_ID.toString());
...
export async function testMenuSceneWithNeuroPhotoSubscription(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест отображения клавиатуры с подпиской нейрофото успешно пройден'
    };
  } catch (error) {
    logger.error('Ошибка в тесте отображения клавиатуры с подпиской нейрофото:', error);
    return {
      name: 'menuScene: Отображение клавиатуры с подпиской нейрофото',
      category: TestCategory.All,
      success: false,
...
export async function testMenuSceneWithNeuroTesterSubscription(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест отображения клавиатуры с подпиской neurotester успешно пройден'
    };
  } catch (error) {
    logger.error('Ошибка в тесте отображения клавиатуры с подпиской neurotester:', error);
    return {
      name: 'menuScene: Отображение клавиатуры с подпиской neurotester',
      category: TestCategory.All,
      success: false,
...
export async function testMenuSceneWithoutFullAccess(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест отображения клавиатуры без полного доступа успешно пройден'
    };
  } catch (error) {
    logger.error('Ошибка в тесте отображения клавиатуры без полного доступа:', error);
    return {
      name: 'menuScene: Отображение клавиатуры без полного доступа',
      category: TestCategory.All,
      success: false,
