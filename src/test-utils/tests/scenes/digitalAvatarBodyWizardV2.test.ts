...
export async function testDigitalAvatarBodyWizardV2_SelectSteps1000(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      success: true,
      message: 'Успешно выбрано 1000 шагов V2 и перенаправлено на следующую сцену'
    };
  } catch (error) {
    return {
      name: 'digitalAvatarBodyWizardV2: Select 1000 Steps',
...
export async function testDigitalAvatarBodyWizardV2_SelectSteps3000(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      success: true,
      message: 'Успешно выбрано 3000 шагов V2 и перенаправлено на следующую сцену'
    };
  } catch (error) {
    return {
      name: 'digitalAvatarBodyWizardV2: Select 3000 Steps',
...
export async function testDigitalAvatarBodyWizardV2_Localization(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      success: true,
      message: 'Корректно обрабатывается локализация для русского и английского языков в V2'
    };
  } catch (error) {
    return {
      name: 'digitalAvatarBodyWizardV2: Localization',
...
export async function testDigitalAvatarBodyWizardV2_CostComparison(): Promise<TestResult> {
  try {
    ...
    ctxV1.from = { id: 123456789, is_bot: false, first_name: 'Test', language_code: 'ru' };
    
    const ctxV2 = createMockWizardContext();
    ctxV2.from = { id: 123456789, is_bot: false, first_name: 'Test', language_code: 'ru' };
    ...
    return {
      ...
      success: true,
      message: 'Успешно подтверждено, что V2 имеет отличное от V1 ценообразование'
    };
  } catch (error) {
    return {
      name: 'digitalAvatarBodyWizardV2: Cost Comparison',
 ...
 */
export async function runDigitalAvatarBodyWizardV2Tests(): Promise<TestResult[]> {
  console.log('🧪 Запуск тестов сцены digitalAvatarBodyWizardV2 (Цифровое тело 2)...');
  ...
  try {
    ...
    console.error('❌ Ошибка при запуске тестов digitalAvatarBodyWizardV2:', error);
  }
