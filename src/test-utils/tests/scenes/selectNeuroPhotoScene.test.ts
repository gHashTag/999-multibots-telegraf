...
const mockFn = () => {
  return {
    mockReturnValue: (val: any) => ({ mock: { calls: [] }, mockReturnValue: () => val }),
    mock: { calls: [] }
  };
};
...
const assertReplyContains = (ctx: any, expectedText: string) => {
  ...
  if (!replyCall) {
    throw new Error(`Expected reply with text containing "${expectedText}" but not found`);
  }
};

const assertReplyKeyboard = (ctx: any, expectedButtons: string[][]) => {
  const replyCall = ctx.reply.mock.calls.find(
    (call: any[]) => call[1] && call[1].reply_markup && call[1].reply_markup.keyboard
  );
  ...
  expectedButtons.forEach((expectedRow, rowIndex) => {
    expectedRow.forEach((expectedButton, buttonIndex) => {
      const buttonFound = keyboard.some((row: string[]) => row.some((button: string) => button.includes(expectedButton)));
      if (!buttonFound) {
        throw new Error(`Expected button "${expectedButton}" but not found in keyboard`);
      }
    });
  });
};
 ...
 */
const createTestContext = (options: { language?: string, text?: string } = {}) => {
...
export async function testSelectNeuroPhotoScene_SelectFlux(): Promise<TestResult> {
  try {
    ...
    if ((ctx as any).session.mode !== ModeEnum.NeuroPhoto) {
      throw new Error(`Expected session.mode to be ${ModeEnum.NeuroPhoto} but got ${(ctx as any).session.mode}`);
    }

    // Проверяем, что произошел переход на сцену CheckBalanceScene
    if (!(ctx as any).scene.enter.mock.calls.some((call: any[]) => call[0] === ModeEnum.CheckBalanceScene)) {
      throw new Error('Expected scene.enter to be called with CheckBalanceScene');
    }
...
export async function testSelectNeuroPhotoScene_SelectFluxPro(): Promise<TestResult> {
  try {
    ...
    if ((ctx as any).session.mode !== ModeEnum.NeuroPhotoV2) {
      throw new Error(`Expected session.mode to be ${ModeEnum.NeuroPhotoV2} but got ${(ctx as any).session.mode}`);
    }
...
export async function testSelectNeuroPhotoScene_Localization(): Promise<TestResult> {
  try {
    ...
    assertReplyKeyboard(ctxRussian, [
...
export async function testSelectNeuroPhotoScene_KeywordRecognition(): Promise<TestResult> {
  try {
    ...
    for (const text of fluxVariants) {
      ...
      // Проверяем, что был установлен правильный режим
      if ((ctx as any).session.mode !== ModeEnum.NeuroPhoto) {
        throw new Error(`Expected session.mode to be ${ModeEnum.NeuroPhoto} for text "${text}" but got ${(ctx as any).session.mode}`);
      }
    }
    ...
    for (const text of fluxProVariants) {
      ...
      // Проверяем, что был установлен правильный режим
      if ((ctx as any).session.mode !== ModeEnum.NeuroPhotoV2) {
        throw new Error(`Expected session.mode to be ${ModeEnum.NeuroPhotoV2} for text "${text}" but got ${(ctx as any).session.mode}`);
      }
    }
...
export async function testSelectNeuroPhotoScene_CheckBalanceIntegration(): Promise<TestResult> {
  try {
    ...
    if (!balanceSceneCall) {
      throw new Error(`Expected scene.enter to be called with ${ModeEnum.CheckBalanceScene} but got ${
        enterCalls.map((call: any[]) => call[0]).join(', ')
      }`);
    }
...
export async function testSelectNeuroPhotoScene_VeryLongInput(): Promise<TestResult> {
  try {
    ...
    const { ctx } = createTestContext({ 
      text: 'This is a very long input that exceeds normal message length limits and should still be properly handled by the scene. '.repeat(10) 
    });
...
export async function testSelectNeuroPhotoScene_StatePersistence(): Promise<TestResult> {
  try {
    ...
    if ((ctx as any).session.__scenes.state.testValue !== 'test-state-persistence') {
...
export async function runSelectNeuroPhotoSceneTests(): Promise<TestResult[]> {
  console.log('🧪 Запуск тестов сцены выбора нейрофото (selectNeuroPhotoScene)...');
  ...
  console.log(`📊 Результаты тестов selectNeuroPhotoScene: всего ${results.length}, успешно ${successful}, с ошибками ${failed}`);
...
if (require.main === module) {
  runSelectNeuroPhotoSceneTests()
    ...
    .catch(error => {
      console.error('❌ Критическая ошибка при запуске тестов selectNeuroPhotoScene:', error);
      process.exit(1);
    });
} 