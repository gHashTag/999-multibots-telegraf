...
function setupContext(params: {
  ...
  ctx.replyWithPhoto = jest.fn().mockImplementation((photo: string, extra?: any) => {
  ...
  if (params.hasPhoto) {
    ctx.message.photo = [
      { file_id: 'small-' + TEST_FILE_ID, file_unique_id: 'unique-small', width: 100, height: 100, file_size: 1024 },
      { file_id: TEST_FILE_ID, file_unique_id: 'unique-id', width: 800, height: 600, file_size: 102400 }
    ];
  }
...
async function testStyleTransferScene_Enter(): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      category: TestCategory.SCENE,
      success: true,
      message: 'Сцена корректно отображает приветственное сообщение на русском языке',
    };
  } catch (error) {
...
async function testStyleTransferScene_EnterEnglish(): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      category: TestCategory.SCENE,
      success: true,
      message: 'Сцена корректно отображает приветственное сообщение на английском языке',
    };
  } catch (error) {
...
async function testStyleTransferScene_NoSourceImage(): Promise<TestResult> {
  ...
  try {
    ...
    const replyText = ctx.replies?.[0]?.text;
    if (!replyText || (!replyText.includes('Пожалуйста, отправьте изображение') && !replyText.includes('фото'))) {
      return {
        name: 'Отсутствие исходного изображения',
        category: TestCategory.SCENE,
        success: false,
...
async function testStyleTransferScene_SelectStrength(): Promise<TestResult> {
  ...
  try {
    ...
    const styleTransferArgs = styleTransferModule.applyStyleTransfer.mock.calls[0];
    if (!styleTransferArgs || styleTransferArgs[0] !== TEST_PHOTO_URL || 
        styleTransferArgs[1] !== TEST_PHOTO_URL || styleTransferArgs[2] !== 75) {
      return {
        name: 'Выбор силы эффекта',
        category: TestCategory.SCENE,
        success: false,
    ...
    return {
      ...
      category: TestCategory.SCENE,
      success: true,
      message: 'Сцена корректно обрабатывает выбор силы эффекта и отправляет результат',
    };
  } catch (error) {
  ...
  try {
    ...
    if (styleTransferModule.applyStyleTransfer.mock.calls.length > 0) {
      return {
        ...
        category: TestCategory.SCENE,
        success: false,
        message: 'Метод applyStyleTransfer был вызван несмотря на неверный ввод',
      };
    }
