...
function getWizardStep(index: number) {
  if (
    Array.isArray(sizeWizard.middleware) &&
    index >= 0 &&
    index < sizeWizard.middleware.length
  ) {
...
async function testSizeWizardFirstStep(): Promise<TestResult> {
  ...
  try {
    ...
    if (actualMessage !== expectedMessage) {
      throw new Error(
        `Неправильное сообщение: ${actualMessage}, ожидалось: ${expectedMessage}`
      )
    }
...
async function testValidSizeSelection(): Promise<TestResult> {
  ...
  try {
    ...
    if (selectedSize !== '512x512') {
      throw new Error(
        `Неправильный размер: ${selectedSize}, ожидалось: 512x512`
      )
    }
...
async function testNoTextMessage(): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      success: true,
      message:
        'Успешная проверка отправки текстового сообщения вместо выбора размера',
    }
  } catch (error: any) {
...
export async function runSizeWizardTests(): Promise<TestResult[]> {
  ...
  console.log(
    `Тесты для размерного визарда завершены: ${successCount}/${results.length} успешно`
  )
