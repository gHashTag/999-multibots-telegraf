...
function setupTest() {
  ...
  mockedGenerateUniqueShortInvId.mockReturnValue(Promise.resolve(Number(TEST_INV_ID)));
...
export async function runPaymentSceneTests(): Promise<TestResult[]> {
  ...
  logger.info(`📊 Результаты тестирования paymentScene: Всего ${results.length}, Успех: ${successCount}, Ошибки: ${failCount}`);
