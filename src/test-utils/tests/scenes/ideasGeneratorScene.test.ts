...
export async function testIdeasGeneratorScene_Enter(): Promise<TestResult> {
  ...
  try {
    ...
    // Импортируем сцену
    const { ideasGeneratorScene } = await import(
      '../../../scenes/ideasGeneratorScene'
    )
    ...
    logger.info(
      '✅ Тест успешно пройден: вход в сцену генератора идей (русский язык)'
    )
    ...
    return {
      ...
      success: true,
      message:
        'Тест входа в сцену генератора идей (русский язык) успешно пройден',
    }
  } catch (error) {
    logger.error(
      '❌ Ошибка в тесте входа в сцену генератора идей (русский язык):',
      error
    )
...
export async function testIdeasGeneratorScene_EnterEnglish(): Promise<TestResult> {
  ...
  try {
    ...
    logger.info(
      '✅ Тест успешно пройден: вход в сцену генератора идей (английский язык)'
    )
    ...
    return {
      ...
      success: true,
      message:
        'Тест входа в сцену генератора идей (английский язык) успешно пройден',
    }
  } catch (error) {
    logger.error(
      '❌ Ошибка в тесте входа в сцену генератора идей (английский язык):',
      error
    )
...
export async function testIdeasGeneratorScene_GenerateIdeas(): Promise<TestResult> {
  ...
  try {
    ...
    logger.error(
      '❌ Ошибка в тесте обработки запроса на генерацию идей:',
      error
    )
...
export async function testIdeasGeneratorScene_NoTextMessage(): Promise<TestResult> {
  ...
  try {
    ...
    logger.info(
      '✅ Тест успешно пройден: обработка отсутствия текста сообщения'
    )
    ...
    logger.error(
      '❌ Ошибка в тесте обработки отсутствия текста сообщения:',
      error
    )
...
export async function runIdeasGeneratorSceneTests(): Promise<TestResult[]> {
  ...
  logger.info(
    `📊 Результаты тестов: ${successCount}/${results.length} успешно, ${failCount} неудачно`
  )
...
function expect<T = any, R = any>(mockFn: MockedFunction<T, R>) {
  return {
    ...
    toHaveBeenCalledWith: (expectedArg: any) => {
      ...
      if (!wasCalled) {
        throw new Error(
          `Ожидался вызов функции с аргументом: ${JSON.stringify(expectedArg)}`
        )
      }
    },
  }
