...
const TEST_TEXT_PROMPT =
  'A beautiful sunset with ocean waves crashing on the shore'
...
export async function testTextToVideoWizard_ProvideImageForImageModel(): Promise<TestResult> {
  try {
    ...
    assertReplyContains(
      ctx,
      'Эта модель требует изображение для генерации видео'
    )
    ...
    logger.error(
      'Error in testTextToVideoWizard_ProvideImageForImageModel:',
      error
    )
    return {
      name: 'textToVideoWizard: Provide Image For Image Model',
