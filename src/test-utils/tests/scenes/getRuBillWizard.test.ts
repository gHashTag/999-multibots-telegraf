...
describe('getRuBillWizard', () => {
  ...
  let mockUpdateUserSubscription: MockFunction<
    typeof supabaseModule.updateUserSubscription
  >
  ...
  let mockGenerateUniqueShortInvId: MockFunction<
    typeof helper.generateUniqueShortInvId
  >
  ...
  beforeEach(() => {
    ...
    mockGetInvoiceId = makeMockFunction(
      jest.fn(async () => 'https://test-invoice-url.com')
    )
    mockUpdateUserSubscription = makeMockFunction(
      jest.fn(async () => ({ success: true }))
    )
    mockCreatePayment = makeMockFunction(
      jest.fn(async () => ({ success: true }))
    )
    mockGetBotNameByToken = makeMockFunction(
      jest.fn(() => ({ bot_name: 'TestBot' }))
    )
    ...
    jest
      .spyOn(helper, 'generateUniqueShortInvId')
      .mockImplementation(mockGenerateUniqueShortInvId)
    jest
      .spyOn(supabaseModule, 'updateUserSubscription')
      .mockImplementation(mockUpdateUserSubscription)
    jest
      .spyOn(supabaseModule, 'createPayment')
      .mockImplementation(mockCreatePayment)
    jest
      .spyOn(coreModule, 'getBotNameByToken')
      .mockImplementation(mockGetBotNameByToken)
    jest
      .spyOn(require('@/helpers'), 'isRussian')
      .mockImplementation(mockIsRussian)
  })
  ...
  const getRuBillWizardTests: TestSuite = {
    ...
    testGenerateInvoiceStep_MissingPayment:
      async function (): Promise<TestResults> {
        ...
        try {
          ...
          return {
            ...
            success: true,
            message:
              '✅ Тест прошел успешно: правильно обработано отсутствие способа оплаты',
          }
        } catch (error) {
          logError(`❌ Ошибка в тесте: ${error.message}`)
          return {
    ...
    testGenerateInvoiceStep_InvoiceError:
      async function (): Promise<TestResults> {
        ...
        try {
          ...
          assertCalledWith(
            logger.error,
            expect.stringContaining('Invoice generation failed')
          )
          ...
          return {
            ...
            success: true,
            message:
              '✅ Тест прошел успешно: правильно обработана ошибка создания счета',
          }
        } catch (error) {
          logError(`❌ Ошибка в тесте: ${error.message}`)
          return {
    ...
    testGenerateInvoiceStep_MissingUserId:
      async function (): Promise<TestResults> {
        ...
        try {
          ...
          return {
            ...
            success: true,
            message:
              '✅ Тест прошел успешно: правильно обработано отсутствие ID пользователя',
          }
        } catch (error) {
          logError(`❌ Ошибка в тесте: ${error.message}`)
          return {
