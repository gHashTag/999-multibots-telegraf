...
const setupTest = () => {
  ...
  (supabaseModule as any).getUserByTelegramIdString = mockGetUserByTelegramIdString;
 ...
 */
const createTestContext = (options: { startCommand?: string, inviteCode?: string } = {}) => {
...
export async function testCreateUserScene_CreateUserWithoutReferral(): Promise<TestResult> {
  ...
  try {
    ...
    assertReplyContains(replyMock, 'created successfully', 'Сообщение о успешном создании аватара не найдено');
    ...
    if (enterCalls[0][0] !== ModeEnum.SubscriptionCheckScene) {
      throw new Error(`Неверная сцена: ожидалась '${ModeEnum.SubscriptionCheckScene}', получена '${enterCalls[0][0]}'`);
    }
    ...
    return {
      ...
      success: true,
      message: 'Тест создания пользователя без реферальной ссылки успешно выполнен',
    };
  } catch (error) {
...
export async function testCreateUserScene_CreateUserWithReferral(): Promise<TestResult> {
  ...
  try {
    ...
    if (sendMessageMock.mock.calls.length < 2) {
      throw new Error('Метод sendMessage не был вызван для уведомления реферера');
    }
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест создания пользователя с реферальной ссылкой успешно выполнен',
    };
  } catch (error) {
    logger.error(`[TEST] Ошибка в тесте ${testName}:`, error);
    return {
  ...
  try {
    ...
    try {
      ...
      if (!(error instanceof Error) || !error.message.includes('User data not found')) {
        throw new Error(`Неожиданный текст ошибки: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест обработки ошибки при отсутствии данных пользователя успешно выполнен',
    };
  } catch (error) {
    logger.error(`[TEST] Ошибка в тесте ${testName}:`, error);
    return {
...
export async function testCreateUserScene_HandleMissingMessageText(): Promise<TestResult> {
  ...
  try {
    ...
    try {
      ...
      // Проверяем, что ошибка содержит ожидаемое сообщение
      if (!(error instanceof Error) || !error.message.includes('Message text not found')) {
        throw new Error(`Неожиданный текст ошибки: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест обработки ошибки при отсутствии текста сообщения успешно выполнен',
    };
  } catch (error) {
    logger.error(`[TEST] Ошибка в тесте ${testName}:`, error);
    return {
...
export async function testCreateUserScene_CreateUserWithFullReferralLink(): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      category: TestCategory.All,
      success: true,
      message: 'Тест создания пользователя с полной реферальной ссылкой успешно выполнен',
    };
  } catch (error) {
    logger.error(`[TEST] Ошибка в тесте ${testName}:`, error);
    return {
