...
export async function testSelectModelScene_ChooseFlux(): Promise<TestResult> {
  try {
    ...
    if (ctx.session.mode !== ModeEnum.DigitalAvatarBody) {
      throw new Error(
        `Ожидалось установление режима ${ModeEnum.DigitalAvatarBody}, но получили ${ctx.session.mode}`
      )
    }
    ...
    const lastSceneEnterCall =
      ctx.scene.enter.mock.calls[ctx.scene.enter.mock.calls.length - 1]
    if (lastSceneEnterCall[0] !== ModeEnum.CheckBalanceScene) {
      throw new Error(
        `Ожидался переход в сцену ${ModeEnum.CheckBalanceScene}, но перешли в ${lastSceneEnterCall[0]}`
      )
    }
...
export async function testSelectModelScene_ChooseFluxPro(): Promise<TestResult> {
  try {
    ...
    if (ctx.session.mode !== ModeEnum.DigitalAvatarBodyV2) {
      throw new Error(
        `Ожидалось установление режима ${ModeEnum.DigitalAvatarBodyV2}, но получили ${ctx.session.mode}`
      )
    }
...
export async function testSelectModelScene_EnglishLocalization(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      success: true,
      message:
        'Тест локализации сообщения об ошибке на английском успешно пройден',
    }
  } catch (error) {
