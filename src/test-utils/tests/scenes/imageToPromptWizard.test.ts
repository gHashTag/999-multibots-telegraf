  ...
  try {
    ...
    try {
      ...
      return {
        ...
        category: TestCategory.All,
        success: true,
        message: 'Successfully processed image upload and sent event to generate prompt'
      }
    } finally {
...
export async function testImageToPromptWizard_ProcessingError(): Promise<TestResult> {
  try {
    ...
    ctx.telegram.getFile = () => { throw new Error('Test error') }
 ...
 */
function expectMockCallWith(mock: any, index: number, paramIndex: number, expected: any) {
  if (!mock.mock || !Array.isArray(mock.mock.calls) || mock.mock.calls.length <= index) {
    throw new Error(`Expected mock to have at least ${index + 1} calls, but it has ${mock.mock?.calls?.length || 0}`);
  }
  
  const call = mock.mock.calls[index];
  if (!Array.isArray(call) || call.length <= paramIndex) {
    throw new Error(`Expected call to have at least ${paramIndex + 1} parameters, but it has ${call?.length || 0}`);
  }
  
  if (call[paramIndex] !== expected) {
    throw new Error(`Expected parameter ${paramIndex} to be ${expected}, but it was ${call[paramIndex]}`);
  }
}
    ...
    toHaveBeenCalledWith: (...args: any[]) => {
      ...
      for (let i = 0; i < args.length; i++) {
        if (lastCall[i] !== args[i]) {
          throw new Error(`Expected function to have been called with ${args[i]} at position ${i} but got ${lastCall[i]}`);
        }
      }
    },
