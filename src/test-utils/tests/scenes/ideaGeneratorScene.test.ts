 ...
 */
export default async function runIdeaGeneratorSceneTests(): Promise<TestResult[]> {
  logger.info('⚠️ ideaGeneratorScene.test.ts: Перенаправление к ideasGeneratorScene.test.ts');
  
  // Импортируем фактический файл с тестами
  const { default: runIdeasGeneratorTests } = await import('./ideasGeneratorScene.test');
...
async function testIdeaGeneratorScene_RedirectToIdeas(): Promise<TestResult> {
  try {
    logger.info('⚠️ Вызов testIdeaGeneratorScene_RedirectToIdeas перенаправлен к ideasGeneratorScene.test.ts');
