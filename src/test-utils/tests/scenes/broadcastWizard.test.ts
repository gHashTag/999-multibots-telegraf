...
import { broadcastWizard, BroadcastContentType } from '../../../scenes/broadcastWizard';
...
async function setupContext(language: string = 'ru'): Promise<any> {
  ...
  ctx.reply = mockApi.create().mockImplementation((text: string, extra: any) => {
...
async function testBroadcastWizard_Start(): Promise<TestResult> {
  ...
  try {
    ...
    if (!actualMessage.includes(expectedMessage)) {
      throw new Error(`Неправильное сообщение: ${actualMessage}, ожидалось содержание: ${expectedMessage}`);
    }
...
async function testBroadcastWizard_SelectContentType(): Promise<TestResult> {
  ...
  try {
    ...
    if (ctx.scene.session.contentType !== BroadcastContentType.PHOTO) {
      throw new Error(`Неправильный тип контента сохранен: ${ctx.scene.session.contentType}, ожидался: ${BroadcastContentType.PHOTO}`);
    }
...
async function testBroadcastWizard_UploadPhoto(): Promise<TestResult> {
  ...
  try {
    ...
    if (ctx.scene.session.photoFileId !== TEST_PHOTO_FILE_ID) {
      throw new Error(`Неправильный file_id фото сохранен: ${ctx.scene.session.photoFileId}, ожидался: ${TEST_PHOTO_FILE_ID}`);
    }
...
async function testBroadcastWizard_InputText(): Promise<TestResult> {
  ...
  try {
    ...
    if (ctx.scene.session.textRu !== TEST_TEXT_RU) {
      throw new Error(`Неправильный русский текст сохранен: ${ctx.scene.session.textRu}, ожидался: ${TEST_TEXT_RU}`);
    }
    ...
    if (ctx.scene.session.textInputStep !== 'english') {
      throw new Error(`Неправильный этап ввода текста: ${ctx.scene.session.textInputStep}, ожидался: english`);
    }
    ...
    if (ctxEn.scene.session.textEn !== TEST_TEXT_EN) {
      throw new Error(`Неправильный английский текст сохранен: ${ctxEn.scene.session.textEn}, ожидался: ${TEST_TEXT_EN}`);
    }
    
    // Проверяем, что этап ввода текста изменился на завершенный
    if (ctxEn.scene.session.textInputStep !== 'completed') {
      throw new Error(`Неправильный этап ввода текста: ${ctxEn.scene.session.textInputStep}, ожидался: completed`);
    }
...
async function testBroadcastWizard_Confirm(): Promise<TestResult> {
  ...
  try {
    ...
    const originalBroadcastService = (broadcastWizard as any).__proto__.broadcastService;
    ...
    const hasSendBroadcastCalls = mockBroadcastService.sendBroadcastWithImage.mock?.calls?.length > 0;
    ...
    (broadcastWizard as any).__proto__.broadcastService = originalBroadcastService;
...
async function testBroadcastWizard_Cancel(): Promise<TestResult> {
  ...
  try {
    ...
    if (!actualMessage2.includes('отменена')) {
      throw new Error(`Неправильное сообщение: ${actualMessage2}, ожидалось содержание: ${expectedMessage}`);
    }
...
export async function runBroadcastWizardTests(): Promise<TestResult[]> {
  ...
  console.log(`Тесты для визарда рассылки завершены: ${successCount}/${results.length} успешно`);
