...
import { assertReplyContains, assertReplyMarkupContains, assertScene, assertPhotoSent } from '../../core/assertions';
...
export async function testTextToImageWizard_EnterScene(): Promise<TestResult> {
  try {
    ...
    const textToImageWizard = (await import('@/scenes/textToImageWizard')).textToImageWizard;
    ...
    const modelShortNames = Object.values(imageModelPrices)
      .filter(model => !model.inputType.includes('dev') && (model.inputType.includes('text')))
      .map(model => model.shortName);
      
    for (const modelName of modelShortNames.slice(0, 3)) { // Check just a few models to confirm
...
export async function testTextToImageWizard_SelectImageModel(): Promise<TestResult> {
  try {
    ...
    if ((ctx.session as any).selectedModel !== TEST_MODEL_ID) {
      throw new Error(`Expected model ${TEST_MODEL_ID} to be saved in session but got ${(ctx.session as any).selectedModel}`);
    }
...
export async function testTextToImageWizard_ProvideTextPrompt(): Promise<TestResult> {
  try {
    ...
    if ((ctx.session as any).prompt !== TEST_PROMPT) {
      throw new Error(`Expected prompt "${TEST_PROMPT}" to be saved in session but got "${(ctx.session as any).prompt}"`);
    }
...
function expect(value: any): { toHaveBeenCalled: () => void; toHaveBeenCalledTimes: (n: number) => void } {
  return {
    toHaveBeenCalled: () => {
    ...
    toHaveBeenCalledTimes: (n: number) => {
      if (!value || !value.mock || value.mock.calls.length !== n) {
        throw new Error(`Expected function to have been called ${n} times, but it was called ${value?.mock?.calls?.length || 0} times`);
      }
    }
  };
} 