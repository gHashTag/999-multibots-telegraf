...
export async function testVoiceAvatarWizard_EnterScene(): Promise<TestResult> {
  return runTest(
    async () => {
      ...
      testExpect(mockedGetUserBalance.mock.calls[0][0]).toBe(TEST_USER_ID.toString());
  ...
  return runTest(
    async () => {
      ...
      try {
        ...
        testExpect(ctx.telegram.getFile.mock.calls[0][0]).toBe(TEST_VOICE_FILE_ID);
        ...
        return {
          message: 'Успешно обработано голосовое сообщение и отправлен запрос на создание аватара'
        };
      } finally {
  ...
  return runTest(
    async () => {
      ...
      try {
        ...
        return {
          message: 'Успешно обработано аудио сообщение и отправлен запрос на создание аватара'
        };
      } finally {
        // Восстанавливаем оригинальную функцию, даже если тест не прошел
        restoreInngest();
...
export async function testVoiceAvatarWizard_FileError(): Promise<TestResult> {
  return runTest(
    async () => {
      ...
      return {
        message: 'Успешно обработана ошибка получения файла голосового сообщения'
      };
    },
    {
