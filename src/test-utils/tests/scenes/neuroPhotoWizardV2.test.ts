...
// Mocked functions
const mockedGetUserInfo =
  mockFunction<typeof import('@/handlers/getUserInfo').getUserInfo>()
const mockedGetLatestUserModel =
  mockFunction<typeof import('@/core/supabase').getLatestUserModel>()
const mockedGetReferalsCountAndUserData =
  mockFunction<typeof import('@/core/supabase').getReferalsCountAndUserData>()
const mockedSendPhotoDescriptionRequest =
  mockFunction<typeof import('@/menu').sendPhotoDescriptionRequest>()
const mockedGenerateNeuroImageV2 =
  mockFunction<
    typeof import('@/services/generateNeuroImageV2').generateNeuroImageV2
  >()
const mockedHandleHelpCancel =
  mockFunction<typeof import('@/handlers/handleHelpCancel').handleHelpCancel>()
...
function setupTest() {
  ...
  mockedGetLatestUserModel.mockReturnValue(
    Promise.resolve({
...
export async function testNeuroPhotoWizardV2_EnterScene(): Promise<TestResult> {
  try {
    ...
    const neuroPhotoWizardV2 = (await import('@/scenes/neuroPhotoWizardV2'))
      .neuroPhotoWizardV2
    ...
    expect(mockedGetLatestUserModel.mock.calls[0][0]).toBe(
    ...
    return {
      ...
      success: true,
      message:
        'Successfully entered neuroPhotoWizardV2 and displayed prompt instructions',
    }
  } catch (error) {
...
export async function testNeuroPhotoWizardV2_GenerateMultipleImages(): Promise<TestResult> {
  try {
    ...
    logger.error(
      'Error in testNeuroPhotoWizardV2_GenerateMultipleImages:',
      error
    )
    return {
      name: 'neuroPhotoWizardV2: Generate Multiple Images',
    ...
    toHaveBeenCalledWith: (...args: any[]) => {
      ...
      for (let i = 0; i < args.length; i++) {
        if (args[i] !== lastCall[i]) {
          throw new Error(
            `Expected call argument ${i} to be ${args[i]} but got ${lastCall[i]}`
          )
        }
      }
    },
