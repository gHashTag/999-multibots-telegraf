...
const runTests = async () => {
  ...
  try {
    ...
    if (ctx.reply.mock.calls.length > 0 && 
      ...
      console.log('✅ Тест 1 успешно пройден: Сцена отображает приветственное сообщение');
      passed++;
    } else {
  ...
  try {
    ...
    if (ctx.reply.mock.calls.some(call => 
        call[0].includes('American') && call[0].includes('selected'))) {
      console.log('✅ Тест 2 успешно пройден: Сцена обрабатывает выбор американского акцента');
      passed++;
    } else {
  ...
  try {
    ...
    // Проверяем подтверждение выбора
    if (ctx.reply.mock.calls.some(call => 
        call[0].includes('British') && call[0].includes('selected'))) {
      console.log('✅ Тест 3 успешно пройден: Сцена обрабатывает выбор британского акцента');
      passed++;
    } else {
  ...
  try {
    ...
    if (ctx.scene.leave.mock.calls.length > 0) {
      console.log('✅ Тест 4 успешно пройден: Сцена обрабатывает команду отмены');
      passed++;
    } else {
  ...
  try {
    ...
    if (ctx.reply.mock.calls.some(call => call[0].includes('help'))) {
      console.log('✅ Тест 5 успешно пройден: Сцена обрабатывает команду помощи');
      passed++;
    } else {
  ...
  try {
    ...
    if (ctx.reply.mock.calls.some(call => 
        call[0].includes('Выберите голос') || 
        call[0].includes('акцент'))) {
      console.log('✅ Тест 8 успешно пройден: Сцена поддерживает русскую локализацию');
      passed++;
    } else {
  ...
  try {
    ...
    if (ctx.reply.mock.calls.some(call => 
        call[0].includes('Choose a voice') || 
        call[0].includes('accent'))) {
      console.log('✅ Тест 9 успешно пройден: Сцена поддерживает английскую локализацию');
      passed++;
    } else {
