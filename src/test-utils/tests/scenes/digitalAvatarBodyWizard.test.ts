...
export async function testDigitalAvatarBodyWizard_Localization(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      success: true,
      message:
        'Корректно обрабатывается локализация для русского и английского языков',
    }
  } catch (error) {
    return {
      name: 'digitalAvatarBodyWizard: Localization',
...
export async function runDigitalAvatarBodyWizardTests(): Promise<TestResult[]> {
  console.log(
    '🧪 Запуск тестов сцены digitalAvatarBodyWizard (Цифровое тело 1)...'
  )
  ...
  try {
    ...
    console.error(
      '❌ Ошибка при запуске тестов digitalAvatarBodyWizard:',
      error
    )
  }
