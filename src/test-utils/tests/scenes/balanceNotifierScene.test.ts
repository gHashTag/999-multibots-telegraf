    ...
    if ((ctx as any).replies && (ctx as any).replies.length > 0) {
      ...
      if (
        ...
        return {
          ...
          success: true,
          message:
            'balanceNotifierScene правильно показывает настройки уведомлений на русском языке',
        }
      } else {
        return {
          name: 'Вход в сцену balanceNotifierScene (русский)',
    ...
    if ((ctx as any).replies && (ctx as any).replies.length > 0) {
      ...
      if (
        ...
        return {
          ...
          success: true,
          message:
            'balanceNotifierScene правильно показывает настройки уведомлений на английском языке',
        }
      } else {
        return {
          name: 'Вход в сцену balanceNotifierScene (английский)',
 ...
 */
const testBalanceNotifierSceneToggleNotifications =
  async (): Promise<TestResult> => {
    try {
      ...
      const actionHandler = balanceNotifierScene
        .action('toggle_notifications', (ctx: any) => {})
        .middleware() as Middleware<MyContext>
      await invokeHandler(actionHandler, ctx)

      // Assert
      if (ctx.session?.balanceNotifications?.enabled === true) {
        // Check if scene.reenter was called
        const sceneReenterCalled =
          ((ctx.scene.reenter as any).mock?.calls?.length || 0) > 0
        ...
        if (sceneReenterCalled) {
          ...
          return {
            ...
            success: false,
            message:
              'balanceNotifierScene не вызывает reenter после изменения настроек',
          }
        }
      } else {
        return {
      ...
      console.error(
        'Ошибка в testBalanceNotifierSceneToggleNotifications:',
        error
      )
      return {
        name: 'Включение уведомлений в balanceNotifierScene',
 ...
 */
const testBalanceNotifierSceneChangeThreshold =
  async (): Promise<TestResult> => {
    try {
      // Arrange
      setupTest()
      ...
      // Act - invoke the action handler
      const actionHandler = balanceNotifierScene
        .action('change_threshold', (ctx: any) => {})
        .middleware() as Middleware<MyContext>
      await invokeHandler(actionHandler, ctx)
      ...
      if ((ctx as any).replies && (ctx as any).replies.length > 0) {
        ...
        if (
          ...
          return {
            ...
            success: true,
            message:
              'balanceNotifierScene правильно запрашивает новый порог уведомлений',
          }
        } else {
          return {
            name: 'Запрос изменения порога в balanceNotifierScene',
...
const testBalanceNotifierSceneSetThreshold = async (): Promise<TestResult> => {
  try {
    ...
    const textHandler = balanceNotifierScene
      .on('text', (ctx: any) => {})
      .middleware() as Middleware<MyContext>
    ...
    if (thresholdUpdated && sceneReenterCalled) {
      return {
        ...
        success: true,
        message:
          'balanceNotifierScene правильно устанавливает новый порог уведомлений',
      }
    } else {
      return {
        name: 'Установка порога уведомлений в balanceNotifierScene',
 ...
 */
const testBalanceNotifierSceneCommandHandling =
  async (): Promise<TestResult> => {
    try {
      // Arrange
      setupTest()
      ...
      const commandHandler = balanceNotifierScene
        .command(['start', 'menu', 'exit', 'cancel'], (ctx: any) => {})
        .middleware() as Middleware<MyContext>
...
export async function runBalanceNotifierSceneTests(): Promise<TestResult[]> {
  ...
  try {
    ...
    console.log(
      `✅ Тесты сцены уведомлений о балансе: ${passCount}/${results.length} успешно`
    )
    return results
  } catch (error: any) {
    console.error(
      '❌ Тесты сцены уведомлений о балансе завершились с ошибкой:',
      error
    )
    results.push({
