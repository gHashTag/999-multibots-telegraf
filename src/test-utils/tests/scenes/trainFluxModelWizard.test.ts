...
export async function testTrainFluxModelWizard_EnterScene(): Promise<TestResult> {
  try {
    ...
    ctx.from = { id: 123456789, is_bot: false, first_name: 'Test', username: 'test_user', language_code: 'ru' };
...
export async function testTrainFluxModelWizard_NotEnoughImages(): Promise<TestResult> {
  try {
    ...
    return {
      ...
      success: true,
      message: 'Корректно обработана ситуация с недостаточным количеством изображений'
    };
  } catch (error) {
    return {
      name: 'trainFluxModelWizard: Not Enough Images',
...
export async function testTrainFluxModelWizard_CompleteUpload(): Promise<TestResult> {
  try {
    ...
    ctx.session = {
      ...ctx.session,
      images: Array(10).fill({ buffer: Buffer.from('test'), filename: 'test.jpg' })
    };
    ...
    return {
      ...
      success: true,
      message: 'Успешно завершена загрузка фотографий и выполнен переход на следующую сцену'
    };
  } catch (error) {
    return {
      name: 'trainFluxModelWizard: Complete Upload',
...
export async function testTrainFluxModelWizard_TooLargeImage(): Promise<TestResult> {
  try {
    ...
    global.fetch = jest.fn().mockResolvedValue({
      arrayBuffer: jest.fn().mockResolvedValue(new ArrayBuffer(15 * 1024 * 1024))
    });
...
export async function runTrainFluxModelWizardTests(): Promise<TestResult[]> {
  console.log('🧪 Запуск тестов сцены trainFluxModelWizard (Загрузка фотографий)...');
