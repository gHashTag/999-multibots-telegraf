...
const mockedGenerateNeuroImage =
  mockFunction<
    typeof import('@/services/generateNeuroImage').generateNeuroImage
  >()
...
export async function testNeuroPhotoWizard_EnterScene(): Promise<TestResult> {
  try {
    ...
    const neuroPhotoWizard = (await import('@/scenes/neuroPhotoWizard'))
      .neuroPhotoWizard
    ...
    return {
      ...
      success: true,
      message:
        'Successfully entered neuroPhotoWizard and displayed prompt instructions',
    }
  } catch (error) {
...
function expect(value: any): {
  ...
  return {
    ...
    toHaveBeenCalledWith: (...args: any[]) => {
      ...
      if (!match) {
        throw new Error(
          `Expected function to have been called with ${JSON.stringify(args)} but was called with ${JSON.stringify(call)}`
        )
      }
    },
  }
