...
const setupMocks = () => {
  ...
  mockSupabaseEq = mockApi
    .create()
    .mockReturnValue({ single: mockSupabaseSingle })
  ...
  const insertSelectMock = mockApi
    .create()
    .mockReturnValue({ single: insertSelectSingleMock })
  mockSupabaseInsert = mockApi
    .create()
  ...
  Object.defineProperty(supabaseClient, 'supabase', {
    value: { from: mockSupabaseFrom },
    configurable: true,
  })
  allMocks.push(
    mockSupabaseSingle,
...
export async function testCreateUser_SuccessNew(): Promise<TestResult> {
  ...
  try {
    ...
    assert.deepEqual(
      result,
      newUserDataResponse,
      `${testName} - should return new user data array`
    )
    assert.isTrue(
      mockSupabaseFrom.mock.calls.length >= 2,
      `${testName} - supabase.from called check and insert`
    )
    assert.isTrue(
      mockSupabaseInsert.mock.calls.length === 1,
      `${testName} - insert called once`
    )
    assert.deepEqual(
      mockSupabaseInsert.mock.calls[0][0][0],
      {
        ...testUserData,
        chat_id: '123456789',
        mode: 'clean',
    ...
    assert.isTrue(
      mockLoggerInfo.mock.calls.length > 0,
      `${testName} - logger.info called`
    )
    assert.contains(
      mockLoggerInfo.mock.calls[mockLoggerInfo.mock.calls.length - 1][0],
      'Пользователь успешно создан',
      `${testName} - success log`
    )
...
export async function testCreateUser_ExistingUser(): Promise<TestResult> {
  ...
  try {
    ...
    assert.deepEqual(
      result,
      existingUserData,
      `${testName} - should return existing user data`
    )
    assert.isTrue(
      mockSupabaseFrom.mock.calls.length === 1,
      `${testName} - supabase.from called once`
    )
    assert.deepEqual(
      mockSupabaseFrom.mock.calls[0],
      ['users'],
      `${testName} - from args check`
    )
    assert.equal(
      mockSupabaseInsert.mock.calls.length,
      0,
      `${testName} - insert should not be called`
    )
    assert.isTrue(
      mockLoggerInfo.mock.calls.length > 0,
      `${testName} - logger.info called`
...
export async function testCreateUser_CheckError(): Promise<TestResult> {
  ...
  try {
    ...
    assert.ok(
      ...
      `${testName} - thrown error should be an Error instance`
    )
    assert.equal(
      thrownError.message,
      checkError.message,
      `${testName} - error message should match`
    )

    assert.isTrue(
      mockLoggerError.mock.calls.length > 0,
      `${testName} - logger.error called`
    )
    assert.equal(
      mockLoggerError.mock.calls[0][0],
      '❌ Ошибка при проверке существующего пользователя:',
      `${testName} - error log message`
    )
...
export async function testCreateUser_InsertError(): Promise<TestResult> {
  ...
  try {
    ...
    assert.strictEqual(
      thrownError,
      insertError,
      `${testName} - thrown error should be the exact error object`
    )
...
export async function runCreateUserTests(
