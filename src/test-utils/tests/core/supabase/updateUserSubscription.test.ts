...
export async function testUpdateUserSubscription_Success(): Promise<TestResult> {
  ...
  try {
    ...
    assert.isTrue(
      mockSupabaseUpdate.mock.calls.length === 1,
      `${testName} - update called once`
    )
    assert.deepEqual(
      mockSupabaseUpdate.mock.calls[0][0],
      { subscription: newSubscription },
      `${testName} - update data`
    )
    assert.isTrue(
      mockSupabaseEq.mock.calls.length === 1,
      `${testName} - eq called once`
    )
    assert.deepEqual(
      mockSupabaseEq.mock.calls[0],
...
testUpdateUserSubscription_SupabaseError.meta = {
  category: TestCategory.Database,
} // Add meta

// --- Test Runner Function ---

export async function runUpdateUserSubscriptionTests(
