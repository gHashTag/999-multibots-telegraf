...
const MCP_SERVER_URL =
  ...
  `ws://localhost:${process.env.MOCK_SERVER_PORT || '8888'}`
...
const processUserInput = async (input: string): Promise<void> => {
  try {
    ...
    if (input.startsWith('/')) {
      switch (input) {
        case '/start':
          addToHistory(
            'assistant',
            '👋 Привет! Я автономный агент-разработчик. Пообщайтесь со мной, чтобы я мог улучшить свои возможности.'
          )
          return
        case '/help':
          addToHistory(
            'assistant',
            '🔍 Я могу анализировать запросы на самосовершенствование и создавать файлы. Попробуйте:\n1. "Научись работать с базами данных"\n2. "Создай файл utils/logger.ts с классом для логирования"'
          )
          return
        case '/improve':
          addToHistory(
        ...
        case '/check_tasks':
          ...
          const backgroundTasks = allTasks.filter(
            (task: Task) => task.type === TaskType.BACKGROUND_IMPROVEMENT
          )
        ...
        case '/status':
          addToHistory(
            'assistant',
            `📊 Статус агента: ${telegramState.initialized ? 'активен' : 'не инициализирован'}`
          )
          return
        case '/history':
          const historyText = telegramState.chatHistory
            .map(
              item =>
                `[${item.timestamp.toLocaleTimeString()}] ${item.role === 'user' ? '👤' : '🤖'} ${item.message}`
            )
            .join('\n')
          console.log(`\n📜 История чата:\n${historyText}`)
          return
        ...
        case '/set_commands':
          ...
          const commandsList = [
            ...
            {
              command: 'background',
              description: 'Запустить фоновое улучшение',
            },
            {
              command: 'check_tasks',
              description: 'Проверить статус фоновых задач',
            },
    ...
    if (isBackground) {
      ...
      try {
        ...
        const backgroundTask =
          await telegramState.agent.startBackgroundImprovement(
            input,
            'test-user-id'
          )
      ...
      if (isFileCreation) {
        ...
        addToHistory(
          'assistant',
          '📝 Понял, вы хотите создать новый файл. Работаю над этим...'
        )
      } else {
    ...
    if (
      isFileCreation &&
      dialogResult.result &&
      dialogResult.result.createdFiles
    ) {
...
const runTelegramTest = async (): Promise<void> => {
  try {
    console.log(
      '🧪 Запуск тестирования самосовершенствования и создания файлов через эмуляцию Telegram'
    )
    ...
    console.log('\n💡 Примеры запросов:')
    console.log(
      '   • Создай файл src/utils/logger.ts с классом для логирования'
    )
