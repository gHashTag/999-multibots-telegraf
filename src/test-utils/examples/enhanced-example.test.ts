...
class CalculatorTests {
  ...
  async testDivisionByZero() {
    ...
    expect.toThrow(() => this.calculator.divide(5, 0), Error, 'Division by zero');
  }
  ...
  async testSpyOnMethods() {
    ...
    expect.toBeTrue(spy.calledWith(5, 3), 'Method was not called with correct arguments');
    expect.toEqual(spy.callCount, 1, 'Method was called incorrect number of times');
  }
  ...
  async testMockMethod() {
    ...
    expect.toEqual(this.calculator.add(5, 3), 8, 'Original method was not restored');
  }
...
if (require.main === module) {
  import('../core/TestDiscovery').then(async ({ TestDiscovery }) => {
    ...
    logger.info(`📊 Found ${suites.length} test suites with ${suites.reduce((sum, suite) => sum + suite.tests.length, 0)} tests`);
