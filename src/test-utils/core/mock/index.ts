...
export { mockApi, mockFn, createMockContext, setupTestEnvironment }

// Re-export types
export type { TestResult, TestConfig, TestCase, TestSuite }
...
export function createMock<T extends (...args: any[]) => any>(
  ...
  const mockFn = function (...args: any[]): any {
    ...
    try {
      ...
      if (options.implementation) {
        ...
        call.threw = true
        call.error =
          options.throwError instanceof Error
            ? options.throwError
  ...
  mockFn.verify = () => {
    ...
    if (options.callCount !== undefined && calls.length !== options.callCount) {
      logger.error(
        `Ожидалось вызовов: ${options.callCount}, получено: ${calls.length}`
      )
      return false
    }
    ...
    if (options.expectedArgs) {
      if (calls.length !== options.expectedArgs.length) {
        logger.error(
          `Ожидалось вызовов: ${options.expectedArgs.length}, получено: ${calls.length}`
        )
        return false
      }
      ...
      for (let i = 0; i < calls.length; i++) {
        ...
        if (expectedArgs.length !== actualArgs.length) {
          logger.error(
            `Вызов ${i + 1}: ожидалось аргументов: ${expectedArgs.length}, получено: ${actualArgs.length}`
          )
          return false
        }
        ...
        for (let j = 0; j < expectedArgs.length; j++) {
          ...
          if (JSON.stringify(expectedArg) !== JSON.stringify(actualArg)) {
            logger.error(
              `Вызов ${i + 1}, аргумент ${j + 1}: ожидалось: ${JSON.stringify(expectedArg)}, получено: ${JSON.stringify(actualArg)}`
            )
            return false
          }
        }
...
export function mockObject<T extends object>(
  ...
  const methodsToMock =
    ...
    Object.getOwnPropertyNames(object).filter(name => {
      return (
        typeof object[name as keyof T] === 'function' && name !== 'constructor'
      )
    })
  ...
  for (const methodName of methodsToMock) {
    if (typeof object[methodName as keyof T] === 'function') {
      mocks[methodName as string] = mockMethod(
        object,
        methodName as keyof T & string,
        options
      )
    }
  }
