...
export class TestContext {
   ...
   */
  createStub(objectName: string = 'stub', methods: string[] = []): Record<string, MockFunction> {
   ...
   */
  tryCatch<T>(callback: () => T, errorHandler?: (error: any) => T): T | undefined {
    try {
      return callback();
    } catch (error: any) {
      if (errorHandler) {
   ...
   */
  spy<T extends object, K extends keyof T>(obj: T, methodName: K): MockFunction {
...
export const expect = {
   ...
   */
  toEqual: (actual: any, expected: any, message?: string) => Assert.equal(actual, expected, message),
  
  /**
   * Проверяет неравенство значений
   */
  notToEqual: (actual: any, expected: any, message?: string) => Assert.notEqual(actual, expected, message),
  
  /**
   * Проверяет, что значение не undefined
   */
  toBeDefined: (value: any, message?: string) => Assert.isDefined(value, message),
  
  /**
   * Проверяет, что строка содержит подстроку
   */
  toContain: (str: string, substring: string, message?: string) => Assert.contains(str, substring, message),
  
  /**
   * Проверяет, что массив содержит элемент
   */
  toInclude: <T>(array: T[], element: T, message?: string) => Assert.includes(array, element, message),
  
  /**
   * Проверяет, что функция выбрасывает исключение
   */
  toThrow: (fn: Function, errorType?: any, message?: string) => Assert.throws(fn, errorType, message),
  
  /**
   * Проверяет, что функция не выбрасывает исключение
   */
  notToThrow: (fn: Function, message?: string) => Assert.doesNotThrow(fn, message),
  
  /**
   * Проверяет, что промис выполняется успешно
   */
  toResolve: async (promise: Promise<any>, message?: string) => await Assert.resolves(promise, message),
   ...
   */
  toMatchSnapshot: (name: string, data: any) => snapshot.toMatchSnapshot(name, data),
};
