...
export function TestSuite(metadata: TestSuiteMetadata) {
  return function(constructor: Function) {
    ...
    logger.debug(`Зарегистрирован тестовый набор: ${metadata.name} (${metadata.category})`);
  };
}

// Получение метаданных тестового набора
export function getTestSuiteMetadata(target: any): TestSuiteMetadata | undefined {
...
export function Test(metadata: TestMetadata) {
  return function(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    const testMethods = Reflect.getMetadata(TEST_METHOD_KEY, target.constructor) || [];
    testMethods.push({
    ...
    descriptor.value = async function(...args: any[]) {
      ...
      try {
        ...
        const duration = Date.now() - startTime;
        logger.error(`Тест завершился с ошибкой: ${metadata.name} (${duration}ms)`, error);
        throw error;
      }
    };
...
export function getTestMethods(target: any): Array<{methodName: string, metadata: TestMetadata}> {
