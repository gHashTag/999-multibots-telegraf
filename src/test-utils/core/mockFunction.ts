...
export const mockUtils = {
   ...
   */
  wasCalled: <T extends (...args: any[]) => any>(
   ...
   */
  getCallCount: <T extends (...args: any[]) => any>(
    mock: IMockFunction<T>
  ): number => {
...
export const mockFn = <T extends (...args: any[]) => any>(
  ...
  fn.mockReturnValueOnce = function (
    this: IMockFunction<T>,
    value: ReturnType<T>
  ): IMockFunction<T> {
    ...
    return this.mockImplementation(function (
      this: any,
      ...args: Parameters<T>
    ): ReturnType<T> {
      if (!called) {
      ...
      return (originalImpl || ((() => undefined) as unknown as T)).apply(
        this,
        args
      )
    } as unknown as T)
 ...
 */
export function mockObject<T extends Record<string, (...args: any[]) => any>>(
  obj: T
): { [K in keyof T]: IMockFunction<T[K]> } {
