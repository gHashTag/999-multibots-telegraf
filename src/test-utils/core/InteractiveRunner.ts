...
export class InteractiveRunner {
   ...
   * Выбор дополнительных опций
   */
  private async selectOptions(): Promise<{
    verbose: boolean
    parallel: boolean
  }> {
    const options = {
    ...
    const verboseAnswer = await this.question(
      'Включить подробный вывод? (y/n) [n]: '
    )
    options.verbose = verboseAnswer.toLowerCase() === 'y'

    // Опция параллельного запуска
    const parallelAnswer = await this.question(
      'Запустить тесты параллельно? (y/n) [n]: '
    )
