 ...
 */
export async function resolves<T>(
  promise: Promise<T>,
  message?: string
): Promise<T> {
  try {
    ...
    throw new AssertionError(
      message ||
        `Ожидалось успешное завершение промиса, но было выброшено исключение: ${(error as Error).message}`,
      error,
      'fulfilled promise',
      'resolves'
    )
    ...
    if (expected) {
      if (expected instanceof RegExp) {
        if (!expected.test(actualError.message)) {
          throw new AssertionError(
            message ||
              `Ожидалось исключение с сообщением, соответствующим ${expected}, получено: ${actualError.message}`,
            actualError,
            expected,
            'rejects'
          )
        ...
        if (
          actualError.name !== expected.name ||
          actualError.message !== expected.message
        ) {
          throw new AssertionError(
            message ||
              `Ожидалось исключение ${expected.name}: ${expected.message}, получено: ${actualError.name}: ${actualError.message}`,
            actualError,
            expected,
            'rejects'
          )
        ...
        if (!(actualError instanceof expected)) {
          throw new AssertionError(
            message ||
              `Ожидалось исключение типа ${expected.name}, получено: ${actualError.name}`,
            actualError,
            expected,
            'rejects'
          )
...
export async function eventually(
  ...
  throw new AssertionError(
    message ||
      `Таймаут ${timeout}мс истек, но значение не стало истинным. Последнее значение: ${formatValue(lastValue)}`,
    lastValue,
    true,
    'eventually'
  )
...
export async function completesWithin<T>(
  ...
  try {
    ...
    if (execTime > maxTime) {
      throw new AssertionError(
        message ||
          `Ожидалось выполнение за ${maxTime}мс, но потребовалось ${execTime}мс`,
        execTime,
        maxTime,
        'completesWithin'
      )
    ...
    if (
      ...
      error.operator === 'completesWithin'
    ) {
      throw error
    }
