...
export function verifyInngestEvent(
  ...
  if (!mockFn || !mockFn.mock || mockFn.mock.calls.length === 0) {
    throw new Error(
      `Expected inngest.send to be called with event "${eventName}" but it was not called`
    )
  }

  if (mockFn.mock.calls.length === 0 || !mockFn.mock.calls[0][0]) {
    throw new Error(
      `Expected inngest.send to be called with event "${eventName}" but call data is missing`
    )
  }
  ...
  if (eventData.name !== eventName) {
    throw new Error(
      `Expected event name to be "${eventName}" but got "${eventData.name}"`
    )
  }
  ...
  Object.entries(requiredData).forEach(([key, expectedValue]) => {
    if (!eventData.data || eventData.data[key] !== expectedValue) {
      ...
      throw new Error(
        `Expected event data to have ${key}="${expectedValue}" but got ${key}="${actualValue}"`
      )
    }
  })
...
export function logTestResults(results: any[], groupName: string) {
  ...
  console.log(
    `✅ ${groupName} tests completed: ${successCount} passed, ${failCount} failed`
  )
