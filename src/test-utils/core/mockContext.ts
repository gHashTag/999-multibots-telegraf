...
global.fetch = mockFn<typeof fetch>().mockImplementation(
  async () =>
    ({
      ok: true,
      status: 200,
...
export interface MockContext extends Partial<Context> {
  ...
  editMessageText: IMockFunction<
    (
      text: string,
      extra?: any
    ) => Promise<(Message.TextMessage & { edit_date: number }) | true>
  >
...
export function createMockContext(
  ...
  const defaultContext: MockContext = {
    ...
    scene: {
      enter: mockFn<
        (sceneId: string, state?: any) => Promise<unknown>
      >().mockImplementation(async () => Promise.resolve(undefined)),
      reenter: mockFn<() => Promise<unknown>>().mockImplementation(async () =>
    ...
    wizard: {
      next: mockFn<() => WizardContext>().mockImplementation(
        () => ({}) as WizardContext
      ),
      back: mockFn<() => WizardContext>().mockImplementation(
        () => ({}) as WizardContext
      ),
      selectStep: mockFn<(step: number) => WizardContext>().mockImplementation(
        () => ({}) as WizardContext
      ),
    },
