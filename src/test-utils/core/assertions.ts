...
export function assertReplyContains(ctx: any, expectedText: string): void {
  ...
  if (!containsText) {
    throw new Error(
      `Ожидаемый текст "${expectedText}" не найден в ответах бота. Ответы: ${JSON.stringify(replies)}`
    )
  }
}
...
export function assertReplyMarkupContains(ctx: any, buttonText: string): void {
  ...
  if (!containsButton) {
    throw new Error(
      `Кнопка с текстом "${buttonText}" не найдена в разметке клавиатуры. Ответы: ${JSON.stringify(replies)}`
    )
  }
}
...
export function assertScene(ctx: any, sceneName: string, step: number): void {
  ...
  if (sceneName && ctx.wizard.scene && ctx.wizard.scene.current !== sceneName) {
    throw new Error(
      `Текущая сцена "${ctx.wizard.scene.current}" не соответствует ожидаемой "${sceneName}"`
    )
  }

  if (step !== undefined && ctx.wizard.cursor !== step) {
    throw new Error(
      `Текущий шаг сцены ${ctx.wizard.cursor} не соответствует ожидаемому ${step}`
    )
  }
}
...
export function assertContains(actual: any, expected: any): void {
  if (typeof actual === 'string' && typeof expected === 'string') {
    if (!actual.includes(expected)) {
      throw new Error(
        `Значение "${actual}" не содержит ожидаемую подстроку "${expected}"`
      )
    }
    return
  }
  ...
  if (Array.isArray(actual) && typeof expected === 'number') {
    if (actual.length !== expected) {
      throw new Error(
        `Длина массива ${actual.length} не равна ожидаемой ${expected}`
      )
    }
    return
  }
  ...
  if (!actualStr.includes(expectedStr)) {
    throw new Error(
      `Значение "${actualStr}" не содержит ожидаемое значение "${expectedStr}"`
    )
  }
}
...
export function assertSessionContains(
  ...
  for (const [key, value] of Object.entries(expectedProps)) {
    ...
    if (ctx.session[key] !== value) {
      throw new Error(
        `Значение свойства "${key}" в сессии (${ctx.session[key]}) не соответствует ожидаемому (${value})`
      )
    }
  }
}
