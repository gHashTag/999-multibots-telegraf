...
export function assertReplyContains(ctx: Context<Update>, text: string) {
  ...
  if (!found) {
    throw new Error(`Expected reply to contain "${text}" but got:\n${JSON.stringify(replies, null, 2)}`);
  }
}
...
export function assertReplyKeyboard(ctx: Context<Update>, buttons: string[][]) {
  const replies = (ctx as any).replies || [];
  const keyboard = replies.find((reply: any) => 
    reply.reply_markup && 
    reply.reply_markup.keyboard
  );
  
  if (!keyboard) {
    throw new Error(`Expected reply to contain a keyboard but got:\n${JSON.stringify(replies, null, 2)}`);
  }
  ...
  buttons.forEach((buttonRow, rowIndex) => {
    if (rowIndex >= replyKeyboard.length) {
      throw new Error(`Expected keyboard to have at least ${rowIndex + 1} rows but got ${replyKeyboard.length}`);
    }
    ...
    buttonRow.forEach((buttonText, buttonIndex) => {
      ...
      if (buttonIndex >= actualRow.length) {
        throw new Error(`Expected row ${rowIndex} to have at least ${buttonIndex + 1} buttons but got ${actualRow.length}`);
      }
      
      const actualButton = actualRow[buttonIndex];
      const actualButtonText = typeof actualButton === 'string' ? actualButton : actualButton.text;
      
      if (actualButtonText !== buttonText) {
        throw new Error(`Expected button at row ${rowIndex}, position ${buttonIndex} to be "${buttonText}" but got "${actualButtonText}"`);
      }
    });
  });
}
 ...
 */
export function assertReplyInlineKeyboard(ctx: Context<Update>, buttons: string[][]) {
  const replies = (ctx as any).replies || [];
  const keyboard = replies.find((reply: any) => 
    reply.reply_markup && 
  ...
  if (!keyboard) {
    throw new Error(`Expected reply to contain an inline keyboard but got:\n${JSON.stringify(replies, null, 2)}`);
  }
  ...
  buttons.forEach((buttonRow, rowIndex) => {
    if (rowIndex >= inlineKeyboard.length) {
      throw new Error(`Expected inline keyboard to have at least ${rowIndex + 1} rows but got ${inlineKeyboard.length}`);
    }
    ...
    buttonRow.forEach((buttonText, buttonIndex) => {
      ...
      if (actualButton.text !== buttonText) {
        throw new Error(`Expected button at row ${rowIndex}, position ${buttonIndex} to be "${buttonText}" but got "${actualButton.text}"`);
      }
    });
  });
}
  ...
  if (expectedArgs) {
    ...
    expectedArgs.forEach((arg, index) => {
      if (JSON.stringify(lastCallArgs[index]) !== JSON.stringify(arg)) {
        throw new Error(`Expected argument ${index} to be ${JSON.stringify(arg)} but got ${JSON.stringify(lastCallArgs[index])}`);
      }
    });
  }
}
