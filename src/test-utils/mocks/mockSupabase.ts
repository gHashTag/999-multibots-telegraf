...
class MockStorage {
  ...
  private balanceNotificationSettings: Record<
    string,
  ...
  async createUser(userData: Partial<MockUser>): Promise<MockUser> {
    ...
    const user: MockUser = {
      ...
      telegram_id:
        userData.telegram_id || `${Math.floor(Math.random() * 1000000000)}`,
  ...
  async createOperation(
    operationData: Partial<MockOperation>
  ): Promise<MockOperation> {
    const now = new Date().toISOString()
    const operation: MockOperation = {
      id:
        operationData.id || `op_${Math.random().toString(36).substring(2, 11)}`,
      user_id: operationData.user_id || this.users[0]?.id || 'user_default',
      telegram_id:
        operationData.telegram_id || this.users[0]?.telegram_id || '123456789',
  ...
  async updateOperation(
    id: string,
    updates: Partial<MockOperation>
  ): Promise<MockOperation | null> {
  ...
  async getUserBalanceNotificationSettings(
    telegramId: string
  ): Promise<{ enabled: boolean; threshold: number }> {
    return (
      this.balanceNotificationSettings[telegramId] || {
        enabled: true,
        threshold: 50,
      }
    )
...
export const mockSupabase = {
  ...
  getUserBalanceNotificationSettings:
    mockStorage.getUserBalanceNotificationSettings.bind(mockStorage),
  updateUserBalanceNotificationSettings:
    mockStorage.updateUserBalanceNotificationSettings.bind(mockStorage),
