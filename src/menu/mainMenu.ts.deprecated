...
export async function mainMenu({
  ...
  const filterServiceLevels = (lvl: Level) =>
    ...
    lvl !== levels[108] &&  // ✅ ИСКЛЮЧАЕМ транскрибацию Reels (отдельная кнопка)
    lvl !== levels[109] &&  // ✅ Мониторинг конкурентов (только админы, добавляется отдельно)
    lvl !== levels[110]     // ✅ ИИ Рилс (только админы)
