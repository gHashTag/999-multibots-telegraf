...
async function getBotOwnerIds(botName: string): Promise<string[]> {
  try {
    ...
    return data.map(owner => owner.telegram_id?.toString()).filter((id): id is string => !!id);
  } catch (error) {
...
export async function notifyAdminsAboutPayment(
  ...
  const { telegram_id, bot_name, username, amount, stars, subscription, currency } = details;
  ...
  try {
    ...
    if (pulseBot && pulseBot.telegram && pulseBot.telegram.sendMessage) {
      ...
      logger.info(`✅ Уведомление отправлено в ${pulseGroupId}`, { telegram_id });
    } else {
        logger.warn(`[notifyAdminsAboutPayment] pulseBot не инициализирован, пропуск отправки в ${pulseGroupId}.`);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!botData?.bot) {
      logger.warn(`[notifyAdminsAboutPayment] Бот ${bot_name} не найден, уведомления владельцам/группе не отправлены.`);
    ...
    if (owners.length > 0) {
      for (const ownerId of owners) {
        try {
          await bot.telegram.sendMessage(ownerId, `🔔 Уведомление\n\n${message}`);
          successfulOwnerNotifications++;
        } catch (error: any) { 
           logger.error('❌ Ошибка при отправке уведомления владельцу', { error: error?.message || error, ownerId, botName: bot_name });
        }
      }
       ...
       logger.warn('⚠️ Не найдены владельцы бота для уведомлений', { botName: bot_name });
    }
    ...
    if (groupId) {
      try {
        ...
        logger.info(`✅ Уведомление отправлено в группу бота ${groupId}`, { botName: bot_name });
      } catch (error: any) {
          logger.error('❌ Ошибка при отправке уведомления в группу бота', { error: error?.message || error, groupId, botName: bot_name });
      }
    } else {
       logger.warn('⚠️ groupId не настроен для бота, пропускаем отправку в группу', { botName: bot_name });
    }

  } catch (error) {
     logger.error('❌ Критическая ошибка при отправке уведомлений админам/группе', { error, botName: bot_name });
  }
} 