...
export async function notifyUserAboutSuccess(
  ...
  const { telegram_id, bot_name, language_code = 'ru', amount, stars, description, currentBalance, newBalance, operationId, type } = details
  ...
  if (description.toLowerCase().includes('пополнение') || description.toLowerCase().includes('balance top-up')) {
    message = isRu
  ...
  } else if (description.toLowerCase().includes('покупка') || description.toLowerCase().includes('purchase')) {
    message = isRu
  ...
  } else if (type === TransactionType.MONEY_EXPENSE && currentBalance !== undefined && newBalance !== undefined) {
  ...
  try {
    await botData.bot.telegram.sendMessage(telegram_id.toString(), message, { parse_mode: 'Markdown' })
    logger.info('✅ Уведомление об успехе отправлено пользователю', { telegram_id, description })
  } catch (error) {
...
export async function notifyUserAboutFailure(
  ...
  const { telegram_id, bot_name, language_code = 'ru', error, attemptedAction } = details
  const botData = await createBotByName(bot_name)
  if (!botData?.bot) {
...
Please try again or contact support /tech.`
  ...
  try {
    await botData.bot.telegram.sendMessage(telegram_id.toString(), message, { parse_mode: 'Markdown' })
    logger.info('✅ Уведомление об ошибке отправлено пользователю', { telegram_id, error: errorMessage })
  } catch (sendError) {
 ...
 */
export async function sendTransactionNotificationTest(params: any): Promise<{ success: boolean }> {
