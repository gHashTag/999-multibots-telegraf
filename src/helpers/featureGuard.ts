...
export async function checkFeatureAccess(
  ...
  try {
    ...
    // ОТКАЗ ПРИ ОШИБКЕ — по решению владельца продукта.
    //
    // Раньше здесь стояло `return true` с комментарием «чтобы не блокировать
    // пользователя». Цена такого выбора: если проверка баланса падает —
    // например база недоступна, — платная функция становится бесплатной для
    // ВСЕХ на время сбоя, и узнать об этом можно только по счетам провайдеров.
    //
    // Выбор между двумя способами ошибиться сделан владельцем: блокировать
    // платящих во время аварии. Это видно сразу и чинится восстановлением
    // сервиса, тогда как раздача платного бесплатно тихо стоит денег.
    //
    // Пользователю сообщается причина: отказ без объяснения неотличим от
    // отказа по недостатку средств, а это разные вещи.
    try {
      await ctx.reply(
        isRu
          ? '⚠️ Не удалось проверить баланс — сервис временно недоступен. Попробуйте через несколько минут.'
          : '⚠️ Could not verify your balance — the service is temporarily unavailable. Please try again in a few minutes.'
      )
    } catch {
      // Не смогли написать — не повод менять решение об отказе.
    }
    return false
  }
}
