 ...
 */
/**
 * ПОЧЕМУ ЗДЕСЬ 13 РЕЖИМОВ ИЗ 82 — разобрано, а не забыто.
 *
 * `checkFeatureAccess` возвращает доступ, если режима здесь нет
 * (`featureGuard.ts`, ветка `if (!info) return true`). Выглядит как дыра на
 * 69 режимов, но измерения говорят иначе.
 *
 * Из 69 неописанных реально используются 64. Из них 46 лежат в файлах, где
 * вообще нет списания — Balance, ChangeLanguageScene, BalanceScene и подобные
 * служебные экраны. Остаются 18 кандидатов.
 *
 * Два формальных признака по этим 18 ПРОТИВОРЕЧАТ друг другу: наличие в
 * `src/price/` даёт «цену» у PaymentScene и StartScene (очевидно бесплатных),
 * и не даёт у MusicGeneration и VideoTranscription (звучащих платными). Ни
 * один из признаков не авторитетен.
 *
 * Ответ дал третий источник — за что люди РЕАЛЬНО платили
 * (`payments_v2.service_type`, только MONEY_OUTCOME):
 *
 *   neuro_photo 8594, image_to_video 1500, text_to_image 1055,
 *   image_to_prompt 621, text_to_video 204, digital_avatar_body 126,
 *   lip_sync 59, text_to_speech 36, instagram_parser 28
 *
 * Всё остальное из 18 — AICover, MusicGeneration, VideoTranscription,
 * VoiceTraining, NeuroAudio, DigitalAvatarBodyV2 — не списало НИ РАЗУ. Значит
 * это либо неиспользуемые режимы, либо бесплатные, и добавлять их сюда
 * означало бы закрыть шлюзом то, за что никто не платит.
 *
 * Единственный кандидат с подтверждёнными платежами и без записи здесь —
 * instagram_parser (28 платежей). Его стоит добавить, но сначала выяснить,
 * какой ModeEnum ему соответствует: имён вида InstagramParserWizard в enum
 * несколько, и ошибиться значит закрыть не тот экран.
 *
 * Правило, из которого это вышло: когда два признака расходятся, не выбирай
 * между ними — найди третий, отражающий поведение, а не намерение.
 */
