...
export class ZOTValidator {
  ...
  public async validateBotFinancials(
    ...
    try {
      ...
      const processingContext: ZOTProcessingContext = {
        ...
        options: {
          enableStrictValidation:
            this.config.validationSettings?.enableStrictMode || true,
          enableAutoCorrection:
            this.config.validationSettings?.enableAutoCorrection || false,
      ...
      const preValidationResult = this.performPreValidation(
        transactions,
        processingContext
      )
      if (
        !preValidationResult.isValid &&
        this.config.validationSettings?.enableStrictMode
      ) {
      ...
      const classificationResults =
        this.classifier.classifyTransactions(transactions)
      processingContext.statistics.recordsProcessed = transactions.length

      // Step 3: Aggregate financial data
      const aggregation = this.aggregateFinancialData(
        classificationResults,
        botName
      )

      // Step 4: Validate aggregated data
      const aggregationValidation = this.validateAggregation(
        aggregation,
        processingContext
      )

      // Step 5: Validate monthly data consistency
      const monthlyValidation = this.validateMonthlyConsistency(
        aggregation.monthlyBreakdown
      )

      // Step 6: Detect anomalies
      const anomalyValidation = this.detectAnomalies(
        aggregation,
        classificationResults
      )
      ...
      return {
        ...
        qualityMetrics,
        missingData: this.detectMissingFinancialData(
          aggregation,
          classificationResults
        ),
  ...
  private aggregateFinancialData(
      ...
      if (result.serviceCategory !== ZOTServiceCategory.UNKNOWN_SERVICE) {
        ...
        service.avgCost =
          service.transactions > 0
            ? service.totalStars / service.transactions
            : 0
      }
    })
    ...
    aggregation.virtualMargin =
      aggregation.virtualIncome - aggregation.virtualExpenses
    aggregation.avgTransactionValue =
      aggregation.totalTransactions > 0
        ? (aggregation.realIncome + aggregation.virtualIncome) /
          aggregation.totalTransactions
        : 0

    // Generate monthly breakdown
    aggregation.monthlyBreakdown =
      this.generateMonthlyBreakdown(classifiedResults)
   ...
   */
  private initializeServiceBreakdown(): Record<
    ZOTServiceCategory,
    {
      transactions: number
      totalStars: number
  ...
  private initializeMonthlyServiceBreakdown(): Record<
    ZOTServiceCategory,
    {
      transactions: number
      totalStars: number
  ...
  private generateMonthlyBreakdown(
    ...
    monthlyMap.forEach(monthly => {
      ...
      monthly.avgTransactionValue =
        monthly.transactionCount > 0
          ? (monthly.realIncome + monthly.virtualIncome) /
            monthly.transactionCount
          : 0
  ...
  private validateAggregation(
    ...
    const unknownServiceTransactions =
      aggregation.serviceBreakdown[ZOTServiceCategory.UNKNOWN_SERVICE]
        .transactions
   ...
   */
  private validateMonthlyConsistency(monthlyData: ZOTMonthlyFinancials[]): {
    ...
    for (let i = 1; i < monthlyData.length; i++) {
      ...
      const incomeChange =
        prev.realIncome > 0
          ? Math.abs(curr.realIncome - prev.realIncome) / prev.realIncome
          : 0
  ...
  private detectAnomalies(
    ...
    const amounts = classifiedResults
      .map(r => r.originalData.amount || 0)
      .filter(a => a > 0)
    const stars = classifiedResults
      .map(r => r.originalData.stars || 0)
      .filter(s => s > 0)
    ...
    const lowConfidenceResults = classifiedResults.filter(
      r => r.confidence < 70
    )
  ...
  private calculateQualityMetrics(
    ...
    const completeness =
      classifiedResults.length > 0
        ? completenessScore / classifiedResults.length
        : 0

    // Accuracy: Classification confidence
    const totalConfidence = classifiedResults.reduce(
      (sum, r) => sum + r.confidence,
      0
    )
    const accuracy =
      classifiedResults.length > 0
        ? totalConfidence / classifiedResults.length
        : 0

    // Consistency: Similar transactions classified similarly
    const consistency =
      this.calculateClassificationConsistency(classifiedResults)
    ...
    // Overall score
    const overallScore =
      completeness * 0.3 + accuracy * 0.3 + consistency * 0.2 + timeliness * 0.2
    ...
    return {
      ...
      recordsWithIssues: classifiedResults.filter(
   ...
   */
  private calculateClassificationConsistency(
    classifiedResults: ZOTClassificationResult[]
  ): number {
  ...
  private calculateDataTimeliness(
    ...
    return classifiedResults.length > 0
      ? timelinessScore / classifiedResults.length
      : 100
  }
  ...
  private generateValidationSuggestions(
    ...
    if (errorCodes.includes('LOW_CLASSIFICATION_CONFIDENCE')) {
      suggestions.push(
        'Review transaction descriptions and add more classification rules'
      )
    }
    ...
    if (aggregation.totalTransactions < 10) {
      suggestions.push(
        'Collect more transaction data for better validation accuracy'
      )
    }
  ...
  private detectMissingFinancialData(
    ...
    const noServiceType = classifiedResults.filter(
      r => !r.originalData.service_type
    ).length
    if (noServiceType > 0) {
      missing.push({
        type: 'REQUIRED_FIELD',
    ...
    const noMetadata = classifiedResults.filter(
      r =>
        !r.originalData.metadata ||
        Object.keys(r.originalData.metadata).length === 0
    ).length
