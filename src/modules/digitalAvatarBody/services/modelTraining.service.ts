...
export function createModelTrainingService(
  ...
  async function uploadFileAndGetUrl(filePath: string): Promise<string> {
    try {
      ...
      const uploadBaseDir =
        config.CONFIG_UPLOAD_DIR || path.join(process.cwd(), 'uploads')
      ...
      throw new Error(
        `Ошибка при сохранении файла: ${(error as Error).message}`
      )
    }
  }
  ...
  return async function modelTrainingService(
    ...
    logger.info('modelTrainingService called', {
      requestData,
    ...
    if (USE_PLAN_B_DIRECT_CALL) {
      ...
      logger.info({
        message:
          '🚀 Запуск тренировки модели через ПЛАН Б (прямой вызов Replicate)',
        requestData: {
          ...requestData,
          file_path: `${requestData.file_path.substring(0, 20)}...`,
        },
      ...
      try {
        ...
        const modelIdString =
          config.REPLICATE?.TRAINING_MODEL_ID || 'stability-ai/sdxl'
        const modelVersion =
          config.REPLICATE?.TRAINING_MODEL_VERSION ||
        ...
        if (!modelOwner || !modelName) {
          throw new Error(
            `Некорректный формат TRAINING_MODEL_ID: ${modelIdString}. Ожидается 'owner/name'.`
          )
        }
        ...
        if (
          ...
          typeof requestData.model_name !== 'string' ||
          requestData.model_name.trim() === ''
        ) {
        ...
        const replicateDestination =
          `neuroblogger/${requestData.model_name}` as const // Используем as const для более строгого типа
        ...
        logger.info({
          message: 'Данные для Replicate (План Б)',
          input: trainingInput,
          model: `${modelOwner}/${modelName}:${modelVersion}`,
        ...
        if (fs.existsSync(requestData.file_path)) {
          await fs.promises.unlink(requestData.file_path).catch(e =>
            logger.error('Не удалось удалить файл (План Б)', {
        ...
        await ctx.replyWithHTML(
          requestData.is_ru
            ? `🚀 <b>Тренировка модели (План Б - Replicate) запущена!</b> ID: ${prediction.id}. Ожидайте уведомления.`
            : `🚀 <b>Model training (Plan B - Replicate) started!</b> ID: ${prediction.id}. Await notification.`
        )
        ...
        await sendTelegramMessage?.(
          telegramId,
          isRu
            ? `⚠️ Произошла ошибка при запуске обучения (План Б): ${errorMessage}. Если средства были списаны, обратитесь в поддержку для возврата.`
            : `⚠️ An error occurred starting training (Plan B): ${errorMessage}. If funds were debited, please contact support for a refund.`
        )

        if (fs.existsSync(requestData.file_path)) {
          await fs.promises.unlink(requestData.file_path).catch(e =>
            logger.error('Не удалось удалить файл после ошибки Плана Б', {
              error: (e as Error).message,
            })
          )
        }
      ...
      try {
        ...
        const zipUrlForInngest = await uploadFileAndGetUrl(
        ...
        logger.info(
          'Событие для тренировки модели отправлено в Inngest (План А)',
          {
            eventName: 'digital-avatar-body/model-training.requested',
            eventIds,
          }
        )
        ...
        if (fs.existsSync(requestData.file_path)) {
          await fs.promises.unlink(requestData.file_path).catch(e =>
            logger.error('Не удалось удалить файл (План А) после успеха', {
              error: (e as Error).message,
        ...
        return {
          success: true,
          message:
            'Запрос на тренировку модели успешно отправлен в обработку (План А).',
        ...
        await sendTelegramMessage?.(
          telegramId,
          isRu
            ? `⚠️ Произошла ошибка при отправке запроса на обучение (План А): ${errorMessage}. Если средства были списаны, обратитесь в поддержку для возврата.`
            : `⚠️ An error occurred sending training request (Plan A): ${errorMessage}. If funds were debited, please contact support for a refund.`
        )

        if (fs.existsSync(requestData.file_path)) {
          await fs.promises.unlink(requestData.file_path).catch(e =>
            logger.error('Не удалось удалить файл после ошибки Плана А', {
              error: (e as Error).message,
            })
          )
        }
