...
describe('modelTrainingService', () => {
    ...
    it('should return success:false and correct error message if cost is not configured', async () => {
      ...
      expect(mockSendTelegramMessage).toHaveBeenCalledWith(
        ...
        expect.stringContaining(
          'Ошибка конфигурации: не удалось определить стоимость операции'
        )
      )
  ...
  describe('Scenario: File Upload Error (Plan B)', () => {
    ...
    it('should return success:false and specific error if file upload fails in Plan B', async () => {
      ...
      expect(result.details).toContain(
        'Ошибка при сохранении файла: Test copyFileSync error'
      )
      ...
      expect(mockSendTelegramMessage).toHaveBeenCalledWith(
        requestData.telegram_id,
        expect.stringContaining(
          'Произошла ошибка при запуске обучения (План Б): Ошибка при сохранении файла: Test copyFileSync error'
        )
      )
    })
  })
  ...
  describe('Scenario: Successful Plan B', () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(fs, 'existsSync').mockImplementation(p => {
        const permanentDirConstructed = path.join(
          mockConfig.CONFIG_UPLOAD_DIR || '',
          'training_archives'
        )
    ...
    it('should successfully initiate training via Plan B and call Replicate', async () => {
      ...
      expect(result.message).toBe(
        'План Б (Replicate) выполнен, тренировка запущена.'
      )
      ...
      const [expectedModelOwner, expectedModelName] =
        mockConfig.REPLICATE.TRAINING_MODEL_ID.split('/')
        ...
        expect.objectContaining({
          ...
          input: expect.objectContaining({
            input_images: expect.stringContaining(
              path.parse(requestData.file_path).name
            ),
  ...
  describe('Scenario: Replicate Call Error (Plan B)', () => {
    beforeEach(() => {
      ...
      // Мокируем успешную загрузку файла
      vi.spyOn(fs, 'existsSync').mockImplementation(p => {
        if (typeof p === 'string' && p.includes('test_uploads_error_replicate'))
          return false
        return true
      })
      vi.spyOn(fs, 'statSync').mockReturnValue({
      ...
      mockReplicate.trainings.create.mockRejectedValue(
        new Error('Replicate API Error Test')
      )
    })
    ...
    it('should return success:false, specific error message, and notify user if Replicate call fails', async () => {
      ...
      expect(mockSendTelegramMessage).toHaveBeenCalledWith(
        requestData.telegram_id,
        expect.stringContaining(
          'An error occurred starting training (Plan B): Replicate API Error Test'
        )
      )
    ...
    it('should successfully send event to Inngest and notify user', async () => {
      ...
      const expectedEventPayload = {
        ...
        data: {
          ...
          zipUrl: expect.stringContaining(
            `/${requestData.file_path.split('/').pop()?.replace('.zip', '')}_`
          ), // Проверяем, что URL содержит имя файла
      ...
      expect(mockInngest.send).toHaveBeenCalledWith(
        expect.objectContaining(expectedEventPayload)
      )
      ...
      expect(mockSendTelegramMessage).toHaveBeenCalledWith(
        requestData.telegram_id,
        expect.stringContaining(
          `Запрос на тренировку модели "${requestData.model_name}" успешно отправлен`
