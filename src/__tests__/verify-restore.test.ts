...
describe('Verify Restore', () => {
  test('Проверяем полноту переноса данных', async () => {
    ...
    const backupRubSum = (backupMetaMuseRub || []).reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);

    console.log(`\n📊 METAMUSE В BACKUP:`);
    console.log(`   Записей: ${backupMetaMuse}`);
    console.log(`   RUB доходы: ${Math.round(backupRubSum).toLocaleString()}₽ (${backupMetaMuseRub?.length || 0} транз.)`);
    ...
    const mainRubSum = (mainMetaMuseRub || []).reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);

    console.log(`\n📊 METAMUSE В ОСНОВНОЙ:`);
    console.log(`   Записей: ${mainMetaMuse}`);
    console.log(`   RUB доходы: ${Math.round(mainRubSum).toLocaleString()}₽ (${mainMetaMuseRub?.length || 0} транз.)`);

    console.log(`\n📊 РАЗНИЦА:`);
    console.log(`   Записей MetaMuse: ${(mainMetaMuse || 0) - (backupMetaMuse || 0)}`);
    console.log(`   RUB доходы: ${Math.round(mainRubSum - backupRubSum).toLocaleString()}₽`);

    if (mainMetaMuse !== backupMetaMuse || Math.abs(mainRubSum - backupRubSum) > 1) {
