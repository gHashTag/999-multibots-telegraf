...
describe('Compare RUB Income: Backup vs Main', () => {
  test('Сравниваем RUB доходы и ищем недостающие в backup', async () => {
    ...
    try {
      ...
      const rubIncomeMainSum = rubIncomeMain.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
      console.log(`   Найдено: ${rubIncomeMain.length} транзакций, сумма: ${Math.round(rubIncomeMainSum).toLocaleString()}₽`);
      ...
      const rubIncomeBackupSum = rubIncomeBackup.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
      console.log(`   Найдено: ${rubIncomeBackup.length} транзакций, сумма: ${Math.round(rubIncomeBackupSum).toLocaleString()}₽`);
      ...
      if (onlyInBackup.length > 0) {
        const onlyInBackupSum = onlyInBackup.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
        console.log(`Сумма недостающих: ${Math.round(onlyInBackupSum).toLocaleString()}₽`);
      ...
      if (onlyInMain.length > 0) {
        const onlyInMainSum = onlyInMain.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
      ...
      console.log(`   Сумма: ${Math.round(rubIncomeBackupSum).toLocaleString()}₽`);

      const difference = rubIncomeBackupSum - rubIncomeMainSum;
      console.log(`\nРазница (backup - основная):`);
      console.log(`   Транзакции: ${rubIncomeBackup.length - rubIncomeMain.length > 0 ? '+' : ''}${rubIncomeBackup.length - rubIncomeMain.length}`);
      console.log(`   Сумма: ${Math.round(difference).toLocaleString()}₽ ${difference > 0 ? '⬆️' : difference < 0 ? '⬇️' : '➡️'}`);
