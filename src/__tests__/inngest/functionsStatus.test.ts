...
function run(p: Partial<InngestRunNode> & { hoursAgo: number }): InngestRunNode {
...
function gqlFetch(handlers: {
  ...
  const fetchImpl = vi.fn(async (_url: string, init: any) => {
    ...
    return {
      ...
      status,
      text: async () => JSON.stringify(status < 300 ? { data } : { error: 'nope' }),
    }
  })
...
describe('resolveInngestGqlUrl', () => {
  it('INNGEST_GQL_URL wins; otherwise ${INNGEST_BASE_URL}/v0/gql', () => {
    expect(resolveInngestGqlUrl({ INNGEST_GQL_URL: 'http://x/v0/gql' } as any)).toBe('http://x/v0/gql')
    expect(resolveInngestGqlUrl({ INNGEST_BASE_URL: 'http://inngest.railway.internal:8288/' } as any)).toBe(
      'http://inngest.railway.internal:8288/v0/gql'
    )
...
describe('InngestGraphqlClient', () => {
  ...
  it('posts JSON, passes variables and the optional bearer token', async () => {
    ...
    const c = new InngestGraphqlClient({ url: 'http://gql', fetchImpl, authToken: 'sk' })
  ...
  it('wraps HTTP failures into InngestGraphqlError with url and status', async () => {
    ...
    await expect(c.apps()).rejects.toMatchObject({ url: 'http://gql', httpStatus: 502 })
  })
})
...
describe('summarizeRuns / buildFunctionsStatus', () => {
  it('splits 24h vs 7d counters and keeps newest run + last error', () => {
    ...
    expect(s.runs24h).toMatchObject({ completed: 1, failed: 1, running: 1, total: 3 })
    expect(s.runs7d).toMatchObject({ completed: 2, failed: 2, running: 1, total: 5 })
  ...
  it('payload has the documented shape, filters -failure twins, flags drift', () => {
    ...
    expect(payload.functions.length).toBe(getRegisteredManifestFunctions().length)
    ...
    expect(payload.functions.find(f => f.id === 'reels-ai-generate')?.deployed).toBe(false)
    // `-failure` twins are not functions of their own; ghosts are drift
    expect(payload.unknownInApp).toEqual(['telegram-bot-client-something-not-in-manifest'])
  })
...
describe('fetchFunctionsStatus cache', () => {
  it('serves from cache for 30 s and refetches afterwards', async () => {
    const { fetchImpl } = gqlFetch({ apps: () => [app()], runs: () => [run({ hoursAgo: 1 })] })
    ...
    const p2 = await fetchFunctionsStatus({ ...opts, now: new Date(NOW.getTime() + STATUS_CACHE_TTL_MS - 1) })
    expect(p2.source.cached).toBe(true)
    expect(fetchImpl).toHaveBeenCalledTimes(2)
    await fetchFunctionsStatus({ ...opts, now: new Date(NOW.getTime() + STATUS_CACHE_TTL_MS + 1) })
    expect(fetchImpl).toHaveBeenCalledTimes(4)
  })

  it('fetchFunctionsStatusSafe never throws', async () => {
    const { fetchImpl } = gqlFetch({ status: 500 })
    const r = await fetchFunctionsStatusSafe({ url: 'http://gql', fetchImpl, now: NOW })
...
describe('GET /api/inngest/functions/status handler', () => {
  ...
  it('returns 503 with an error payload when Inngest is unreachable, still no throw', async () => {
    ...
    try {
      ...
      expect(r.body).toMatchObject({ error: 'inngest-unreachable', gqlUrl: 'http://127.0.0.1:9/v0/gql' })
  ...
  it('OPTIONS answers 204 and does not echo unknown origins', () => {
    const r = res()
    handleFunctionsStatusOptions({ headers: { origin: 'https://attacker.example' } }, r)
