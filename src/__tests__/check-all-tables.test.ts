...
describe('All payments_v2 Tables Check', () => {
  test('Проверяем все таблицы и ищем STARS доходы для MetaMuse', async () => {
    ...
    try {
      ...
      const tables = ['payments_v2', 'payments_v2_backup', 'payments_v2_duplicate'];
      ...
      for (const tableName of tables) {
        ...
        try {
          ...
          const { data: metamuseStarsIncome, error: incomeError } = await supabase
            .from(tableName)
            .select('*')
          ...
          if (incomeError) {
            console.log(`   ❌ Ошибка запроса STARS доходов: ${incomeError.message}`);
          } else {
            const starsIncomeSum = metamuseStarsIncome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
            console.log(`   STARS доходы MetaMuse: ${metamuseStarsIncome.length} транз., ${Math.round(starsIncomeSum).toLocaleString()}⭐`);
          }
          ...
          if (rubError) {
            ...
            const rubIncomeSum = metamuseRubIncome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
            console.log(`   RUB доходы MetaMuse: ${metamuseRubIncome.length} транз., ${Math.round(rubIncomeSum).toLocaleString()}₽`);
          }
          ...
          if (outcomeError) {
            console.log(`   ❌ Ошибка запроса STARS расходов: ${outcomeError.message}`);
          } else {
            const starsOutcomeSum = metamuseStarsOutcome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
            console.log(`   STARS расходы MetaMuse: ${metamuseStarsOutcome.length} транз., ${Math.round(starsOutcomeSum).toLocaleString()}⭐`);
          }
          ...
          if (allStarsError) {
            console.log(`   ❌ Ошибка запроса всех STARS доходов: ${allStarsError.message}`);
          } else {
            const allStarsSum = allStarsIncome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
            console.log(`   ВСЕГО STARS доходов (все боты): ${allStarsIncome.length} транз. (показано 10), ${Math.round(allStarsSum).toLocaleString()}⭐`);
            ...
            if (allStarsIncome.length > 0) {
              ...
              Object.entries(byBot).forEach(([bot, sum]) => {
                console.log(`        ${bot}: ${Math.round(sum).toLocaleString()}⭐`);
              });
            }
          }
      ...
      tables.forEach(tableName => {
        ...
        const metamuseStarsIncomeSum = result.metamuseStarsIncome.reduce((sum: number, tx: any) => sum + Math.abs(parseFloat(tx.amount) || 0), 0);
        const metamuseRubIncomeSum = result.metamuseRubIncome.reduce((sum: number, tx: any) => sum + Math.abs(parseFloat(tx.amount) || 0), 0);
        const metamuseStarsOutcomeSum = result.metamuseStarsOutcome.reduce((sum: number, tx: any) => sum + Math.abs(parseFloat(tx.amount) || 0), 0);

        console.log(`\n${tableName}:`);
        console.log(`   Всего записей: ${result.totalCount}`);
        console.log(`   MetaMuse STARS доходы: ${Math.round(metamuseStarsIncomeSum).toLocaleString()}⭐ (${result.metamuseStarsIncome.length} транз.)`);
        console.log(`   MetaMuse RUB доходы: ${Math.round(metamuseRubIncomeSum).toLocaleString()}₽ (${result.metamuseRubIncome.length} транз.)`);
        console.log(`   MetaMuse STARS расходы: ${Math.round(metamuseStarsOutcomeSum).toLocaleString()}⭐ (${result.metamuseStarsOutcome.length} транз.)`);
      });
      ...
      tables.forEach(tableName => {
        ...
        const score = (result.totalCount || 0) + (result.metamuseStarsIncome.length * 1000);

        console.log(`\n${tableName}: Score = ${score}`);
        console.log(`   Записей: ${result.totalCount}`);
        console.log(`   MetaMuse STARS доходы: ${result.metamuseStarsIncome.length} транз.`);
