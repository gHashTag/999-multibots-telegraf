...
describe.skipIf(!HAS_INFISICAL)('Final Correct Numbers', () => {
  test('Считаем правильные три цифры с учётом поля stars', async () => {
    ...
    console.log(
      `Общая сумма STARS: ${Math.round(starsIncomeSum).toLocaleString()}⭐`
    )
    ...
    Object.entries(byCurrency).forEach(([currency, stats]) => {
      console.log(
        `   ${currency}: ${stats.count} транз., ${Math.round(stats.stars).toLocaleString()}⭐, ${Math.round(stats.amount).toLocaleString()} (сумма amount)`
      )
    })
    ...
    console.log(
      `Общая сумма STARS: ${Math.round(starsOutcomeSum).toLocaleString()}⭐`
    )
    ...
    console.log(
      `   Источник: сумма поля amount в MONEY_INCOME с currency = RUB`
    )
    console.log(
