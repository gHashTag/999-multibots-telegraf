...
describe('Create Final Excel After Restore', () => {
  test('Создаём финальный Excel отчёт из восстановленной основной таблицы', async () => {
    ...
    // СВОДКА
    const summarySheet = workbook.addWorksheet('СВОДКА', { views: [{ state: 'frozen', ySplit: 1 }] });
    ...
    summarySheet.addRow({ metric: '💰 ДОХОДЫ В РУБЛЯХ (восстановлено)', count: rubIncome.length, amount: Math.round(rubIncomeSum).toLocaleString(), currency: '₽' });
    summarySheet.addRow({ metric: '⭐ ДОХОДЫ В ЗВЕЗДАХ', count: starsIncome.length, amount: Math.round(starsIncomeSum).toLocaleString(), currency: '⭐' });
    summarySheet.addRow({ metric: '💸 РАСХОДЫ В ЗВЕЗДАХ', count: starsOutcome.length, amount: Math.round(starsOutcomeSum).toLocaleString(), currency: '⭐' });

    // RUB ДОХОДЫ
    const rubSheet = workbook.addWorksheet('RUB_доходы', { views: [{ state: 'frozen', ySplit: 1 }] });
    rubSheet.columns = [
    ...
    rubSheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF70AD47' } };
    ...
    console.log(`1️⃣ РАСХОДЫ (STARS): ${Math.round(starsOutcomeSum).toLocaleString()}⭐`);
    console.log(`2️⃣ ДОХОДЫ В РУБЛЯХ: ${Math.round(rubIncomeSum).toLocaleString()}₽ ✅ ВОССТАНОВЛЕНО!`);
    console.log(`3️⃣ ДОХОДЫ В ЗВЁЗДАХ: ${Math.round(starsIncomeSum).toLocaleString()}⭐`);
    console.log(`\n📊 СТАТИСТИКА:`);
