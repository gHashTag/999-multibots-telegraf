...
describe('Run Restore from Backup', () => {
  test('Запускаем восстановление данных', async () => {
    ...
    try {
      ...
      console.log(`   Разница: ${(backupCount || 0) - (mainCount || 0)} записей`);
      ...
      const rubSum = (metamuseRub || []).reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);

      console.log(`\n📊 METAMUSE_MANIFEST_BOT:`);
      console.log(`   Записей: ${metamuseCount}`);
      console.log(`   RUB доходы: ${Math.round(rubSum).toLocaleString()}₽ (${metamuseRub?.length || 0} транз.)`);
