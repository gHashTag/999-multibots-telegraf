...
describe('Restore from Backup', () => {
  test('Переносим все данные из backup в основную таблицу', async () => {
    ...
    try {
      ...
      const dataToTransfer = backupData.filter(tx => !existingIdSet.has(tx.id));
      ...
      for (let i = 0; i < dataToTransfer.length; i += batchSize) {
        ...
        if (error) {
          ...
          console.log(`   ✅ Перенесено: ${transferredCount}/${dataToTransfer.length}`);
        }
      }
      ...
      console.log(`   MetaMuse_Manifest_bot записей в основной: ${metamuseCount}`);
      console.log(`   MetaMuse RUB доходы: ${rubIncomeSum.toLocaleString()}₽ (${metamuseRubIncome.length} транз.)`);
