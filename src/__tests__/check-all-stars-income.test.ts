...
describe.skipIf(!HAS_INFISICAL)('All STARS Income Analysis', () => {
  test('Проверяем все STARS доходы в БД', async () => {
    ...
    try {
      ...
      if (allStarsIncome.length > 0) {
        const totalSum = allStarsIncome.reduce(
        ...
        Object.entries(byBot).forEach(([bot, txs]) => {
          ...
          console.log(
            `   ${bot}: ${txs.length} транз., ${Math.round(sum).toLocaleString()}⭐`
          )
        })
      } else {
      ...
      // Группируем по типам
      const byType = allMetaMuseStars.reduce(
        (acc, tx) => {
      ...
      // ИТОГОВЫЙ ВЫВОД
      const metaMuseStarsIncome = allStarsIncome.filter(
        tx => tx.bot_name === 'MetaMuse_Manifest_bot'
      )
      const metaMuseStarsIncomeSum = metaMuseStarsIncome.reduce(
        (sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0),
        0
      )

      const metaMuseStarsOutcome = allMetaMuseStars.filter(
        tx => tx.type === 'MONEY_OUTCOME'
      )
      const metaMuseStarsOutcomeSum = metaMuseStarsOutcome.reduce(
        (sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0),
        0
      )
      ...
      console.log('='.repeat(80))
      console.log(
        `1️⃣ РАСХОДЫ (STARS): ${Math.round(metaMuseStarsOutcomeSum).toLocaleString()}⭐`
      )
      console.log(`2️⃣ ДОХОДЫ В РУБЛЯХ: нужно проверить отдельно`)
      console.log(
        `3️⃣ ДОХОДЫ В ЗВЁЗДАХ: ${Math.round(metaMuseStarsIncomeSum).toLocaleString()}⭐`
      )
