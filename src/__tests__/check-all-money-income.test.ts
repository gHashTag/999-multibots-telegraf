...
describe('Check All Money Income', () => {
  test('Проверяем ВСЕ Money Income записи MetaMuse', async () => {
    ...
    Object.entries(byCurrency).forEach(([currency, txs]) => {
      ...
      console.log(`   ${currency}: ${txs.length} транз., сумма: ${Math.round(sum).toLocaleString()}`);
    });
    ...
    if (byCurrency['RUB']) {
      ...
      const sum = byCurrency['RUB'].reduce((s, tx) => s + Math.abs(parseFloat(tx.amount as any) || 0), 0);
      console.log(`Сумма: ${Math.round(sum).toLocaleString()}₽`);
    }

    // Проверяем статусы
    console.log('\n\n📊 По статусам:');
    const byStatus = allMoneyIncome.reduce((acc, tx) => {
    ...
    console.log(`Другие валюты: ${Object.keys(byCurrency).filter(c => c !== 'RUB' && c !== 'STARS').length}`);
