...
describe.skipIf(!HAS_INFISICAL)('MetaMuse Final Report from Backup', () => {
  test('Создаем финальный отчет из backup таблицы', async () => {
    ...
    try {
      ...
      rubIncome.slice(0, 15).forEach((tx, i) => {
        ...
        console.log(
          `${(i + 1).toString().padStart(2, ' ')}. ${amount.toLocaleString()}₽ | ${dateStr} | User: ${tx.telegram_id}`
        )
      })
      ...
      starsOutcome.slice(0, 10).forEach((tx, i) => {
        ...
        console.log(
          `${i + 1}. ${amount.toLocaleString()}⭐ | ${dateStr} | ${tx.description || 'без описания'}`
        )
      })
