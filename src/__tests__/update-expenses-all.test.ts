...
describe('Update Expenses All', () => {
  test('Находим ВСЕ расходы, включая записи по Генадию', async () => {
    ...
    const byUser = (allOutcome || []).reduce((acc, tx) => {
    ...
    const userStats = Object.entries(byUser)
      .map(([user, txs]) => {
        const starsSum = txs.reduce((s, tx) => s + (parseFloat(tx.stars as any) || 0), 0);
        return { user, count: txs.length, stars: starsSum };
      })
      .sort((a, b) => b.stars - a.stars);

    userStats.slice(0, 10).forEach((stat, i) => {
      console.log(`${i + 1}. User ${stat.user}: ${stat.count} транз., ${Math.round(stat.stars).toLocaleString()}⭐`);
    });

    // Проверяем Геннадия/Генадия
    const gennadyUsers = userStats.filter(s => s.user.includes('352374518') || s.user.includes('Геннад'));
    if (gennadyUsers.length > 0) {
      console.log('\n✅ Найдены записи по Геннадию:');
      gennadyUsers.forEach(stat => {
        console.log(`   User ${stat.user}: ${stat.count} транз., ${Math.round(stat.stars).toLocaleString()}⭐`);
      });
    } else {
      console.log('\n⚠️ Записей по "352374518" не найдено в топ-10, проверяем всех...');
      const user352 = userStats.find(s => s.user === '352374518');
      if (user352) {
        console.log(`   User 352374518: ${user352.count} транз., ${Math.round(user352.stars).toLocaleString()}⭐`);
      }
    }
    ...
    console.log(`Общая сумма STARS: ${Math.round(totalStarsOutcome).toLocaleString()}⭐`);
    ...
    console.log('='.repeat(80));
    console.log(`1️⃣ РАСХОДЫ (STARS): ${Math.round(totalStarsOutcome).toLocaleString()}⭐`);
