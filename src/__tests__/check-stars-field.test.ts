...
describe('Check Stars Field', () => {
  test('Ищем STARS доходы через поле stars', async () => {
    ...
    if (starsIncome.length > 0) {
      ...
      const totalStars = starsIncome.reduce((sum, tx) => sum + (parseFloat(tx.stars as any) || 0), 0);
    ...
    if (xtrWithStars.length > 0) {
      const sum = xtrWithStars.reduce((s, tx) => s + (parseFloat(tx.stars as any) || 0), 0);
    ...
    if (rubWithStars.length > 0) {
      rubWithStars.forEach((tx, i) => {
        console.log(`${i + 1}. ${tx.amount}₽ + ${tx.stars}⭐ | ${tx.payment_method}`);
      });

      const starsSum = rubWithStars.reduce((s, tx) => s + (parseFloat(tx.stars as any) || 0), 0);
      console.log(`Сумма stars в RUB: ${Math.round(starsSum).toLocaleString()}⭐`);
    }
