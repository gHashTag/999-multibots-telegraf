...
// Модуля '@/services/cancelButtonService' не существует: CancelButtonService и
// соседние помощники экспортируются из бочки '@/navigation'
// (см. src/navigation/index.ts → ./services/CancelButtonService).
import { MyContext } from '@/interfaces/telegram-bot.interface'
import { ModeEnum } from '@/interfaces/modes'
import type { MutableCtx } from '../helpers/mutableContext'

// Create mock function
// vi.mock поднимается выше объявлений, поэтому обычная const внутри фабрики
// недоступна («Cannot access before initialization»). vi.hoisted поднимает
// объявление вместе с моком.
...
describe('CancelButtonService', () => {
  ...
  describe('3. Обработка Reply Keyboard - handleCancelButton', () => {
    it('должна обрабатывать русскую "Отмена"', async () => {
      ...
      // Обработчик кнопки отвечает её подписью «Отмена»; текст
      // «Отменено. Возвращаю в главное меню.» использует executeCancel.
      expect(mockContext.reply).toHaveBeenCalledWith('Отмена', {
        reply_markup: { remove_keyboard: true },
      })
      expect(mockContext.scene?.leave).toHaveBeenCalled()
      // Сервис не входит в сцену MainMenu: он покидает текущую и показывает
      // меню через showMainMenu(ctx) — см. executeMainMenu в CancelButtonService.
      expect(mockContext.scene?.leave).toHaveBeenCalled()
    })
  ...
  describe('4. Обработка Reply Keyboard - handleMainMenuButton', () => {
    it('должна обрабатывать русское "Главное меню"', async () => {
      ...
      expect(mockContext.reply).toHaveBeenCalledWith(
        // Обработчик кнопки «Главное меню» показывает САМО меню
        // («🏠 *Главное меню*» с Markdown-разметкой), а строка
        // «Переходим в главное меню.» используется на другом пути.
  ...
  describe('7. Прямые действия executeCancel и executeMainMenu', () => {
    it('executeCancel должна выполнять отмену с дефолтным сообщением', async () => {
      ...
      // Реальный текст обработчика отмены: «Отменено. Возвращаю в главное меню.»
      // ('Отмена' — это подпись КНОПКИ, а не сообщение).
      expect(mockContext.reply).toHaveBeenCalledWith(
    ...
    it('executeMainMenu должна выполнять переход с дефолтным сообщением', async () => {
      ...
      // Сообщения по умолчанию у executeMainMenu НЕТ: текст отправляется
      // только если передан customMessage (`if (customMessage) ctx.reply(...)`).
      // Без аргумента метод просто покидает сцену и показывает меню.
      expect(mockContext.scene?.leave).toHaveBeenCalled()
      // Сервис не входит в сцену MainMenu: он покидает текущую и показывает
      // меню через showMainMenu(ctx) — см. executeMainMenu в CancelButtonService.
      expect(mockContext.scene?.leave).toHaveBeenCalled()
  ...
  describe('10. Edge cases и безопасность', () => {
    ...
    it('не должна падать если scene.leave выбрасывает ошибку', async () => {
      ...
      // Тест назывался «не должна падать», а требовал падения — противоречие.
      // Сервис действительно перехватывает ошибку scene.leave (try/catch с
      // фолбэком в executeMainMenu) и завершает обработку успешно. Это и есть
      // заявленная устойчивость, её и проверяем.
      await expect(
        CancelButtonService.handleCancelButton(mockContext as MyContext)
      ).resolves.toBe(true)
    })
