...
describe('MetaMuse Exact Numbers', () => {
  test('Точные цифры из backup без лимитов', async () => {
    ...
    try {
      ...
      const rubZeroAmount = rubIncomeAll.filter(tx => parseFloat(tx.amount as any) === 0);
      ...
      const starsZeroAmount = starsIncomeAll.filter(tx => parseFloat(tx.amount as any) === 0);
      ...
      console.log(`- STARS транзакций с нулевой суммой: ${starsZeroAmount.length}`);
