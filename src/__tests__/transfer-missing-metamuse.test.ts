...
describe('Transfer Missing MetaMuse', () => {
  test('Переносим недостающие MetaMuse записи из backup', async () => {
    ...
    const mainMetaMuseIdSet = new Set((mainMetaMuseIds || []).map(row => row.id));
    ...
    console.log(`Примеры недостающих ID: ${missingIds.slice(0, 10).join(', ')}...`);
