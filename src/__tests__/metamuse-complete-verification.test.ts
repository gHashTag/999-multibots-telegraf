...
describe('MetaMuse_Manifest_bot Complete Verification', () => {
  test('Полная проверка всех доходов и расходов', async () => {
    ...
    try {
      ...
      if (rubIncome.length > 0) {
        const totalRubSum = rubIncome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
        console.log(`Общая сумма: ${Math.round(totalRubSum).toLocaleString()}₽`);
      ...
      const totalStarsIncomeSum = starsIncome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
      console.log(`Общая сумма: ${Math.round(totalStarsIncomeSum).toLocaleString()}⭐`);
      ...
      const totalStarsOutcomeSum = starsOutcome.reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);
      console.log(`Общая сумма: ${Math.round(totalStarsOutcomeSum).toLocaleString()}⭐`);

      // 4. ИТОГОВЫЕ ЦИФРЫ
      console.log('\n' + '='.repeat(80));
      console.log('💎 ФИНАЛЬНЫЕ ТРИ ЦИФРЫ (LIVE SUPABASE):');
      console.log('='.repeat(80));
      console.log(`1️⃣ РАСХОДЫ (STARS): ${Math.round(totalStarsOutcomeSum).toLocaleString()}⭐`);
      console.log(`2️⃣ ДОХОДЫ В РУБЛЯХ: ${Math.round(rubIncome.reduce((s, tx) => s + Math.abs(parseFloat(tx.amount as any) || 0), 0)).toLocaleString()}₽`);
      console.log(`3️⃣ ДОХОДЫ В ЗВЁЗДАХ: ${Math.round(totalStarsIncomeSum).toLocaleString()}⭐`);
      console.log('='.repeat(80));
