...
describe('Transfer Missing XTR', () => {
  test('Переносим недостающие XTR Money Income записи из backup', async () => {
    ...
    const newSum = (newMainXtrIncome || []).reduce((sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0), 0);

    console.log(`Теперь XTR Money Income в основной: ${newMainXtrIncome?.length || 0} записей, сумма: ${Math.round(newSum).toLocaleString()} XTR`);
