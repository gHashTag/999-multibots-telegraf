...
describe.skipIf(!HAS_INFISICAL)('Final All Data', () => {
  test('Финальные данные по всем ботам', async () => {
    ...
    for (const botName of uniqueBots) {
      ...
      const starsIncomeSum = (incomeData || []).reduce(
      ...
      const starsOutcomeSum = (outcomeData || []).reduce(
        (sum, tx) => sum + (parseFloat(tx.stars as any) || 0),
        0
      )
      ...
      console.log(
        `   Расходы: ${Math.round(starsOutcomeSum).toLocaleString()}⭐`
      )
      console.log(
        `   Доходы в рублях: ${Math.round(rubIncomeSum).toLocaleString()}₽`
      )
      console.log(
        `   Доходы в звёздах: ${Math.round(starsIncomeSum).toLocaleString()}⭐`
      )
    }
    ...
    const totalStarsIncome = (allIncome || []).reduce(
      (sum, tx) => sum + (parseFloat(tx.stars as any) || 0),
      0
    )
    ...
    const totalRubIncome = (allRubIncome || []).reduce(
      (sum, tx) => sum + Math.abs(parseFloat(tx.amount as any) || 0),
      0
    )
    ...
    console.log(
      `Общие расходы: ${Math.round(totalStarsOutcome).toLocaleString()}⭐`
    )
    console.log(
      `Общие доходы в рублях: ${Math.round(totalRubIncome).toLocaleString()}₽`
    )
    console.log(
      `Общие доходы в звёздах: ${Math.round(totalStarsIncome).toLocaleString()}⭐`
    )
