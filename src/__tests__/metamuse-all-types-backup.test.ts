...
describe('MetaMuse All STARS Types in Backup', () => {
  test('Проверяем все типы STARS транзакций в backup', async () => {
    ...
    try {
      ...
      console.log(`✅ Загружено ${allStarsTransactions.length} STARS транзакций\n`);

      // Группируем по типам
      const byType = allStarsTransactions.reduce((acc, tx) => {
      ...
      Object.entries(byType).forEach(([type, txs]) => {
        ...
        if (type.toUpperCase().includes('INCOME') || type.toUpperCase().includes('BONUS') || type.toUpperCase().includes('ADD')) {
      ...
      const possibleIncomeTypes = Object.keys(byType).filter(type =>
      ...
      console.log('🎯 ИТОГ:');
      console.log('='.repeat(80));
      console.log(`Всего STARS транзакций MetaMuse в backup: ${allStarsTransactions.length}`);
      console.log(`Типов транзакций: ${Object.keys(byType).length}`);

      // Проверяем есть ли хоть какие-то положительные суммы
      const positiveTransactions = allStarsTransactions.filter(tx => parseFloat(tx.amount as any) > 0);
      console.log(`Транзакций с положительной суммой: ${positiveTransactions.length}`);
