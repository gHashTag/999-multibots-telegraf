...
describe.skipIf(!HAS_INFISICAL)('Count All Expenses', () => {
  test('Считаем все расходы без загрузки данных', async () => {
    ...
    const sampleSum = (sampleData || []).reduce(
      (sum, tx) => sum + (parseFloat(tx.stars as any) || 0),
      0
    )
    console.log(
      `Сумма STARS (первые 1000): ${Math.round(sampleSum).toLocaleString()}⭐`
    )
    ...
    if (totalCount && totalCount > 1000) {
      ...
      console.log(
        `Среднее на запись: ${Math.round(avgPerRecord * 100) / 100}⭐`
      )
      console.log(
        `Оценочная общая сумма: ${Math.round(estimatedTotal).toLocaleString()}⭐`
      )
    }
    ...
    const moreSum = (moreData || []).reduce(
      (sum, tx) => sum + (parseFloat(tx.stars as any) || 0),
      0
    )
    console.log(
      `Сумма STARS (первые 5000): ${Math.round(moreSum).toLocaleString()}⭐`
    )
    ...
    topUsers.forEach((stat, i) => {
      console.log(
        `${i + 1}. User ${stat.user}: ${Math.round(stat.stars).toLocaleString()}⭐`
      )
    })
    ...
    if (moreData && moreData.length < 5000) {
      console.log(
        `1️⃣ РАСХОДЫ (STARS): ~${Math.round(moreSum).toLocaleString()}⭐`
      )
      console.log(`   Точная цифра (все ${moreData.length} записей)`)
    } else {
      console.log(
        `1️⃣ РАСХОДЫ (STARS): ~${Math.round((moreSum * (totalCount || 0)) / 5000).toLocaleString()}⭐`
      )
