...
describe('Fal.ai Veed Fabric 1.0 Fast - Проверка наценки', () => {
  describe('Централизованная наценка', () => {
    it('должен применять наценку 50% к базовым ценам', () => {
      ...
      const expectedStars480p = Math.floor(
        (expectedPrice480p * duration) / STAR_COST_USD
      )
      const expectedStars720p = Math.floor(
        (expectedPrice720p * duration) / STAR_COST_USD
      )

      // Фактические звезды
      const actualStars480p = calculateLipSyncCostStars(
        'fal_veed_fabric',
        duration,
        '480p'
      )
      const actualStars720p = calculateLipSyncCostStars(
        'fal_veed_fabric',
        duration,
        '720p'
      )
    ...
    it('должен быть дороже чем цены без наценки', () => {
      ...
      // Цены с наценкой
      const priceWithMarkup480p = calculateLipSyncCostStars(
        'fal_veed_fabric',
        duration,
        '480p'
      )
      const priceWithMarkup720p = calculateLipSyncCostStars(
        'fal_veed_fabric',
        duration,
        '720p'
      )
  ...
  describe('Сравнение с другими моделями', () => {
    it('должен быть конкурентоспособным с kie провайдером', () => {
      ...
      const falStars480p = calculateLipSyncCostStars(
        'fal_veed_fabric',
        duration,
        '480p'
      )
  ...
  describe('Математическая проверка наценки', () => {
    ...
    it('должен правильно конвертировать в звезды с наценкой', () => {
      ...
      const actualStars = calculateLipSyncCostStars(
        'fal_veed_fabric',
        duration,
        '480p'
      )
