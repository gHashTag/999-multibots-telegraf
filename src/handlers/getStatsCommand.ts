...
const getStatsCommand = async (ctx: MyContext): Promise<void> => {
  ...
  logger.info('🔍 Начало обработки команды /stats', {
  ...
  logger.info(isUserOwner ? '✅ Доступ разрешен' : '🚫 Доступ запрещен', {
  ...
  try {
    ...
    if (error) {
      logger.error('❌ Ошибка SQL при получении статистики', {
    ...
    if (!stats || stats.length === 0) {
      logger.error('❌ Нет данных статистики', {
