...
export async function handleRestartVideoGeneration(
  ...
  if (lastCompletedScene === textToVideoSceneId) {
    logger.info(
      `[handleRestartVideoGeneration] Entering ${textToVideoSceneId}`,
      {
        telegramId,
      }
    )
    ...
    logger.info(
      `[handleRestartVideoGeneration] Entering ${imageToVideoSceneId}`,
      {
        telegramId,
      }
    )
    ...
    logger.warn(
      `[handleRestartVideoGeneration] Cannot determine scene to restart. lastCompletedVideoScene: ${lastCompletedScene ?? 'undefined'}`,
      {
        telegramId,
      }
    )
