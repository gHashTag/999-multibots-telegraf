...
export class FalVeedFabricProvider implements ILipSyncProvider {
  ...
  readonly providerName = 'Fal Fabric'
this.supportedModels = ['fal-fabric']
  ...
  async generate(
    ...
    try {
      ...
      if (
        input.provider !== 'fal' ||
        input.modelId !== 'fal-fabric'
      ) {
        return {
          message: 'Invalid input for Fal Fabric provider',
          error: `Expected provider: fal, modelId: fal-fabric`,
          code: 'INVALID_INPUT',
          provider: 'fal',
      ...
      logger.info('🎬 Запуск Fal Fabric генерации', {
      ...
      if (!result.data?.video || !result.data.video.url) {
        return {
          message: 'No video generated by Fal Fabric API',
      ...
      logger.info('✅ Fal Fabric видео успешно сгенерировано', {
        telegramId: falInput.telegramId,
        uniqueId,
...
modelUsed: 'Fal Fabric',
      ...
      logger.error('❌ Ошибка генерации Fal Fabric', {
        telegramId: input.telegramId,
        error: error.message,
        stack: error.stack,
        provider: 'fal',
      ...
      if (
        ...
        return {
          message:
            'Fal Fabric account balance exhausted. Please top up your balance.',
          error: 'Fal Fabric account locked due to insufficient balance',
      ...
      if (error.status === 401) {
        return {
          message:
            'Fal Fabric API authentication failed. Please check your API key.',
          error: 'Invalid or expired Fal Fabric API key',
      ...
      return {
        message: 'Failed to generate video with Fal Fabric',
        error: error.message,
  ...
  async getStatus(taskId: string): Promise<any> {
    try {
      ...
      return {
        ...
        modelUsed: 'Fal Fabric',
          provider: 'fal',
          message: 'Fal Fabric is synchronous',      }
    } catch (error: any) {
  ...
  getModelsConfig(): LipSyncModelConfig[] {
    return this.supportedModels.map(modelId => ({
      ...
      description: 'Быстрая и стабильная модель для lip-sync от Fal Fabric',
      maxDuration: 60,
    }))
  }
