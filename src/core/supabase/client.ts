...
if (!SUPABASE_URL) {
  logger.error('🚨 CRITICAL ERROR: SUPABASE_URL is undefined. Check .env file loading.')
  logger.error('Available env keys:', Object.keys(process.env).filter(k => k.includes('SUPABASE')))
...
if (!SUPABASE_SERVICE_KEY && !SUPABASE_SERVICE_ROLE_KEY) {
  logger.error('🚨 CRITICAL ERROR: Neither SUPABASE_SERVICE_KEY nor SUPABASE_SERVICE_ROLE_KEY is defined')
