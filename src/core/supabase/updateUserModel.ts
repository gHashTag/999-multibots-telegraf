...
export const updateUserModel = async (
  ...
  if (error) {
    logger.error(
      `Error updating user model in Supabase: ${error.message}`,
      { telegram_id, model, error }
    )
    throw new Error(`Ошибка при обновлении модели пользователя: ${error.message}`)
  }

  if (!data) {
    logger.warn(
      'User not found when trying to update model',
      { telegram_id, model }
    )
