...
const setupHandlers = (bot: Telegraf<Context>, state: BotState): void => {
  ...
  // Добавляем обработчик команды /message_owner
  bot.command('message_owner', async ctx => {
    if (!isAllowedUser(ctx, state)) {
      return ctx.reply('⛔ У вас нет доступа к этому боту.')
    }

    const messageText = ctx.message.text.trim()
    const commandRegex = /^\/message_owner(?:@\w+)?\s*(.*)$/
    const match = commandRegex.exec(messageText)

    if (!match || !match[1]) {
      return ctx.reply(
        '📝 Пожалуйста, укажите текст сообщения после команды:\n' +
          '/message_owner Ваше сообщение для владельца бота'
      )
    }

    const message = match[1].trim()
    await ctx.reply('⏳ Отправляем сообщение владельцу бота...')

    try {
      // Импортируем скрипт отправки сообщения владельцу
      const { sendMessageToOwner } = await import(
        '@/scripts/send-message-to-owner'
      )

      const taskId = await sendMessageToOwner({
        message,
        bot_name: ctx.botInfo.username,
        telegram_id: ctx.from.id.toString(),
        metadata: {
          command: 'message_owner',
          chat_id: ctx.chat.id,
      ...
      if (taskId) {
        console.log('✅ Сообщение владельцу отправлено', {
          task_id: taskId,
          from: ctx.from.id,
        })

        await ctx.reply(
          '✅ Ваше сообщение успешно отправлено владельцу бота!\n\n' +
            'Благодарим за обратную связь.'
        )
      } else {
        console.error('❌ Ошибка при отправке сообщения владельцу', {
          from: ctx.from.id,
        })
      ...
      console.error('❌ Ошибка при обработке команды /message_owner', {
...
const startBot = async (token: string): Promise<void> => {
  try {
    ...
    await bot.telegram.setMyCommands([
      ...
      {
        command: 'message_owner',
        description: 'Отправить сообщение владельцу бота',
      },
    ])
