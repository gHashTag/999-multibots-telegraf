...
const createBotState = (): BotState => {
  ...
  return {
    ...
    initialized: false,
    config: {
      repositories: process.env.REPOSITORIES ? process.env.REPOSITORIES.split(',').map(repo => ({
        path: repo.trim(),
...
const setupHandlers = (bot: Telegraf<Context>, state: BotState): void => {
  ...
  bot.command('scan_improvements', async (ctx) => {
    ...
    try {
      await ctx.reply('🔍 Начинаю сканирование кодовой базы для поиска потенциальных улучшений...');
      ...
      setTimeout(async () => {
        try {
          ...
          await ctx.reply(`❌ Ошибка при сканировании: ${error instanceof Error ? error.message : String(error)}`);
        }
      }, 0);
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          const scanResults = await state.agent.scanMultipleRepositories(repositories, {
            limit: 20,
            aspectTypes: ['code_quality', 'performance', 'security']
          });
          
          let replyMessage = `✅ Сканирование нескольких репозиториев завершено!\n\n` +
            `📊 Статистика:\n` +
          ...
          if (scanResults.analyzed_repositories && scanResults.analyzed_repositories.length > 0) {
            replyMessage += `📂 Репозитории:\n` +
              scanResults.analyzed_repositories.map(repo => `- ${repo}`).join('\n') + '\n\n';
          }
          ...
          if (highPriorityItems.length > 0) {
            ...
            highPriorityItems.forEach((item, index) => {
              priorityMessage += `${index + 1}. [${item.id}] ${item.title}\n` +
  ...
  bot.command('improvement_details', async (ctx) => {
    ...
    if (!improvementId) {
      return ctx.reply('⚠️ Необходимо указать ID улучшения. Например: /improvement_details imp_123456');
    }
    ...
    try {
      ...
      let detailsMessage = `📝 Детали улучшения [${improvement.id}]\n\n` +
      ...
      await ctx.reply(`❌ Ошибка при получении деталей: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  bot.command('apply_improvement', async (ctx) => {
    ...
    if (!improvementId) {
      return ctx.reply('⚠️ Необходимо указать ID улучшения. Например: /apply_improvement imp_123456');
    }
    ...
    try {
      ...
      await ctx.reply(`❌ Ошибка при применении улучшения: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  bot.command('rate_improvement', async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(`❌ Ошибка при сохранении оценки: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  bot.command('enable_periodic_scan', async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(`❌ Ошибка при включении периодического сканирования: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  bot.command('disable_periodic_scan', async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(`❌ Ошибка при отключении периодического сканирования: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  bot.command('improvement_report', async (ctx) => {
    ...
    try {
      ...
      if (!report) {
        return ctx.reply('⚠️ Не удалось сгенерировать отчет. Возможно, нет данных по улучшениям.');
      }
      ...
      if (report.fullReportPath) {
        await ctx.replyWithDocument({ source: report.fullReportPath, filename: 'improvement_report.md' });
      }
    } catch (error) {
      console.error('Error generating improvement report:', error);
      await ctx.reply(`❌ Ошибка при генерации отчета: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
  ...
  bot.command('get_last_changelog', async (ctx) => {
    ...
    try {
      ...
      if (lastEntry) {
        ...
        await ctx.reply('⚠️ Не удалось найти записи в CG Log. Возможно, файл пуст или не существует.');
      }
    } catch (error) {
      console.error('Error getting latest changelog entry:', error);
      await ctx.reply('❌ Произошла ошибка при получении последней записи из CG Log');
    }
  });
  ...
  const startChangelogMonitoring = () => {
    ...
    const checkForChangelogUpdates = async () => {
      try {
        ...
        const newEntries = await getNewChangelogEntries(state.notifications.lastCheckedChangelog);
        
        if (newEntries.length > 0) {
          console.log(`📝 Обнаружено ${newEntries.length} новых записей в CG Log`);
            ...
            for (const entry of newEntries) {
              ...
              for (const userId of subscribedUsers) {
                try {
                  ...
                  console.error(`Error sending changelog notification to user ${userId}:`, sendError);
                }
              }
            }
          }
