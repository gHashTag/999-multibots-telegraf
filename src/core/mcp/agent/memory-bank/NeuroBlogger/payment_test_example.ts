...
export async function testMoneyIncome(): Promise<TestResult> {
  try {
    ...
    const processedEvent = await inngestTestEngine.waitForEvent(
      'payment/processed',
      {
        timeout: 5000,
    ...
    if (!processedEvent) {
      ...
      return {
        success: false,
        message:
          'Событие payment/processed не было получено в течение таймаута',
    ...
    if (newBalance !== expectedBalance) {
      console.log(
        `❌ Баланс не соответствует ожидаемому: ${newBalance} != ${expectedBalance}`
      )
      return {
        success: false,
...
export async function testDuplicatePayments(): Promise<TestResult> {
  try {
    ...
    const balanceAfterFirstOperation = await getUserBalance(
      testUser.telegram_id
    )
    console.log(
      `💰 Баланс после первой операции: ${balanceAfterFirstOperation}`
    )
    ...
    const balanceAfterSecondOperation = await getUserBalance(
      testUser.telegram_id
    )
    console.log(
      `💰 Баланс после второй операции: ${balanceAfterSecondOperation}`
    )

    // Проверка, что баланс не изменился после дублирующей операции
    if (balanceAfterFirstOperation !== balanceAfterSecondOperation) {
      console.log(
        `❌ Баланс изменился после дублирующей операции: ${balanceAfterFirstOperation} → ${balanceAfterSecondOperation}`
      )
      return {
        success: false,
    ...
    return {
      success: true,
      message:
        'Система корректно обработала дублирующуюся транзакцию (не изменила баланс)',
      name: 'Duplicate Payment Test',
    }
  } catch (error) {
...
export async function runPaymentProcessorTests(): Promise<TestResult[]> {
  ...
  console.log(
    `🏁 Тестирование завершено. Успешно: ${successCount}/${totalCount}`
  )
