...
- **Ошибка Replicate API (локально)**: На локальной машине пользователя отсутствует или недействителен `REPLICATE_API_TOKEN`, что приводит к ошибке 401 при генерации Нейрофото. *(Не является блокировщиком для деплоя, проблема локальной конфигурации)*.
...
*Эти правила можно будет формализовать и добавить в `MAIN.md` или в отдельный документ.*
