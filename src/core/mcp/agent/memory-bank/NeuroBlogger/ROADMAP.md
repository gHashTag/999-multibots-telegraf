...
- **Проверка конфигурации типов Vitest/TSConfig** (29 апреля 2025)
  - Статус: ✅ Зеленый
  - Описание: Проверена конфигурация `vitest.config.ts` и `tsconfig.json`. Конфликтов не обнаружено. Проблема разрешения типов в редакторе решена перезапуском TS-сервера/редактора. Команда `pnpm typecheck` проходит успешно.
- **Рефакторинг Цен (Завершение)** ({current_date_placeholder})
  - Статус: ✅ Зеленый
  - Описание: Завершен рефакторинг системы цен. Реализована централизованная логика расчета и списания звезд с использованием `calculateFinalStarPrice`. Обновлены места использования, реализовано списание в `enterTargetScene`. Базовые тесты проходят.
...
2. **⏳ Проверка зависимостей**
   ...
   - Установка bun
   - Наличие `node_modules` (или эквивалента bun)
...
### P3: Написание Тестов для Цен
*   **Статус:** ✅ Юнит-тесты для калькулятора (`calculator.ts`) завершены. ⚠️ Интеграционные тесты (`checkBalanceScene.ts`) требуют рефакторинга зависимостей для запуска.
*   **Результат:** Создан документ `__tests__/README.md` с прейскурантом и диагностикой.

### P4: Запуск и Проверка (`pnpm dev`)
*   **Статус:** ✅ Запускали ранее, сервер работает.
...
**Статус:** ✅ Завершено (с частичным откатом)
**Приоритет:** Высокий

- [✅] **Шаг 1:** Создать `src/config/pricing.config.ts` с константами.
- [✅] **Шаг 2:** Создать `src/price/calculator.ts` с функцией `calculateFinalStarPrice`.
- [⏪] **Шаг 3:** Адаптировать `calculateFinalStarPrice` (частично откачено).
- [✅] **Шаг 4:** Обновить `src/scenes/checkBalanceScene.ts`.
- [✅] **Шаг 5:** Удалить `src/price/constants/modelsCost.ts`.
- [✅] **Шаг 6:** Обновить основные места использования старой логики (`/price`, визарды, сервисы).
- [✅] **Шаг 7:** Реализовать логику списания звезд в `enterTargetScene`.
- [⏳] **Шаг 8:** Обновить документацию по ценам (`docs/pricing.md`).
- [✅] **Задача:** Исправить ошибки типов.
- [✅] **Задача:** Стабилизировать код после отката.

---

## 🧪 3. Написание тестов для системы ценообразования

**Статус:** ⏳ В Работе (базовые тесты пройдены)
**Приоритет:** Средний

- [✅] Обновить и запустить базовые тесты для `calculateFinalStarPrice` (`calculator.test.ts`, `steps.test.ts`).
- [📝] Написать тесты для `calculateFinalStarPrice`, покрывающие логику `modelId` (когда она будет реализована).
- [📝] Написать интеграционные тесты для `checkBalanceScene` и команды `/price`.
