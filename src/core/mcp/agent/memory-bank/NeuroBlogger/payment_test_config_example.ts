...
export const TEST_CONFIG = {
  ...
  helpers: {
    ...
    generateTestOperationId: () => `test_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`,
    
    // Функция для очистки тестовых данных
    async cleanupTestData(supabaseClient: ReturnType<typeof createClient<Database>>, telegram_id: string) {
      try {
        ...
        console.log(`🧹 Очищены тестовые данные для пользователя ${telegram_id}`);
      } catch (error) {
    ...
    async setupTestEnvironment() {
      try {
        ...
        console.error(`❌ Ошибка при инициализации тестового окружения: ${error.message}`);
        throw error;
      }
    },
...
export function getTestConfig(environment: 'local' | 'docker' = 'local'): TestConfig {
