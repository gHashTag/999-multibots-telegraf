...
export const pipe2 = <A, B, R>(a: A, f: (a: A) => B, g: (b: B) => R): R =>
  g(f(a))
...
export const pipeFlow =
  <T extends any[], R>(
    ...fns: {
      [K in keyof T]: (
        arg: K extends 0 ? T[0] : T[K] extends keyof T ? T[K] : never
      ) => any
    }
  ): ((...args: T) => R) =>
  (...args: T) =>
    fns.reduce((acc, fn, index) => fn(index === 0 ? args[0] : acc), {} as any)
...
export const flow = ((...fns: any[]) => {
  ...
  // Первая функция получает ВСЕ аргументы вызова, остальные — по одному
  // значению. Так flow устроен в fp-ts, и без этого композиция теряет
  // арность: flow((a, b) => a + b, x => x * 2)(5, 3) давала NaN.
  const [first, ...rest] = fns
  return (...args: any[]) => rest.reduce((acc, fn) => fn(acc), first(...args))
}) as {
  (): (x: any) => any
  <A, B>(fn: (a: A) => B): (a: A) => B
...
export const flow2 =
  <A, R>(f: (a: A) => R, g: (r: R) => R): ((a: A) => R) =>
  a =>
    g(f(a))

export const flow3 =
  <A, B, R>(f: (a: A) => B, g: (b: B) => R, h: (r: R) => R): ((a: A) => R) =>
  a =>
    h(g(f(a)))
...
export const flowComposition =
  <T extends any[], R>(
    ...fns: {
      [K in keyof T]: (
        arg: K extends 0 ? R : T[K] extends keyof T ? T[K] : never
      ) => any
    }
  ): ((a: T[0]) => R) =>
  (a: T[0]) =>
    fns.reduceRight((acc, fn) => fn(acc), {} as any)
...
export const curry = <A, B, R>(fn: (a: A, b: B) => R) => {
  ...
  // `length` у функции — свойство только для чтения (writable: false).
  // Присваивание молча не работает в нестрогом режиме и ВЫБРАСЫВАЕТ
  // TypeError в строгом, то есть в любом ES-модуле. Правильный способ —
  // defineProperty: свойство configurable, поэтому переопределяется.
  Object.defineProperty(curried, 'length', { value: 2 })
  return curried as {
    (a: A): (b: B) => R
    (a: A, b: B): R
  }
...
export const compose =
  <A, R>(f: (r: R) => R, g: (a: A) => R): ((a: A) => R) =>
  a =>
    f(g(a))
export const compose2 =
  <A, B, R>(f: (b: B) => R, g: (a: A) => B): ((a: A) => R) =>
  a =>
    f(g(a))
export const compose3 =
  <A, B, C, R>(f: (c: C) => R, g: (b: B) => C, h: (a: A) => B): ((a: A) => R) =>
  a =>
    f(g(h(a)))
...
export const conditional =
  ...
  (a: A) =>
    condition(a) ? onTrue(a) : onFalse(a)
...
export const prop =
  ...
  <A extends { [P in K]: A[K] }>(obj: A): A[K] =>
    obj[key]

export const propOr =
  <K extends string, A extends { [P in K]: A[K] }, B>(
    defaultValue: B,
    key: K
  ) =>
  (obj: A): A[K] | B =>
...
export const pathOr =
  <K extends string[], A extends Record<K[number], any>, B>(
    defaultValue: B,
    keys: K
  ) =>
  (obj: A): A[K[number]] | B => {
