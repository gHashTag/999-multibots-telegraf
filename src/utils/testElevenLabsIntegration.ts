...
import {
  getVoiceId,
  getFallbackVoiceId,
  getAvailableDefaultVoices,
} from '@/core/supabase/getVoiceId'
...
class ElevenLabsIntegrationTester {
  ...
  async testApiKeyPresence() {
    ...
    if (!apiKey) {
      this.addResult(
        'API_KEY',
        'FAIL',
        'ELEVENLABS_API_KEY не найден в переменных окружения'
      )
      return false
    }
    ...
    if (apiKey.length < 20) {
      this.addResult(
        'API_KEY',
        'WARN',
        'API ключ выглядит подозрительно коротким',
  ...
  async testDefaultVoiceIds() {
    try {
      ...
      this.addResult(
        'DEFAULT_VOICES',
        'PASS',
        `Загружено ${Object.keys(defaultVoices).length} default голосов`,
        defaultVoices
      )

      const primaryFallback = getFallbackVoiceId()
      this.addResult('FALLBACK_VOICE', 'PASS', 'Fallback голос настроен', {
        fallbackVoiceId: primaryFallback,
      })
      ...
      this.addResult(
        'DEFAULT_VOICES',
        'FAIL',
        'Ошибка при получении default голосов',
        error
      )
      return false
    }
    ...
    try {
      ...
      if (voiceExists) {
        this.addResult(
          'VOICE_EXISTS',
          'PASS',
          'Primary fallback голос существует в ElevenLabs',
        ...
        this.addResult(
          'VOICE_EXISTS',
          'FAIL',
          'Primary fallback голос НЕ найден в ElevenLabs!',
          { voiceId: PRIMARY_FALLBACK_VOICE_ID }
        )
        return false
      }
      ...
      this.addResult(
        'VOICE_EXISTS',
        'FAIL',
        'Ошибка при проверке существования голоса',
        error
      )
      return false
    }
    ...
    try {
      ...
      if (voiceId === PRIMARY_FALLBACK_VOICE_ID) {
        this.addResult(
          'VOICE_RETRIEVAL',
          'PASS',
          'getVoiceId правильно возвращает fallback для несуществующего пользователя',
          {
            voiceId,
        ...
        this.addResult(
          'VOICE_RETRIEVAL',
          'FAIL',
          'getVoiceId НЕ возвращает fallback',
          {
            received: voiceId,
      ...
      this.addResult(
        'VOICE_RETRIEVAL',
        'FAIL',
        'Ошибка при тестировании получения voice_id',
        error
      )
      return false
    }
    ...
    try {
      ...
      if (audioPath && fs.existsSync(audioPath)) {
        ...
        this.addResult(
          'AUDIO_GENERATION',
          'FAIL',
          'Аудио файл не создан или не найден',
          { audioPath }
        )
        return false
      }
  ...
  async testInvalidVoiceFallback() {
    try {
      ...
      console.log(
        '🔄 Тестируем fallback механизм с недействительным voice_id...'
      )
      ...
      this.addResult(
        'INVALID_VOICE_FALLBACK',
        'FAIL',
        'Ошибка при тестировании fallback',
        error
      )
      return false
    }
