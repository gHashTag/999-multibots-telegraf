 ...
 * @param isTest - Использовать тестовый режим
...
export function generateRobokassaUrl(
  ...
  logger.debug('📝 Строка для подписи:', {
...
/**
 * Генерирует ID инвойса для Robokassa
 * @param merchantLogin - Логин мерчанта
 * @param outSum - Сумма платежа
 * @param invId - ID инвойса
 * @param description - Описание платежа
 ...
 * @returns URL для оплаты
 */
export async function generateRobokassaInvoiceId(
  merchantLogin: string,
  outSum: number,
  invId: number,
  description: string,
  ...
  logger.info('🚀 Запуск generateRobokassaInvoiceId', {
  ...
  try {
    ...
    logger.error('❌ Ошибка в generateRobokassaInvoiceId:', {
