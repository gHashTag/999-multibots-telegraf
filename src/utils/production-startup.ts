import { autoConfigureProductionWebhooks, validateWebhookSetup } from '../utils/webhook-manager'
...
export class ProductionStartupManager {
   ...
   */
  registerBot(bot: any, name: string, token: string, port?: number, username?: string): void {
    this.botInstances.push({ bot, name, token, port, username })
    botLogger.info('StartupManager', `📝 Registered bot: ${name} (${username || 'unknown'})`)
  }
    ...
    try {
      ...
      const timeoutPromise = new Promise<boolean>((_, reject) => {
        this.startupTimer = setTimeout(() => {
          reject(new Error(`Production startup timeout after ${this.config.startupTimeout}ms`))
      ...
      botLogger.error('StartupManager', `❌ Production startup failed: ${error}`)
      return false
    }
  }
  ...
  private async executeStartup(): Promise<boolean> {
    try {
      ...
      botLogger.info('StartupManager', '✅ Production deployment completed successfully!')
      return true
  ...
  private validateEnvironment(): boolean {
    ...
    for (const envVar of requiredEnvVars) {
      if (!process.env[envVar]) {
        botLogger.error('StartupManager', `❌ Missing required environment variable: ${envVar}`)
        return false
      }
    }
  ...
  private async prepareBotInstances(): Promise<void> {
    botLogger.info('StartupManager', `📦 Preparing ${this.botInstances.length} bot instances...`)
    ...
    for (const botMeta of this.botInstances) {
      try {
        ...
        botLogger.info('StartupManager', `✅ Prepared bot: ${botMeta.name} (@${botMeta.username})`)
      } catch (error) {
        botLogger.error('StartupManager', `❌ Failed to prepare bot ${botMeta.name}: ${error}`)
        throw error
      }
    }
  }
  ...
  private async configureWebhooks(): Promise<void> {
    ...
    this.webhookResults = await autoConfigureProductionWebhooks(botsForWebhook, webhookConfig)
    ...
    if (failureCount > 0) {
      botLogger.warn('StartupManager', `⚠️ ${failureCount} webhook configurations failed`)
    } else {
      botLogger.info('StartupManager', '✅ All webhooks configured successfully')
    }
  }
  ...
  private async startBotInstances(): Promise<void> {
    ...
    for (let i = 0; i < this.botInstances.length; i++) {
      ...
      if (!webhookResult?.success) {
        botLogger.warn('StartupManager', `⚠️ Skipping bot ${botMeta.name} due to webhook failure`)
        continue
      }
      ...
      try {
        ...
        botLogger.info('StartupManager', `✅ Started bot: ${botMeta.name} on port ${port}`)
      } catch (error) {
        botLogger.error('StartupManager', `❌ Failed to start bot ${botMeta.name}: ${error}`)
        throw error
      }
    }
  }
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    const healthResults = await Promise.all(
      this.botInstances.map(async (botMeta) => {
        try {
          const validation = await validateWebhookSetup(botMeta.bot, botMeta.name)
    ...
    botLogger.info('StartupManager', `📊 Health check: ${healthyBots}/${totalBots} bots healthy`)

    if (healthyBots < totalBots) {
      const unhealthyBots = healthResults.filter(r => !r.valid)
      for (const bot of unhealthyBots) {
        botLogger.warn('StartupManager', `⚠️ Unhealthy bot: ${bot.botName} - ${bot.error}`)
      }
    }
  }
  ...
  private async validateDeployment(): Promise<void> {
    ...
    try {
      if (this.config.webhookDomain) {
        ...
        if (response.ok) {
          ...
          botLogger.warn('StartupManager', `⚠️ Domain returned status ${response.status}`)
        }
      }
    } catch (error) {
      botLogger.warn('StartupManager', `⚠️ Domain accessibility check failed: ${error}`)
    }
    ...
    botLogger.info('StartupManager', `📈 Deployment summary: ${runningBots} bots, ${successfulWebhooks} webhooks`)
  }
  ...
  private async rollbackOnFailure(): Promise<void> {
    ...
    for (const botMeta of this.botInstances) {
      try {
        await botMeta.bot.telegram.deleteWebhook({ drop_pending_updates: true })
        botLogger.info('StartupManager', `🗑️ Cleared webhook for ${botMeta.name}`)
      } catch (error) {
        botLogger.error('StartupManager', `❌ Rollback failed for ${botMeta.name}: ${error}`)
      }
    }
  }
...
export function createProductionStartup(): ProductionStartupManager {
  const config: ProductionStartupConfig = {
    ...
    webhookProtocol: process.env.WEBHOOK_PROTOCOL as 'http' | 'https' || undefined,
    botPorts: {
