...
export function isVirtualMoney(payment: PaymentRecord): boolean {
  ...
  return virtualPaymentMethods.includes(payment.payment_method) ||
         virtualDescriptionPatterns.some(pattern => pattern.test(payment.description)) ||
         (payment.amount === 0 && payment.stars > 0); // Stars without actual payment
}

/**
 * Categorizes virtual money by source type
 */
export function categorizeVirtualMoney(payment: PaymentRecord): keyof FinancialBreakdown['virtualMoney']['sources'] {
 ...
 */
export function analyzeFinancialData(payments: PaymentRecord[]): FinancialBreakdown {
  ...
  const realRevenue = {
    ...
    averagePerTransaction: realPayments.length > 0
      ? realPayments.reduce((sum, p) => sum + (p.stars || 0), 0) / realPayments.length
      : 0
  };
  ...
  const inflationRatio = realRevenue.stars > 0
    ? virtualMoney.stars / realRevenue.stars
    : 0;
 ...
 */
export function generateFinancialReport(botName: string, breakdown: FinancialBreakdown): string {
