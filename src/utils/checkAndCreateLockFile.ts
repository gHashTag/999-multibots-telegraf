...
export function checkAndCreateLockFile(): boolean {
  try {
    ...
    if (forceStart) {
      logger.warn(
        '⚠️ Активирован режим принудительного запуска (FORCE_START=true)',
        {
          description: 'Bot forced to start regardless of lock file',
          warning:
            'This may cause conflicts if another bot instance is running',
        }
      )
    }
    ...
    if (fs.existsSync(LOCK_FILE_PATH)) {
      try {
        ...
        if (isProcessRunning && !forceStart) {
          ...
          logger.warn(
            '⚠️ Удаление существующего файла блокировки в режиме принудительного запуска',
            {
