...
export class ProductionMonitor {
  ...
  private async checkContainerStatus(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('ssh -i ~/.ssh/selectel root@185.161.67.53 \'docker ps --filter name=999-multibots --format "{{.Status}}"\'')
    ...
    try {
      const portChecks = await Promise.all(
        Array.from({ length: 10 }, (_, i) => i + 3001).map(async (port) => {
          try {
            const { stdout } = await execAsync(`ssh -i ~/.ssh/selectel root@185.161.67.53 'netstat -tulpn | grep ":${port}.*LISTEN"'`)
  ...
  private async checkDomainAccessibility(): Promise<HealthCheck> {
    try {
      ...
      const { stdout } = await execAsync('curl -s -o /dev/null -w "%{http_code}" "http://test-render-farm.ru/" --max-time 10')
      const responseTime = Date.now() - startTime
      const httpStatus = stdout.trim()
      
      if (httpStatus === '200') {
        const status = responseTime > this.config.alertThresholds.responseTimeMs ? 'warn' : 'pass'
        return {
  ...
  private async checkWebhookConfiguration(): Promise<HealthCheck> {
    try {
      ...
      for (let i = 1; i <= 10; i++) {
        try {
          const { stdout } = await execAsync(`ssh -i ~/.ssh/selectel root@185.161.67.53 'docker exec 999-multibots printenv | grep "BOT_TOKEN_${i}=" | cut -d"=" -f2'`)
  ...
  private async checkWebhookServerConfig(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('ssh -i ~/.ssh/selectel root@185.161.67.53 \'grep "REPO_PATH" /root/webhook-deploy-server.js 2>/dev/null || echo ""\'')
      ...
      if (repoPath.includes('999-agents-telegraf')) {
        return {
          ...
          status: 'pass',
          message: 'Webhook server correctly configured for 999-agents-telegraf',
          timestamp: new Date(),
          critical: false
        }
        ...
        return {
          name: 'webhook_server_config',
          status: 'warn',
          message: 'Webhook server pointing to wrong repo: 999-agents-vibecoder',
          timestamp: new Date(),
          critical: false
        }
      } else {
  ...
  private async checkSystemResources(): Promise<HealthCheck> {
    try {
      const { stdout } = await execAsync('ssh -i ~/.ssh/selectel root@185.161.67.53 \'df -h / | tail -1 | awk "{print \\$5}" | sed "s/%//"\'')
  ...
  private async analyzeAndAlert(checks: HealthCheck[]): Promise<void> {
    const criticalIssues = checks.filter(check => check.status === 'fail' && check.critical)
    ...
    if (warnings.length > 0) {
      ...
      if (!this.alertsSent.has(warningKey)) {
        await this.sendAlert(`⚠️ Warnings detected: ${warnings.map(w => w.message).join(', ')}`, false)
  ...
  private async sendAlert(message: string, critical: boolean): Promise<void> {
    ...
    if (this.config.notifications.telegramBotToken && this.config.notifications.alertChatId) {
      try {
        await fetch(`https://api.telegram.org/bot${this.config.notifications.telegramBotToken}/sendMessage`, {
          method: 'POST',
  ...
  getHealthStatus(): { overall: string; checks: HealthCheck[] } {
    ...
    const hasCritical = recentChecks.some(check => check.status === 'fail' && check.critical)
...
// CLI interface
if (typeof process !== 'undefined' && process.argv.length > 2 && process.argv[1]?.endsWith('production-monitor.ts')) {
