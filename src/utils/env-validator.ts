...
class EnvironmentValidator {
  ...
  private validateBotTokens(
    ...
    if (botTokens.length === 0) {
      ...
      result.warnings.push(
        `Only ${botTokens.length} bot tokens configured, consider adding more for redundancy`
      )
    }
  ...
  private validateWebhookConfiguration(
    ...
    if (env.WEBHOOK_DOMAIN) {
      ...
      if (
        env.WEBHOOK_DOMAIN.startsWith('https://') &&
        domain === 'test-render-farm.ru'
      ) {
        result.warnings.push(
          'Domain test-render-farm.ru may not support HTTPS - consider using HTTP'
        )
      }
    }
  ...
  private validateSecuritySettings(
    ...
    if (secretCount === 0) {
      result.warnings.push(
        'No webhook secrets configured - consider adding for enhanced security'
      )
    }
  ...
  private validateTestModeDisabled(env: any, result: ValidationResult): void {
    ...
    if (env.BOT_TOKEN_TEST_1 || env.BOT_TOKEN_TEST_2) {
      result.warnings.push(
        'Test bot tokens are present but should not be used in production'
      )
    }
...
`
  ...
  auditSecrets(): { issues: string[]; recommendations: string[] } {
    ...
    for (const secret of this.secrets) {
      ...
      if (
        secret.value === secret.value.toLowerCase() &&
        secret.type === 'webhook_secret'
      ) {
        recommendations.push(
          `Webhook secret ${secret.name} should include mixed case characters`
        )
      }
    }
...
if (
  ...
  process.argv[1].endsWith('env-validator.ts')
) {
