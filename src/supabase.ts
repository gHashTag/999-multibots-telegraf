...
const isTestEnvironment = () => {
  ...
  ) // явное указание тестового окружения
}
...
if (isTestEnvironment()) {
  try {
    ...
    const {
      createSupabaseMock,
    } = require('./test-utils/core/mock/supabaseMock.js')
    supabaseClient = createSupabaseMock()
    console.log('[TEST MODE] Successfully loaded enhanced Supabase mock')
  } catch (error) {
    console.log(
      '[TEST MODE] Could not load enhanced mock, using basic mock instead:',
      error
    )
    ...
    supabaseClient = {
      from: (tableName: string) => ({
        ...
        insert: (data: any, options?: { returning?: string }) =>
      ...
      storage: {
        from: (bucket: string) => ({
          ...
          getPublicUrl: (path: string) => ({
            data: { publicUrl: `https://test-storage/${path}` },
          }),
