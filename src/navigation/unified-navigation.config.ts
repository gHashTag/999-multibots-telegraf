...
export interface NavigationConfig {
  ...
  levels: Record<
    number,
    { title_ru: string; title_en: string; admin_only?: boolean }
  >
}
...
export const createMainMenuKeyboard = (ctx: MyContext) => {
  ...
  const mainButtons = NAVIGATION_BUTTONS.filter(btn => {
    ...
    if (btn.category === 'navigation' || btn.category === 'payment')
      return false
 ...
 */
export const handleMenuButtonPress = async (
  ctx: MyContext,
  buttonText: string
): Promise<boolean> => {
  const isRu = isRussianFromState(ctx)
...
export function mainMenu(
  ctxOrOptions:
    | MyContext
    | { isRu?: boolean; subscription?: any; ctx: MyContext }
) {
  ...
  if ('ctx' in ctxOrOptions) {
    ...
    console.log(
      '🔄 [mainMenu WRAPPER] Old signature detected, extracting ctx:',
      {
        telegramId: ctx.from?.id,
    ...
    ctx = ctxOrOptions
    console.log(
      '✅ [mainMenu WRAPPER] New signature detected, using ctx directly:',
      {
        telegramId: ctx.from?.id,
      }
    )
