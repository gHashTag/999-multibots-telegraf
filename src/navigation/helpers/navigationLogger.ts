 ...
 */
const currentLogLevel: NavigationLogLevel = process.env.NAV_LOG_LEVEL
  ? parseInt(process.env.NAV_LOG_LEVEL)
  : NavigationLogLevel.INFO
 ...
 * Лог callback query
 */
export function logCallbackQuery(
  ctx: MyContext,
  data: string,
  handled: boolean
): void {
 ...
 */
export function logNavigationWarning(
  ctx: MyContext,
  warning: string,
  details?: Record<string, unknown>
): void {
...
export function logDeepScene(ctx: MyContext, depth: number): void {
  if (depth >= 3) {
    navLog(
      NavigationLogLevel.INFO,
      '🔻',
      `DEEP SCENE WARNING: depth=${depth}`,
      ctx,
      {
        depth,
        stack: formatNavigationStack(ctx),
 ...
 */
export function withNavigationLogging<
...
>(fn: T, fnName: string): T {
  return (async (...args: any[]) => {
    ...
    try {
      ...
      navLog(
        NavigationLogLevel.DEBUG,
        '✅',
        `END ${fnName} (${duration}ms)`,
        ctx
      )
      return result
    } catch (error) {
  ...
  return async (ctx: MyContext, next: () => Promise<void>) => {
    ...
    if (sceneBefore !== sceneAfter) {
      navLog(
        NavigationLogLevel.DEBUG,
        '🔄',
        `SCENE CHANGED: ${sceneBefore} → ${sceneAfter}`,
        ctx,
        {
          sceneBefore,
          sceneAfter,
