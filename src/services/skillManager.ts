...
export async function trackGeneration(params: {
  ...
  try {
    const { error } = await supabaseAdmin.from('bot_skills_log').insert({
      ...
      prompt_prefix: params.prompt.substring(0, 50),
      model: params.model,
    ...
    logger.debug(
      '[SkillManager] trackGeneration failed (table may not exist)',
      { error: String(err) }
    )
  }
}
...
export async function detectSkillCandidate(
  ...
  try {
    ...
    logger.debug('[SkillManager] detectSkillCandidate failed', {
      error: String(err),
    })
    return []
  }
...
export async function applySkill(
  skillId: string
): Promise<{ prompt_template: string; settings: Record<string, any> } | null> {
  try {
    const { data, error } = await supabaseAdmin
      .from('bot_skills')
      .select('prompt_template, settings')
      .eq('id', skillId)
      .eq('is_active', true)
      .single()
    if (error) throw error
...
export async function incrementSkillUsage(skillId: string): Promise<void> {
  try {
    const { error } = await supabaseAdmin.rpc('increment_skill_usage', {
      skill_id: skillId,
    })
    if (error) {
