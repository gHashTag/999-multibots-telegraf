...
export const convertVoiceToMp3 = async (filePath: string): Promise<string> => {
  try {
    ...
    execSync(
      `ffmpeg -i "${filePath}" -c:a libmp3lame -q:a 4 "${outputFilePath}" -y`
    )
...
export const extractAudioFromVideo = async (
  ...
  try {
    ...
    const outputFilePath = path.join(
      tempDir,
      `${path.basename(filePath, path.extname(filePath))}_audio.mp3`
    )
...
export const getMediaDuration = async (filePath: string): Promise<number> => {
  try {
    ...
    if (isNaN(duration)) {
      throw new Error(
        'Не удалось определить длительность: некорректное значение'
      )
    }
...
export const splitAudioIntoChunks = async (
  ...
  try {
    ...
    for (let i = 0; i < numChunks; i++) {
      ...
      // Создаем часть аудио
      execSync(
        `ffmpeg -i "${filePath}" -ss ${startTime} -t ${chunkDuration} -acodec copy "${outputPath}" -y`
      )
    ...
    if (chunkPaths.length === 0) {
      throw new Error(
        'Не удалось разделить аудио на части: ни одна часть не была создана'
      )
    }
