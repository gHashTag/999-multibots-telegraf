...
export async function generateModelTrainingV2(
  ...
  try {
    ...
    const currentTraining = await response.json() as TrainingResponse

    // Создаем запись о тренировке
    await createModelTrainingV2({
      finetune_id: currentTraining.finetune_id as string,
    ...
    await updateUserBalance({
      ...
      description: `Refund for model training ${modelName} (steps: ${steps})`,
      bot_name,
    })
