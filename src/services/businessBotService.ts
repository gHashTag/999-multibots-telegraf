...
import { deepLink, matchCards } from '@/handlers/inlineQuery'
...
/** Media a customer sends expecting a reaction; stickers and the like are ignored quietly. */
const CUSTOMER_MEDIA_FIELDS = ['photo', 'voice', 'video', 'document'] as const

const NON_TEXT_REPLY =
  'Спасибо! Пока я читаю только текст — напишите, пожалуйста, словами, что вам нужно.'
...
export async function handleBusinessMessage(
  ...
  if (!text) {
    ...
    const isCustomerMedia = CUSTOMER_MEDIA_FIELDS.some(
      f => msg[f] !== undefined
    )
    ...
    if (isCustomerMedia) {
      try {
        await sendAsOwner(NON_TEXT_REPLY)
      } catch (error) {
        logger.warn('[Business] Non-text reply failed', {
          error: error instanceof Error ? error.message : String(error),
          connId,
          chatId,
        })
