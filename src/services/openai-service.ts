    ...
    results.forEach((result, index) => {
      ...
      if (index > 0) {
        ...
        if (
          prevResult &&
          prevResult.segments &&
          prevResult.segments.length > 0
        ) {
          const lastSegment =
            prevResult.segments[prevResult.segments.length - 1]
          if (lastSegment) {
 ...
 */
export const getAudioFileFromMessage = async (
  telegram: any,
  fileId: string
) => {
  try {
    ...
    return new Promise<{ fileId: string; filePath: string }>(
      (resolve, reject) => {
 ...
 */
export const transcribeAudioFromTelegram = async (
  fileId: string,
  telegramBot: any
) => {
  try {
 ...
 */
export const transcribeAudioBasic = async (
  filePath: string,
  language?: string,
  model = 'whisper-1'
): Promise<string> => {
