...
export const SWEEP_TAIL =
  ...
  'ничего не готовь. НИЧЕГО НЕ ОТПРАВЛЯЙ САМ: только подготовь; владелец нажмёт ' +
  'кнопку. Ответ — одна строка: кому и что подготовлено.'
...
export const PRESET_VALUES = ['waiting', 'hot', 'talk'] as const
...
export type ScopeSpec =
  ...
  | { kind: 'filter'; predicates: Predicate[]; limit: number; label: string }
...
export function parseSweepArgs(rawArgs: string[]): ScopeSpec {
  ...
  return { kind: 'filter', predicates, limit, label: labelParts.join(' ') }
}
...
export function itemsFromRows(rows: LeadRowLike[]): ScopeItem[] {
  return rows
    .filter(r => ID_RE.test(String(r.lead ?? '')))
    .map(r => ({
      chat: String(r.lead),
      display: cleanDisplay(r.display),
      next: r.next ? String(r.next) : null,
    }))
}
