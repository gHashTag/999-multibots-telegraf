...
import {
  ...
  ModelTrainingDirectResult,
} from '@/interfaces/neuro/model.interface'
...
export async function createModelTrainingOld(
  ...
  try {
    ...
    const requestId = generateModelRequestId(
      config.telegram_id,
      config.modelName
    )
    ...
    if (useDirectGeneration) {
      ...
      if (!directResult.success) {
        throw new Error(
          directResult.error || 'Unknown error in direct generation'
        )
      }
    ...
    throw new Error(
      'Unexpected flow: neither Inngest nor direct generation was used'
    )
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : 'Unknown error'
