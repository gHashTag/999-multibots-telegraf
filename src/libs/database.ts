 ...
 */
export async function getUserSub(telegramId: string): Promise<UserSubscription | null> {
  try {
 ...
 */
export async function hasActiveSubscription(telegramId: string): Promise<boolean> {
 ...
 */
export async function createSubscription(subscriptionData: Partial<UserSubscription>): Promise<number | null> {
  try {
    const { data, error } = await supabase
      .from('subscriptions')
