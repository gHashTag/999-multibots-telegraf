    ...
    try {
      ...
      const usersResult = await step.run('fetch-users', async () => {
        ...
        if (test_mode) {
          ...
          logger.info(
            `👤 Тестовый режим: получен ${users.length} пользователь`,
            {
          ...
          users = data || []
          logger.info(
            `👥 Получено ${users.length} пользователей для бота ${bot_name}`,
            {
      ...
      logger.info(
        `📦 Рассылка разбита на ${batches.length} групп по ${batchSize} пользователей`,
        {
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        const batchResults = await step.run(
          ...
          async () => {
            logger.info(
              `🔄 Обработка группы ${batchIndex + 1}/${batches.length}`,
              {
            ...
            for (const user of batchUsers) {
              ...
              try {
                ...
                if (contentType === 'post_link' && postLink) {
                  ...
                  throw new Error(
                    `Не указан контент для contentType=${contentType}`
                  )
                }
        ...
        logger.info(
          `✅ Обработана группа ${batchIndex + 1}/${batches.length}`,
          {
