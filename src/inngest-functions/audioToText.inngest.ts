...
import { downloadFile, getUserById, updateUserBalance } from '../core/supabase'
import {
  ...
  async ({ event, step }: { event: AudioToTextEvent; step: any }) => {
    ...
    try {
      ...
      if (duration <= MAX_SINGLE_AUDIO_DURATION) {
        ...
        transcriptionResult = await step.run(
          ...
          async () => {
            try {
              ...
              const audioParts = await splitAudioIntoChunks(
                audioPath,
                CHUNK_SIZE
              )
              ...
              throw new Error(
                'Не удалось выполнить транскрипцию длинного аудио'
              )
            }
          }
        )
      ...
      await step.run('send-result', async () => {
        try {
          ...
          if (resultText.length <= 4000) {
            await bot.telegram.sendMessage(user.id, resultText, {
              parse_mode: 'Markdown',
            })
          } else {
              ...
              reply_markup: {
                inline_keyboard: [
                  [
                    {
                      text: 'TXT',
                      callback_data: `export_txt:${transcriptionResult.taskId}`,
                    },
                    {
                      text: 'DOCX',
                      callback_data: `export_docx:${transcriptionResult.taskId}`,
                    },
                  ],
                  [
                    {
                      text: 'PDF',
                      callback_data: `export_pdf:${transcriptionResult.taskId}`,
                    },
                    {
                      text: 'JSON',
                      callback_data: `export_json:${transcriptionResult.taskId}`,
                    },
                  ],
                ],
              },
