...
const SUPPORTED_FORMATS = ['mp3', 'mp4', 'mpeg', 'mpga', 'ogg', 'oga', 'wav', 'webm', 'm4a']
...
export const audioTranscriptionProcessor = inngest.createFunction(
  ...
  async ({ event, step, logger: inngestLogger }: { 
    ...
    try {
      const { userId, chatId, fileId, messageId, settings = {} } = event.data as AudioTranscriptionEventData
      ...
      const fileExtension = path.extname(fileInfo.file_path).slice(1).toLowerCase()
      ...
      const errorMessage = error instanceof Error ? error.message : String(error)
      logger.error('Error processing audio transcription', { error: errorMessage })
