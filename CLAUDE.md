...
### ⚠️ АВТОМАТИЧЕСКОЕ ЗАВЕРШЕНИЕ ЗАДАЧ - ОБЯЗАТЕЛЬНО!

**ВСЕГДА** после выполнения любой значимой задачи:
...
2. **Автоматически выполнить завершение**:
   ```bash
   /scripts/auto-finish-task.sh "commit_message"
   ```

3. **НЕ ЗАБЫВАТЬ про конфликты**:
   - ✅ Скрипт автоматически проверяет и разрешает конфликты
   - ✅ Специальная обработка textToVideoWizard конфликтов  
   - ✅ Автоматический push и обновление PR
...
- Нерешенные конфликты в PR
- Незакоммиченные значимые изменения
- PR без проверки готовности к merge

### ✅ АВТОМАТИЗАЦИЯ РАБОТАЕТ:
- GitHub Actions автоматически разрешает типичные конфликты
- Скрипт auto-finish-task.sh проверяет все перед push
- Система предотвращает повторение проблем с конфликтами
