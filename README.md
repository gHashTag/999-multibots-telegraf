...
### 1. `TypeError: Cannot read properties of undefined (reading 'resize')` when mocking `Markup.keyboard()`

-   **Проблема:** При попытке вызвать `.resize()` (или `.oneTime()`, `.selective()`) на результате `Markup.keyboard()` в юнит-тестах возникает ошибка `TypeError`.
-   **Причина:** Стандартные моки `telegraf` (например, в `__tests__/setup.ts`) могут не полностью имитировать цепочку вызовов `Markup`. Прямой возврат `{ reply_markup: { keyboard: [] } }` недостаточен, так как у этого объекта нет метода `.resize()`.
-   **Решение 1 (Рекомендуемое - Проверка Аргументов):** Вместо создания точного объекта `Markup` для сравнения в тесте, проверяйте аргументы, с которыми был вызван метод `ctx.reply` (или аналогичный). Убедитесь, что второй аргумент (`extra`) содержит `reply_markup` с ожидаемой структурой, включая `resize_keyboard: true`.

    ```typescript
    // __tests__/menu/startMenu.test.ts
    const expectedKeyboardStructure = {
      keyboard: [[{ text: expectedButtonText }]],
      resize_keyboard: true, // Проверяем этот флаг
    };
    ...
    expect(mockCtx.reply).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        reply_markup: expectedKeyboardStructure,
      })
    );
    ```

-   **Решение 2 (Улучшенный Мок):** Если необходимо точное поведение, исправьте мок `Markup.keyboard` в `__tests__/setup.ts`, чтобы он возвращал объект с мок-функцией `resize`, которая модифицирует объект и возвращает его же.

    ```typescript
    // __tests__/setup.ts (внутри vi.mock('telegraf', ...))
    // ...
    Markup: {
      // ... другие моки Markup ...
      keyboard: vi.fn().mockImplementation((keyboard) => {
        const keyboardMarkup = {
          reply_markup: { keyboard: keyboard },
          resize: vi.fn().mockImplementation(() => {
            keyboardMarkup.reply_markup.resize_keyboard = true;
            return keyboardMarkup; // Возвращаем себя для цепочки
          }),
          // Добавьте моки .oneTime(), .selective() по аналогии, если нужно
        };
...
-   **Примечание:** Убедитесь, что ручные алиасы для `telegraf` в `vitest.config.mts` не конфликтуют с глобальным моком `vi.mock('telegraf', ...)` в `setup.ts`. Обычно лучше использовать только глобальный мок (за исключением `@telegraf/types`, см. пункт 2).

### 2. `Error: Failed to resolve entry for package "@telegraf/types"`

-   **Проблема:** После удаления ручных алиасов для `telegraf` из `vitest.config.mts` (оставляя только алиас для `@`), тесты падают с ошибкой `Failed to resolve entry for package "@telegraf/types"`, даже если в `__tests__/setup.ts` есть глобальный мок `vi.mock('telegraf', ...)` и установлен плагин `vite-tsconfig-paths`.
-   **Причина:** Точная причина взаимодействия между Vitest/Vite, глобальными моками и механизмом разрешения модулей для пакетов типов (`@telegraf/types`) до конца не ясна. Возможно, Vitest или его зависимости требуют явного указания пути к пакету типов на этапе разрешения, даже если основной модуль `telegraf` мокается глобально. `vite-tsconfig-paths` не всегда достаточно для таких случаев.
-   **Решение:** Необходимо **сохранить** ручной алиас **именно для `@telegraf/types`** в секции `resolve.alias` файла `vitest.config.mts`, даже при наличии глобального мока `telegraf`. Этот алиас должен указывать на корректный файл моков (например, `__tests__/mocks/typegram.mock.ts`). Остальные ручные алиасы для `telegraf/*` (например, `telegraf/scenes`) **следует убрать**, чтобы они не конфликтовали с глобальным моком.
      ...
      alias: [
        ...
        // НЕОБХОДИМЫЙ АЛИАС:
        {
          ...
          replacement: path.resolve(
            __dirname,
            '__tests__/mocks/typegram.mock.ts' // Убедитесь, что мок существует
          ),
        },
        // Другие алиасы для telegraf/* здесь НЕ НУЖНЫ, если есть vi.mock('telegraf', ...)
      ],
    },
    ```

-   **Примечание:** Эта проблема отличается от ошибки `TypeError: Cannot read properties of undefined (reading 'resize')`, которая решается улучшением мока для `Markup.keyboard` (см. пункт 1).
