...
После того как вы предоставите всю необходимую информацию, наш бот обработает данные и создаст ваш аватар. Вы получите подтверждение с перечислением введенной информации: название компании, должность и навыки. Если все данные введены корректно, вы увидите сообщение о том, что аватар успешно создан. Если на каком-либо этапе вы не предоставите необходимую информацию, бот уведомит вас об ошибке и предложит повторить ввод данных. Мы стремимся сделать процесс создания аватара максимально простым и удобным для вас.
...
Проект имеет публичный API, доступный по адресу `/api`.

### Доступные эндпоинты

- **GET /api** - базовый Hello World эндпоинт, возвращает приветственное сообщение
...
Проект использует Nginx для проксирования запросов к API и обработки статических файлов. В проекте используются различные конфигурации Nginx для разных окружений:

### Файлы конфигурации

1. **nginx.conf** (корневой) - используется для разработки и тестирования:
   - Монтируется в `/etc/nginx/nginx.conf` в Docker-контейнере во время разработки и тестирования
   - Содержит настройки для проксирования API запросов, обработки webhooks и загрузки файлов
   - Используется в `docker-compose.dev.yml` и `docker-compose.test.yml`

2. **configs/nginx.conf** - используется в продакшене:
   - Копируется в `/etc/nginx/http.d/default.conf` при сборке Docker-образа в Dockerfile
   - Содержит идентичные настройки корневому файлу
   - Обеспечивает единообразие конфигурации в различных средах

3. **roles/nginx/templates/nginx.conf.j2** - шаблон для Ansible:
   - Используется для автоматизированного развертывания с помощью Ansible
   - Содержит параметризированные настройки для различных серверов
   - Поддерживает как HTTP, так и HTTPS с SSL-сертификатами

### Основные настройки Nginx

- Проксирование API запросов на порт 2999 с сохранением заголовков
- Специальная обработка webhook'ов от системы оплаты Robokassa
- Увеличенный лимит размера загружаемых файлов до 100MB для `/uploads`
- Блокирование распространенных уязвимостей через регулярные выражения
- Конфигурации для различных сред (разработка, тестирование, продакшн)
...
## Типизация платежной системы

Платежная система использует строгую типизацию для обеспечения безопасности и надежности операций с балансом пользователей.
...
### Тесты платной смены моделей (SelectModelWizard)

Для тестирования платежной интеграции с SelectModelWizard:
...
Для запуска всех тестов в Docker-контейнере:

```bash
npm run docker:test:build  # Сборка Docker-контейнера для тестов
npm run docker:test:up     # Запуск контейнера с тестами
npm run docker:test:logs   # Просмотр логов выполнения тестов
```
...
## Важная информация о платежном процессоре

Централизованный платежный процессор обеспечивает безопасность и согласованность всех транзакций в системе.

### Использование платежного процессора

```typescript
// Правильный способ обработки платежа
await inngest.send({
  name: 'payment/process',
  data: {
    telegram_id: "123456789",
    amount: 100, // ВСЕГДА положительное число
    type: TransactionType.MONEY_EXPENSE, // Тип определяет направление операции
...
### Важные правила:
- amount и stars ВСЕГДА должны быть положительными числами
- Направление транзакции определяется типом (MONEY_INCOME/MONEY_EXPENSE), а не знаком суммы
- Перед списанием всегда проверяется достаточность баланса
- Уведомления о транзакциях отправляются автоматически

## Система отправки медиафайлов Pulse

Проект включает в себя систему мониторинга и отправки медиа-контента в канал @neuro_blogger_pulse, который используется для контроля качества и демонстрации результатов работы нейросетей.

### Модуль Pulse

Модуль `src/helpers/pulse.ts` предоставляет функции для отправки различных типов медиафайлов в канал мониторинга:

#### Основные функции:

1. **pulse** - базовая функция для отправки изображений (устаревший метод)
   - Поддерживает как строковые параметры, так и объект с опциями
   - Используется в существующих модулях для обратной совместимости

2. **sendMediaToPulse** - универсальная функция для отправки всех типов медиафайлов
   - Поддерживает фото, видео, аудио и документы
   - Работает как с URL, так и с локальными файлами
   - Формирует информативные подписи на русском и английском языках
   - Включает метаданные о пользователе и контенте
...
await sendMediaToPulse({
  mediaType: 'video',
  mediaSource: '/path/to/local/video.mp4',
  telegramId: '123456789',
  username: 'username',
  language: 'ru',
  serviceType: 'TextToVideo',
  prompt: 'Летящий орел в горах',
  botName: 'neuro_blogger_bot'
});
```

### Интеграция с Inngest-функциями

Система Pulse интегрирована в Inngest-функции для автоматической отправки генерируемого контента в канал мониторинга:

- **textToImage.inngest.ts** - отправка сгенерированных изображений
- **textToVideo.inngest.ts** - отправка сгенерированных видео
- **imageToVideo.inngest.ts** - отправка видео, созданных из изображений
- **neuroImageGeneration.ts** - отправка нейрофото

### Тестирование

Для тестирования функциональности Pulse предусмотрены специальные тесты и утилиты:

- `src/test-utils/tests/pulse/pulseMediaTest.ts` - тесты отправки различных типов медиа
- `src/test-utils/runPulseMediaTests.ts` - запуск всех тестов Pulse
- `scripts/run-pulse-tests.sh` - bash-скрипт для запуска тестов из командной строки

### Запуск тестов

```bash
# Запуск тестов Pulse
./scripts/run-pulse-tests.sh
