    ...
    match state.step {
        ...
        2 => {
            if let Some(text) = msg.text() {
                ...
                if let Err(err_msg) = check_balance(&db, tid, HEYGEN_RENDER_COST, lang).await {
                ...
                if !deduct_balance(&db, tid, HEYGEN_RENDER_COST).await {
                    tracing::error!(telegram_id = tid, "Failed to deduct balance for heygen render");
