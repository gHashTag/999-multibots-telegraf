...
impl RobokassaGateway {
    ...
    pub fn generate_signature(&self, amount: f64, inv_id: &str) -> String {
        use hmac::{Hmac, Mac};
        use sha2::Sha256;
...
impl PaymentGateway for RobokassaGateway {
    ...
    async fn verify_callback(&self, params: &serde_json::Value) -> Result<PaymentVerification, AppError> {
        Ok(PaymentVerification {
            transaction_id: params["InvId"].as_str().unwrap_or_default().to_string(),
            amount: params["OutSum"].as_str().and_then(|v| v.parse().ok()).unwrap_or(0.0),
            currency: "RUB".into(),
