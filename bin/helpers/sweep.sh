...
#
# Команда собирает исходы обходов и считает их по видам. `busy` — признак
# застрявшего флага: предыдущий обход не вернулся. `held` — наоборот, норма:
# пока карточка жива, новая стёрла бы её вместе с оплаченной картинкой.
#
...
if by_did.get('failed'):
    ...
    raise SystemExit(1)
# УДЕРЖАНИЕ И «ЗАНЯТО» — РАЗНЫЕ ЗВЕРИ, И СОВЕТ У НИХ РАЗНЫЙ.
#
# Эта проверка складывала их в одну кучу и на перевес говорила «тормоз в
# коде». Совет устарел в тот день, когда появился сторож «прошлая карточка
# ещё жива»: с ним удержание СТАЛО преобладать — по замыслу. Карточка живёт
# двенадцать часов, тик ходит раз в полчаса, значит на одну карточку
# приходится до двух десятков законных удержаний.
#
# То есть инструмент отправлял искать поломку, которой нет. `busy` —
# по-прежнему настоящий признак: он значит, что предыдущий обход не вернулся.
held = by_did.get('held', 0)
busy = by_did.get('busy', 0)
if busy:
    print('  🛑 «busy» (%d): предыдущий обход не вернулся — застрял флаг.' % busy)
    print('     Смотреть STUCK_SWEEP_MS и то, на чём обход висит.')
    raise SystemExit(1)
if held and not cards:
    print('  · только удержания (%d), карточек нет.' % held)
    print('    Это НЕ поломка: пока карточка жива, новая стёрла бы её вместе с')
    print('    оплаченной картинкой. Карточка живёт 12 ч, тик — раз в полчаса,')
    print('    поэтому на одну карточку приходится до двух десятков удержаний.')
    print('    Тревожиться стоит, если удержания идут, а нажатий нет СУТКАМИ:')
    print('    смотреть tri orphans и нажимал ли владелец хоть что-то.')
    raise SystemExit(0)
