...
#
# СНАЧАЛА ЛОГ, ПОТОМ ЖУРНАЛ — И ЭТО ИСПРАВЛЕНИЕ СОБСТВЕННОЙ ОШИБКИ.
#
# Я построил эту команду на журнале улья, где `held` не пишется, и потому она
# умела только рассуждать о пределах. А ответ всё это время лежал в логе
# Railway: обход пишет туда КАЖДЫЙ исход, включая удержание.
#
# 17.09.2026, 20:55 UTC: журнал молчал четыре часа пятьдесят пять минут, и я
# успел сосчитать по формуле отсрочки, что это похоже на выключенный крон.
# Одна строка лога опровергла:
#
#   20:30:00  144022504  held  прошлая карточка ещё жива — новая стёрла бы её
#
# Тик прошёл, продавец жив, карточка держится законно. Догадка стоила витка;
# лог отвечал сразу. Поэтому: если лог доступен — судить по нему, а журнал
# оставить на случай, когда лога нет (он обрывается на последнем деплое).
#
...
# ЛОГ БОТА — ПЕРВЫЙ ИСТОЧНИК. Он знает про `held`, чего журнал не знает.
# Пусто — не ответ: лог живёт с последнего деплоя и после свежего деплоя
# короткий. Тогда молча переходим к журналу, сказав об этом вслух.
LOG=$( (railway service 999-multibots-telegraf >/dev/null 2>&1; \
        railway logs 2>/dev/null) | grep -F '[crm-proactive] sweep' | tail -40 )
if [ -n "$LOG" ]; then
  LAST=$(printf '%s\n' "$LOG" | tail -1)
  WHEN=$(printf '%s' "$LAST" | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}[ T][0-9]{2}:[0-9]{2}:[0-9]{2}' | head -1)
  DID=$(printf '%s' "$LAST" | grep -oE '"did":"[a-z]+"' | cut -d'"' -f4)
  echo "── по логу бота (знает про held, в отличие от журнала) ──"
  echo "   последний обход: ${WHEN:-время не разобрано}, исход: ${DID:-не разобран}"
  printf '%s\n' "$LOG" | grep -oE '"did":"[a-z]+"' | cut -d'"' -f4 | sort | uniq -c \
    | sed 's/^/     /'
  echo
  echo "   Живой тик с исходом «held» — это НЕ тишина: продавец работает и"
  echo "   держит паузу намеренно. Тревога — только если строк нет вовсе."
  echo
else
  echo "── лога обхода нет (свежий деплой или недоступен) — иду в журнал ──"
  echo
fi
