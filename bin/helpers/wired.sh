...
echo
# ВЫЗОВЫ ВНУТРИ СВОЕГО ЖЕ ФАЙЛА — ЭТО ТОЖЕ ПРОВОДКА.
#
# Первая версия считала только вызовы ИЗВНЕ и на функцию, которую зовёт сосед
# по файлу, печатала «НИКТО». 16.09.2026 так вышло с `intentSignals`: её зовёт
# `leadContext` двумя строками ниже, а команда объявила её мёртвой — и я едва
# не пошёл чинить проводку, которой ничего не нужно. Ложная тревога дороже
# молчания: она отправляет работать не туда.
INSIDE=$(printf '%s\n' "$CALLS" |
  grep -E "$NAME *\(" |
  grep -vE "(export (async )?function|export const|function) +$NAME\b" || true)
IN_N=$(printf '%s' "$INSIDE" | grep -c . || true)

echo "── зовут из рабочего пути ──"
N=$(printf '%s' "$OUTSIDE" | grep -c . || true)
if [ "${N:-0}" = "0" ] && [ "${IN_N:-0}" != "0" ]; then
  echo "  Только ВНУТРИ объявляющего файла — это проводка, но локальная:"
  printf '%s' "$INSIDE" | sed 's/^/    /'
  echo
  echo "  Не мёртвый код. Но если этот файл сам никто не зовёт, ищите выше:"
  echo "     tri wired <имя того, кто зовёт отсюда>"
  exit 0
fi
if [ "${N:-0}" = "0" ]; then
  echo "  НИКТО."
