-- Удаляем старые триггеры сначала
DROP TRIGGER IF EXISTS sync_balance_trigger ON payments CASCADE;
DROP TRIGGER IF EXISTS sync_balance_trigger_v2 ON payments_v2 CASCADE;

-- Теперь удаляем функции
DROP FUNCTION IF EXISTS get_user_balance(BIGINT) CASCADE;
DROP FUNCTION IF EXISTS get_user_balance(TEXT) CASCADE;
DROP FUNCTION IF EXISTS sync_user_balance() CASCADE;
DROP FUNCTION IF EXISTS add_stars_to_balance(BIGINT, INTEGER, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS add_stars_to_balance(BIGINT, INTEGER, TEXT, TEXT, TEXT, TEXT) CASCADE;

-- 1. Создаем функцию get_user_balance
...
BEGIN
    ...
    IF telegram_id_numeric IS NULL THEN
        SELECT COALESCE(
            SUM(CASE WHEN type = 'money_income'::operation_type THEN stars ELSE -stars END),
            0
        ) INTO result
        FROM payments_v2 
...
-- 2. Создаем функцию sync_user_balance
...
BEGIN
    ...
    UPDATE users
    SET balance = (
        SELECT COALESCE(SUM(CASE WHEN type = 'money_income'::operation_type THEN stars ELSE -stars END), 0)
        FROM payments_v2
...
-- 5. Создаем перегруженную версию add_stars_to_balance с 6 параметрами для совместимости
CREATE OR REPLACE FUNCTION add_stars_to_balance(
    p_telegram_id bigint,
    p_stars integer,
    p_description text,
