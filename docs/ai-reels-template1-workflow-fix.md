...
### Step 3: Генерация WAN 2.5 (строки 1013-1195)
- Читает `ctx.session.aiReels.firstVideoUrl` из Step 2
- Генерирует второе видео через Kie.ai WAN 2.5
...
Step 2: Генерация lip-sync (Fal.ai) СИНХРОННО
       ...
       ctx.wizard.next()
         ↓
Step 3: Генерация WAN 2.5 (Kie.ai) АСИНХРОННО
         ├─ Создание задачи в WAN 2.5
...
Бот: ✅ Второе видео (WAN 2.5) готово!
...
- WAN 2.5 image-to-video (5 сек): ~160⭐
...
- ✅ WAN 2.5 видео генерируется за 5-10 минут
