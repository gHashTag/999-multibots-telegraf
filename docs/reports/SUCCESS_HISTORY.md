...
## 2025-08-14: КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ - Полное восстановление платежей Robokassa

### Проблема
Платежи в рублях через Robokassa не работали около месяца. Основные ошибки:
1. `TypeError: signature.toLowerCase is not a function` при валидации подписи
2. Неправильное формирование подписи (включался ResultURL)
3. Использовался нестандартный параметр ResultUrl2

### Комплексное решение

1. **Исправлена валидация подписи** в `src/core/robokassa/index.ts`:
   ```typescript
   // Добавлена защита от неправильных типов
   const signatureStr = String(signature)
   const isValid = calculatedSignature.toLowerCase() === signatureStr.toLowerCase()
   ```

2. **Исправлено формирование подписи** в `src/scenes/getRuBillWizard/helper.ts`:
   ```typescript
   // Правильный формат БЕЗ ResultURL
   const signatureValue = md5(
     `${merchantLogin}:${outSum}:${invId}:${password1}`
   ).toUpperCase()
   ```

3. **Исправлен параметр URL**:
   - Было: `ResultUrl2` (нестандартный)
   - Стало: `ResultURL` (стандартный параметр Robokassa)

### Результат
- ✅ Ссылки на оплату открываются корректно в браузере
- ✅ Подпись формируется правильно согласно документации
- ✅ Webhook корректно отправляется на ai-server
- ✅ Полностью восстановлена работа платежей в рублях
- ✅ Созданы тестовые скрипты для проверки

### Тестирование
Создано 2 скрипта для генерации и тестирования ссылок:
- `scripts/generate-robokassa-link.ts` - генерация тестовых ссылок
- `scripts/test-real-payment-link.ts` - ссылки для реальных подписок

### Коммиты
- Коммит: 7884c6c2 - Исправление signature.toLowerCase
- Коммит: d4e53fc2 - Исправление формирования подписи и параметров
- PR #214 слит в main: 9bc688ad
