...
1.  🌐 **ЗАВЕРШИТЬ ЦЕНТРАЛИЗАЦИЮ ЯЗЫКОВОЙ СИСТЕМЫ** - осталось ~70+ мест использования старой системы
    - `src/scenes/levelQuestWizard/handlers.ts` - завершить централизацию (40+ мест)
    - `src/scenes/imageToVideoWizard/index.ts` - полная централизация
    - Все остальные сцены и компоненты с `ctx.from?.language_code === 'ru'`
2.  🧪 **Полное тестирование языковой системы** - переключение языка во всех сценах
3.  🧪 **Протестировать работу "Фото в видео"** (`imageToVideoWizard`) в обоих режимах.
4.  🧪 **Протестировать работу "Текст в фото"** (`textToImageWizard`).
5.  ❗ **Обновить URL вебхуков в Telegram** на формат `https://<домен>/<имя_бота>`.
6.  ❗ **Обновить URL Robokassa Result** на `https://<домен>/api/robokassa-result`.
7.  ❗ **Реализовать/проверить эндпоинт `/api/generate/image-to-video`** в `api-server` (валидация, вызов Replicate с ВЕБХУКОМ, ответ 202).
8.  ❗ **Реализовать/проверить эндпоинт `/api/replicate-webhook`** в `api-server`.
9.  🧪 **Протестировать работу API сервера** по адресу `https://<домен>/api` (генерация, вебхук, Robokassa).
10. 🛠️ Скопировать актуальные интерфейсы в `api-server`.
11. 🛠️ Реализовать заглушки `updateUserBalance`, `updateUserSubscription` в `api-server`.
12. 🚀 **ОЧИСТИТЬ КЭШ DOCKER НА СЕРВЕРЕ** и выполнить деплой с `--no-cache`.
13. 🔑 Проверить валидность всех токенов ботов.
...
## 📈 Прогресс
- Централизация Языковой Системы: 85% 🔄 (осталось ~70+ мест)
- Деплой: 95% ⏳
- Типизация: 98% ✅
- Тестирование: 40% ⏳
- Инфраструктура: 100% ✅
...
- ✅ 2025-01-30: **ЗАВЕРШЕНА ОСНОВНАЯ ЦЕНТРАЛИЗАЦИЯ ЯЗЫКОВОЙ СИСТЕМЫ** - создан middleware, исправлено 32+ файла
- ✅ 2025-01-30: Создан `languageMiddleware.ts` - оптимизация производительности с 4+ запросов до 1 запроса к БД
- ✅ 2025-01-30: Полностью переписаны `registerCommands.ts` (19 мест) и `hearsHandlers.ts` (8 мест)
- ✅ 2025-01-30: Исправлена ошибка "message text is empty" в subscription scene
- ✅ 2025-01-30: Добавлена кнопка переключения языка в главное меню с сохранением в БД
- ✅ 2025-01-30: Устранено дублирование кнопок языка и несогласованность переводов
