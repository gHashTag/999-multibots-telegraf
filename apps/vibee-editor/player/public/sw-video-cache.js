...
self.addEventListener('activate', (event) => {
  ...
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter((name) => name.startsWith('vibee-video-cache-') && name !== CACHE_NAME)
          .map((name) => {
...
async function enforceQuota() {
  try {
    ...
    if (totalSize > MAX_CACHE_SIZE) {
      console.log(`[SW] Cache over limit: ${(totalSize / 1024 / 1024).toFixed(1)}MB > ${MAX_CACHE_SIZE / 1024 / 1024}MB`);
      ...
      while (totalSize > targetSize && entries.length > 0) {
        ...
        console.log(`[SW] Evicted: ${oldest.url} (${(oldest.size / 1024 / 1024).toFixed(1)}MB)`);
      }

      console.log(`[SW] Cache after eviction: ${(totalSize / 1024 / 1024).toFixed(1)}MB`);
    }
  } catch (error) {
