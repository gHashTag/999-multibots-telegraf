 ...
 */
export function recordStartupTime(
  sessionId: string,
  startupTime: number
): void {
 ...
 */
export function recordBuffering(
  sessionId: string,
 ...
 */
export function recordBitrateSwitch(
  sessionId: string,
  switchEvent: BitrateSwitch
): void {
  ...
  if (switches && metrics) {
    ...
    if (allBitrates.length > 0) {
      metrics.avgBitrate =
        allBitrates.reduce((a, b) => a + b, 0) / allBitrates.length
    ...
    console.log(
      '[QoE] Bitrate switch:',
      switchEvent.fromBitrate,
      '->',
      switchEvent.toBitrate
    )
  }
}
...
export function calculateQoEScore(metrics: Partial<QoEMetrics>): number {
  ...
  // Base score: 5
  const score =
    5 - startupPenalty - bufferPenalty - switchPenalty + bitrateFactor
