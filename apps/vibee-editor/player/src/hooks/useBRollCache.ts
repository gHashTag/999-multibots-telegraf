...
export function useBRollCache(options: BRollCacheOptions = {}) {
  ...
  const initDB = useCallback((): Promise<IDBDatabase> => {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => reject(new Error('Failed to open B-Roll cache DB'));
  ...
  const loadCachedEntries = useCallback(async () => {
    try {
      ...
      return new Promise<void>((resolve) => {
        ...
        request.onsuccess = () => {
          const now = Date.now();
          const entries = (request.result as CachedBRoll[])
  ...
  const cacheVideos = useCallback(async (urls: string[]): Promise<void> => {
    const uncachedUrls = urls.filter((url) => url && !url.startsWith('blob:') && !state.cachedUrls.has(url));
  ...
  const touchEntry = useCallback(async (url: string) => {
    try {
      const db = await initDB();
      const transaction = db.transaction(STORE_NAME, 'readwrite');
      const store = transaction.objectStore(STORE_NAME);
