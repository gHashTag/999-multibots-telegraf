...
export type { FeedTemplate, RemixSource, PublishData, FeedSort, FeedType, FeedStats };
...
async function publishTemplate(data: PublishData, userInfo: PublishUserInfo): Promise<FeedTemplate> {
  ...
  console.log('[Feed] Publish response status:', response.status);

  if (!response.ok) {
    const errorText = await response.text();
    console.error('[Feed] Publish error:', errorText);
    throw new Error(`Failed to publish template: ${response.status} ${errorText}`);
  }

  const result = await response.json();
  console.log('[Feed] Publish success:', result);
...
export const publishToFeedAtom = atom(
  null,
  async (get, set, data: Omit<PublishData, 'templateSettings' | 'assets' | 'tracks'> & { templateSettings?: TemplateSettings; postToTelegram?: boolean; postToInstagram?: boolean; telegramCaption?: string }) => {
    ...
    try {
      ...
      const fullData: PublishData = {
        ...data,
        templateSettings: (data.templateSettings || {}) as Record<string, unknown>,
      ...
      console.log('[Feed] Published:', template.name, 'with remix:', remixSource?.templateName);
      return template;
    } catch (error) {
...
// Load stats action
export const loadStatsAtom = atom(
  null,
  async (get, set) => {
