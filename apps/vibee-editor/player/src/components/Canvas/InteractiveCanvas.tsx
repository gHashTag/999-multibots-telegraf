...
export function InteractiveCanvas() {
  ...
  const splitTalkingHeadPropsRaw = useMemo(
    () => convertToSplitTalkingHeadProps(lipSyncPropsWithUrls, project.durationInFrames, project.fps, videoTrackItems, imageTrackItems, assets, avatarSettingsTab),
    [lipSyncPropsWithUrls, project.durationInFrames, project.fps, videoTrackItems, imageTrackItems, assets, avatarSettingsTab]
  );
  ...
  const splitTalkingHeadProps = useMemo(
    () => {
      const props = {
        ...
        backgroundMusic: audioTrackUrl || splitTalkingHeadPropsBase.backgroundMusic,
  ...
  const brollUrls = useMemo(() => {
    return splitTalkingHeadPropsBase.segments
      ?.filter((s: { type: string; bRollUrl?: string }) => s.type === 'split' && s.bRollUrl)
      .map((s: { bRollUrl: string }) => s.bRollUrl) || [];
  ...
  useEffect(() => {
    ...
    if (lipSyncPropsRaw.lipSyncVideo && !lipSyncPropsRaw.lipSyncVideo.startsWith('blob:')) {
    ...
    prefetchVideosParallel(allVideoUrls, {
      ...
      console.log(`[Prefetch] ${results.length} videos: ${cached} cached, ${prefetched} prefetched, avg ${avgDuration.toFixed(0)}ms`);
    });

    // Also preload audio separately
    if (lipSyncPropsRaw.backgroundMusic && !lipSyncPropsRaw.backgroundMusic.startsWith('blob:')) {
      preloadAudio(lipSyncPropsRaw.backgroundMusic);
    }
  }, [lipSyncPropsRaw.lipSyncVideo, lipSyncPropsRaw.backgroundMusic, brollUrls, brollCache.isCached]);
