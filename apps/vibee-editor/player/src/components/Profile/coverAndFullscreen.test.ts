...
describe('перевод SOUL — одним нажатием и в поле, а не в базу', () => {
  ...
  it('кнопка есть и зовёт свой маршрут с личностью', () => {
    ...
    /*
     * ПРОВЕРКА БЫЛА ПЕРЕВЁРНУТА: она ТРЕБОВАЛА дефект.
     *
     * `{...authHeaders()}` даёт пустой объект — `Headers` не разворачивается
     * спредом, данные лежат внутри объекта, а не в собственных свойствах.
     * Запрос уходил ровно с `Content-Type`, без удостоверения, и сервер
     * отвечал 401: кнопка нажималась, ничего не происходило, причины видно
     * не было.
     *
     * Тест при этом закреплял именно эту форму — то есть охранял поломку.
     */
    expect(РЕДАКТОР).not.toContain('...authHeaders()')
    expect(РЕДАКТОР).toContain("authHeaders({ 'Content-Type': 'application/json' })")
  })
