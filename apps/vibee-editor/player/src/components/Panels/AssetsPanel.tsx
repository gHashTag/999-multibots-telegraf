...
export function AssetsPanel({ filterType }: AssetsPanelProps) {
    ...
    async (files: FileList | null) => {
      ...
      for (const file of Array.from(files)) {
        ...
        if (file.type.startsWith('video/')) {
          ...
          type = filterType === 'music' ? 'music' : filterType === 'voice' ? 'voice' : 'audio';
        } else {
          continue;
        }
  ...
  const handleAddToTimeline = (asset: Asset) => {
    ...
    const itemType = (asset.type === 'music' || asset.type === 'voice') ? 'audio' : asset.type;
    ...
    addItem({
      ...
      itemData: {
        ...
        ...((asset.type === 'audio' || asset.type === 'music' || asset.type === 'voice') && { volume: 1 }),
      },
    });
  };
  ...
  const musicAssets = assets.filter((a) => a.type === 'music' || a.type === 'audio'); // Music includes legacy audio
  ...
  const getAcceptType = () => {
    ...
    if (filterType === 'audio' || filterType === 'voice' || filterType === 'music') return 'audio/*';
  ...
  return (
    <div className="assets-panel">
      ...
      {(!filterType || filterType === 'video' || filterType === 'lipsync') && videoAssets.length > 0 && (
      ...
      {/* Music - Grid View */}
      {(!filterType || filterType === 'audio' || filterType === 'music') && musicAssets.length > 0 && (
        <div className="asset-group">
          {!filterType && (
            <h3 className="asset-group-title">
              {t('generate.music')}
      ...
      {(filterType === 'video' || filterType === 'lipsync') && videoAssets.length === 0 && (
