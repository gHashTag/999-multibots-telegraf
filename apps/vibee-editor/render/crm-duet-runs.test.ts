...
function deps(persist: DuetDeps['persist']): DuetDeps {
  let s = 0
  const script = [
    [text('Здравствуйте! Где ваша аудитория?')],
    [text('Понял, вот план.')],
  ]
  return {
    agent: () =>
...
describe('persist', () => {
  ...
  it('upsertRun writes every field once and updates on conflict', async () => {
    ...
    await upsertRun(
      pool,
      freshRun({ coverage: { reel_render: { calls: 1, ok: 1, fail: 0 } } })
    )
    ...
    expect(ins.sql).toMatch(
      /INSERT INTO crm_duet_runs .* ON CONFLICT \(id\) DO UPDATE/
    )
    expect(ins.params.slice(0, 3)).toEqual(['duet-persist', OWNER, BUYER])
    expect(ins.params[11]).toBe(
      JSON.stringify({ reel_render: { calls: 1, ok: 1, fail: 0 } })
    )
  })
})
...
describe('status falls back to the table', () => {
  ...
  it('findRun reads the table when the Map has nothing, by id and as the latest', async () => {
    ...
    expect(seen.some(s => /WHERE id = \$1 AND owner_id = \$2/.test(s))).toBe(
      true
    )
    ...
    expect(
      seen.some(s =>
        /WHERE owner_id = \$1 ORDER BY started_at DESC LIMIT 1/.test(s)
      )
    ).toBe(true)
  })
  ...
  it('listRuns scopes to the owner and the buyer, and carries `lines` instead of the transcript', async () => {
    ...
    const pool = {
      query: async (sql: string, params: unknown[] = []) => {
        ...
        return /SELECT/.test(sql)
          ? { rows: [{ ...row, transcript: undefined, lines: 8 }] }
          : { rows: [] }
      },
    }
    ...
    expect(summaryOf(runs[0], 10_000)).toMatchObject({
      id: 'duet-old',
      buyer: BUYER,
      state: 'done',
      lines: 8,
...
describe('lost is derived on read', () => {
  it('a running run older than 30 minutes reads as lost; the stored state stays running', () => {
    ...
    expect(viewOf(freshRun({ state: 'done' }), LOST_AFTER_MS * 10).state).toBe(
      'done'
    )
