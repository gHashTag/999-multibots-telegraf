...
function подписать(поля: Record<string, string>): string {
  ...
  const секрет = crypto.createHmac('sha256', 'WebAppData').update(ТОКЕН).digest()
    ...
    async query(sql: string, params: unknown[] = []) {
      ...
      if (s.includes('UPDATE app_pairing_codes SET consumed_at = now() WHERE telegram_id')) {
        for (const r of строки) {
          if (r.t === 'code' && r.telegram_id === params[0] && !r.consumed_at) r.consumed_at = 1
        }
        return { rows: [] }
      }
      if (s.startsWith('INSERT INTO app_pairing_codes')) {
        строки.push({
          t: 'code', code_hash: params[0], telegram_id: params[1],
          expires_at: Date.parse(String(params[2])), consumed_at: null, attempts: 0,
        })
        return { rows: [] }
      }
      if (s.startsWith('SELECT telegram_id, expires_at, consumed_at, attempts')) {
        return { rows: строки.filter(r => r.t === 'code' && r.code_hash === params[0]) }
      }
      ...
      if (s.includes('SET consumed_at = now() WHERE code_hash')) {
        const out: any[] = []
        for (const r of строки) {
          if (r.t === 'code' && r.code_hash === params[0] && !r.consumed_at
...
function запрос(url: string, тело: unknown, заголовки: Record<string, string> = {}) {
...
describe('вход по коду: сквозной путь', () => {
  ...
  it('подпись → код → сессия: код, выданный одним запросом, принимает другой', async () => {
    ...
    await handleAuthRoute(
      запрос('/api/auth/pair/start', {}, { 'x-telegram-init-data': подписанные(4242) }),
      о1, пул as any
    )
  ...
  it('подпись читается из ЗАГОЛОВКА — так ходит весь мини-апп', async () => {
    const о = ответ()
    await handleAuthRoute(
      запрос('/api/auth/pair/start', {}, { 'x-telegram-init-data': подписанные(7) }),
      о, пул as any
    )
  ...
  it('подделанная подпись кода не даёт', async () => {
    ...
    const о = ответ()
    await handleAuthRoute(запрос('/api/auth/pair/start', {}, {
  ...
  it('код второй раз не проходит: одноразовость держится на всём пути', async () => {
    ...
    const о2 = ответ()
    await handleAuthRoute(запрос('/api/auth/pair/claim', { code: код }), о2, пул as any)
    expect(о2.код).toBe(200)
