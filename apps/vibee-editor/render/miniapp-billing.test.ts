...
describe('mini-app generation is billed', () => {
  ...
  it('quantity is measured, not a literal, wherever the provider bills per unit', () => {
    ...
    /*
     * Условие расширилось: посекундно платим ТОЛЬКО когда длину задаём мы.
     * Четыре видеомодели просят один `prompt` — длительность до провайдера не
     * доходит, и умножение на неё было платой за незаказанное.
     */
    expect(SERVER).toMatch(/посекунднаяМодель\(model\) && длинойУправляемМы/)
