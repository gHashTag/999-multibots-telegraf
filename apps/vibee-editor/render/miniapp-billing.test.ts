...
describe('mini-app generation is billed', () => {
  ...
  it('quantity is measured, not a literal, wherever the provider bills per unit', () => {
    ...
    /*
     * Условие расщепилось НАДВОЕ, и это было следующей дырой.
     *
     * Сначала пинилось общее `длинойУправляемМы`, куда входил и `video_url`.
     * Под него попал `topaz/video-upscale`: длину ему задаёт присланный
     * РОЛИК, а счёт умножался на чип «6с/10с», который до провайдера не
     * доходит вовсе. Один и тот же файл стоил 96 или 160 токенов от
     * положения переключателя, менявшего ровно ничего.
     *
