...
describe('секрет кассира не попадает в лог', () => {
  ...
  it('секрет сравнивается с сегментом пути — то есть он и есть охрана', () => {
    // If this stops being true the redaction is still correct, but the reason
    // it matters has changed and this file should be re-read.
    expect(SERVER).toMatch(/whMatch\[1\] !== WH_SECRET/)
  })
})
