...
describe('crm_client_plan', () => {
  it('aggregates items per goal and in total, by status', () => {
    const out = aggregatePlan(
      ...
      [
        { id: 1, goal_id: 1, title: 'a', status: 'idea', template_id: 'LeelaPlanReel', updated_at: 't' },
        { id: 2, goal_id: 1, title: 'b', status: 'done', template_id: null, updated_at: 't' },
        { id: 3, goal_id: 1, title: 'c', status: 'draft', template_id: null, updated_at: 't' },
        { id: 4, goal_id: 1, title: 'd', status: 'done', template_id: null, updated_at: 't' },
      ]
    )
    expect(out.total).toBe(4)
    expect(out.done).toBe(2)
    expect(out.goals[0].items).toEqual({ total: 4, done: 2, by_status: { idea: 1, done: 2, draft: 1 } })
  ...
  it('reads both tables for the given client only, after the gate', async () => {
    ...
    const pool = {
      query: async (sql: string, params: unknown[] = []) => {
        ...
        if (/content_plan_goals WHERE/.test(sql)) return { rows: [{ id: 1, title: 'g', intent: null }] }
        if (/content_plan_items WHERE/.test(sql))
          return { rows: [{ id: 1, goal_id: 1, title: 'i', status: 'done', template_id: null, updated_at: 't' }] }
        return { rows: [] }
      },
    }
    const tool = CRM_CLIENT_WORKSPACE_TOOLS.find(t => t.name === 'crm_client_plan')!
    const out = (await tool.handler({ telegram_id: CLIENT }, { telegramId: OWNER, pool } as unknown as ToolContext)) as any
    expect(out).toMatchObject({ telegram_id: CLIENT, total: 1, done: 1 })
    for (const q of seen.filter(s => /WHERE telegram_id/.test(s.sql))) expect(q.params[0]).toBe(CLIENT)
    await expect(tool.handler({ telegram_id: 'abc' }, { telegramId: OWNER, pool } as unknown as ToolContext)).rejects.toThrow()
  })
})
...
describe('crm_clients', () => {
  it('is the union of profiles and people, one row per person, newest first', () => {
    const rows = mergeClients({
      profiles: [{ telegram_id: CLIENT, client: 'Лила Чакра', updated_at: '2026-09-13T10:00:00Z' }],
      people: [
        { lead_id: CLIENT, first_name: 'Geya', last_name: null, username: 'playom', seen_at: '2026-09-12T10:00:00Z' },
        { lead_id: OTHER, first_name: 'Ivan', last_name: 'P', username: null, seen_at: '2026-09-13T12:00:00Z' },
      ],
      souls: new Set([CLIENT]),
      skills: new Map([[CLIENT, 3]]),
      duets: new Map([[CLIENT, 7]]),
      touches: new Map([[OTHER, [{ kind: 'written', at: '2026-09-13T12:00:00Z' }]]]),
    ...
    const pool = {
      query: async (sql: string, params: unknown[] = []) => {
        ...
        if (/FROM crm_people/.test(sql))
          return { rows: [{ lead_id: CLIENT, first_name: 'Geya', last_name: null, username: 'playom', seen_at: 'x' }] }
        if (/FROM crm_duet_runs|FROM user_soul|FROM user_skills|FROM crm_touches/.test(sql)) throw new Error('no such table')
        return { rows: [] }
      },
    }
    const tool = CRM_CLIENT_WORKSPACE_TOOLS.find(t => t.name === 'crm_clients')!
    const out = (await tool.handler({ limit: 5 }, { telegramId: OWNER, pool } as unknown as ToolContext)) as any
    expect(out.clients).toHaveLength(1)
    expect(out.clients[0]).toMatchObject({ telegram_id: CLIENT, name: 'Geya', has_profile: false, duets: 0 })
