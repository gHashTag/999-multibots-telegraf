...
${записи
  .map(({ м, видимое, цена }) => {
    ...
    const требует = отЧеловека.length
      ...
      : видимое.требует
    return `    Модель(id: "${экр(м.id)}", название: "${экр(видимое.название)}", вид: .${видимое.вид}, живая: ${м.state === 'live'}, требует: "${экр(требует)}", опасная: ${видимое.опасная === true}, ценаUSD: ${ц}, единица: ${е}, входИзвне: ${входИзвне(м)}, нуженИсходник: ${нуженИсходник(м)}),`
  })
  .join('\n')}
  ]
