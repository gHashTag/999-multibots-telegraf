...
vi.mock('./src/agent/chat', () => ({
  runAgent: async function* (_h: unknown, _ctx: unknown, opts: Record<string, unknown>) {
    seenOpts = opts
...
vi.mock('./src/agent/telegram-tools', async importOriginal => ({
  ...
  isSeller: async (ctx: { telegramId: string }) => String(ctx.telegramId) === SELLER,
}))
...
function req(url: string, method: string, body?: unknown) {
  const s = Readable.from([body === undefined ? '' : JSON.stringify(body)]) as any
  ...
  const pool = {
    ...
    async query(sql: string, params: unknown[] = []) {
      ...
      if (/^INSERT INTO agent_messages/i.test(text)) {
        rows.push({ id: rows.length + 1, telegram_id: params[0], role: params[1], content: params[2], surface: params[3], thread: params[4] })
        return { rows: [] }
      }
      if (/^SELECT/i.test(text))
        return { rows: rows.filter(r => r.telegram_id === params[0] && r.thread === params[2]) }
      if (/^DELETE/i.test(text)) {
        const before = rows.length
        const keep = rows.filter(r => !(r.telegram_id === params[0] && r.thread === params[1]))
        rows.length = 0
        rows.push(...keep)
...
const getPool = () => pool
const threadQueries = () => pool.calls.filter(c => /agent_messages/.test(c.sql) && !/^(CREATE|ALTER)/i.test(c.sql))

describe('threadFor', () => {
  it('is self without a client and never touches the pool', async () => {
    expect(await threadFor(undefined, SELLER, getPool)).toEqual({ ok: true, thread: 'self', client: null })
    expect(await threadFor('', SELLER, getPool)).toMatchObject({ thread: 'self' })
    expect(pool.calls.length).toBe(0)
  })
  it('refuses a bad id, the owner as client, and a non-seller', async () => {
    expect(await threadFor('abc', SELLER, getPool)).toEqual({ ok: false, status: 400, error: 'bad client' })
    expect(await threadFor(SELLER, SELLER, getPool)).toEqual({ ok: false, status: 400, error: 'bad client' })
    expect(await threadFor(CLIENT, STRANGER, getPool)).toEqual({ ok: false, status: 403, error: 'not a seller' })
    expect(await threadFor(CLIENT, SELLER, getPool)).toEqual({ ok: true, thread: 'client:' + CLIENT, client: CLIENT })
  })
})
...
describe('the four routes with a client', () => {
  it('chat: 400 / 403 before the stream opens and before any thread read', async () => {
    const bad = res()
    await handleAgentChat(req('/api/agent/chat', 'POST', { messages: [{ role: 'user', content: 'x' }], client: 'nope' }), bad, SELLER, getPool)
    expect(bad.out.code).toBe(400)
    expect(bad.out.body).toEqual({ error: 'bad client' })
    const stranger = res()
    await handleAgentChat(req('/api/agent/chat', 'POST', { messages: [{ role: 'user', content: 'x' }], client: CLIENT }), stranger, STRANGER, getPool)
    expect(stranger.out.code).toBe(403)
    expect(stranger.out.body).toEqual({ error: 'not a seller' })
    const self = res()
    await handleAgentChat(req('/api/agent/chat', 'POST', { messages: [{ role: 'user', content: 'x' }], client: SELLER }), self, SELLER, getPool)
  ...
  it('chat: writes both turns into the client thread and hands the client to the agent', async () => {
    const r = res()
    await handleAgentChat(req('/api/agent/chat', 'POST', { messages: [{ role: 'user', content: 'what about her?' }], client: CLIENT, surface: 'miniapp' }), r, SELLER, getPool)
  ...
  it('chat without a client is unchanged: thread self, no client for the agent', async () => {
    await handleAgentChat(req('/api/agent/chat', 'POST', { messages: [{ role: 'user', content: 'hi' }] }), res(), SELLER, getPool)
  ...
  it('history GET: returns only that thread, every message carrying it', async () => {
    pool.rows.push({ id: 1, telegram_id: SELLER, role: 'user', content: 'mine', surface: 'bot', thread: 'self' })
    pool.rows.push({ id: 2, telegram_id: SELLER, role: 'user', content: 'hers', surface: 'miniapp', thread: 'client:' + CLIENT })
    const r = res()
    await handleAgentHistory(req(`/api/agent/history?limit=50&client=${CLIENT}`, 'GET'), r, SELLER, getPool)
    expect(r.out.code).toBe(200)
    expect(r.out.body.thread).toBe('client:' + CLIENT)
    expect(r.out.body.messages.map((m: any) => [m.content, m.thread])).toEqual([['hers', 'client:' + CLIENT]])
    const s = res()
    await handleAgentHistory(req(`/api/agent/history?client=${CLIENT}`, 'GET'), s, STRANGER, getPool)
  ...
  it('history DELETE with a client clears that thread only', async () => {
    ...
    pool.rows.push({ id: 2, telegram_id: SELLER, role: 'user', content: 'hers', surface: 'miniapp', thread: 'client:' + CLIENT })
    const r = res()
    await handleAgentHistoryDelete(req(`/api/agent/history?client=${CLIENT}`, 'DELETE'), r, SELLER, getPool)
    expect(r.out.code).toBe(200)
    expect(r.out.body).toMatchObject({ ok: true, thread: 'client:' + CLIENT })
    expect(pool.rows.map(x => x.content)).toEqual(['mine'])
    const bad = res()
    await handleAgentHistoryDelete(req('/api/agent/history?client=12', 'DELETE'), bad, SELLER, getPool)
    expect(bad.out.code).toBe(400)
  })

  it('history POST appends into the client thread and refuses a stranger first', async () => {
    const r = res()
    await handleAgentHistoryAppend(req('/api/agent/history', 'POST', { turns: [{ role: 'assistant', content: 'fallback' }], client: CLIENT }), r, SELLER, getPool)
    expect(r.out.body).toMatchObject({ ok: true, stored: 1, thread: 'client:' + CLIENT })
    expect(pool.rows[0].thread).toBe('client:' + CLIENT)
    const before = threadQueries().length
    const s = res()
    await handleAgentHistoryAppend(req('/api/agent/history', 'POST', { turns: [{ role: 'assistant', content: 'x' }], client: CLIENT }), s, STRANGER, getPool)
    expect(s.out.code).toBe(403)
