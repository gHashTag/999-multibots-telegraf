  ...
  return {
    ...
    query: async (sql: string, params: unknown[] = []) => {
      if (/^INSERT INTO hive_events/i.test(sql)) {
        events.push({ kind: params[0], who: params[1], bot: params[2], amount: params[3], what: params[4], severity: params[5] })
      }
      return { rows: [] }
    },
  }
...
describe('a note from the bot lands in the journal', () => {
  it('knows its path and its vocabulary', () => {
    ...
    expect([...NOTABLE_KINDS]).toEqual(['sweep-idle', 'sweep-card', 'sweep-failed'])
  })

  it('writes an allow-listed kind with who, what and severity', async () => {
    const pool = journalPool()
    const out = await handleHiveNote(
      post({ kind: 'sweep-failed', who: '144022504', what: 'ждёт Tim: [[Подпись|tg_send]]', severity: 'attention' }),
      deps(pool)
    )
    expect(out).toEqual({ status: 200, body: { ok: true, recorded: true } })
    expect(pool.events[0]).toMatchObject({ kind: 'sweep-failed', who: '144022504', severity: 'attention' })
  ...
  it('an unknown severity becomes normal; a long note is cut by the journal itself (200)', async () => {
    const pool = journalPool()
    await handleHiveNote(post({ kind: 'sweep-idle', what: 'x'.repeat(500), severity: 'loud' }), deps(pool))
  ...
  it('refuses a Mini App caller, an unknown kind, bad json and GET', async () => {
    const pool = journalPool()
    expect((await handleHiveNote(post({ kind: 'sweep-idle' }), deps(pool, false))).status).toBe(403)
    expect((await handleHiveNote(post({ kind: 'payment' }), deps(pool))).status).toBe(400)
    expect((await handleHiveNote(post('{nope'), deps(pool))).status).toBe(400)
    expect((await handleHiveNote({ method: 'GET' }, deps(pool))).status).toBe(405)
