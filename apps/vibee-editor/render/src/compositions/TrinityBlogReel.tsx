...
const Frame: React.FC<{ inset?: number }> = ({ inset = 54 }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={line({ left: inset, right: inset, top: inset, height: 1, transform: `scaleX(${draw})` })} />
      <div style={line({ left: inset, right: inset, bottom: inset, height: 1, transform: `scaleX(${draw})` })} />
      <div style={line({ top: inset, bottom: inset, left: inset, width: 1, transform: `scaleY(${draw})` })} />
      <div style={line({ top: inset, bottom: inset, right: inset, width: 1, transform: `scaleY(${draw})` })} />
      ...
      {[
        ...
        <div key={i} style={line({ ...pos, width: 26, height: 1, opacity: 0.3 * draw })} />
      ))}
    </AbsoluteFill>
  );
};
...
const Rule: React.FC<{ width: number; delay: number }> = ({ width, delay }) => {
  ...
  return <div style={{ width: w, height: 1, background: CREAM, opacity: 0.5, margin: '26px 0' }} />;
};
...
const SceneMasthead: React.FC<Props> = ({ dateline, tags, lang }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
      <div style={{ ...rise(frame, 4), fontFamily: cormorant, fontWeight: 600, fontSize: 40, letterSpacing: 20, color: CREAM, textTransform: 'uppercase' }}>
        {ui.masthead}
      </div>
      <Rule width={420} delay={12} />
      <div style={{ ...rise(frame, 20), fontFamily: cormorant, fontSize: 34, color: CREAM, opacity: 0.85, letterSpacing: 3 }}>
        {dateline}
      </div>
      <div style={{ ...rise(frame, 30), marginTop: 26, display: 'flex', gap: 18, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 820 }}>
        {tags.map(t => (
          <span key={t} style={{ fontFamily: cormorant, fontSize: 28, color: CREAM, opacity: 0.75, border: `1px solid ${CREAM}55`, borderRadius: 2, padding: '6px 16px', letterSpacing: 2 }}>
            {t}
          </span>
        ))}
      </div>
...
const SceneTitle: React.FC<Props> = ({ title, subtitle }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', padding: 110 }}>
      <div style={{ fontFamily: playfair, fontWeight: 900, fontStyle: 'italic', fontSize: title.length > 46 ? 92 : 108, lineHeight: 1.1, textAlign: 'center', color: GOLD }}>
        {words.map((w, i) => (
          <span key={i} style={{ display: 'inline-block', marginRight: 18, ...rise(frame, 6 + i * 4) }}>
            {w}
          </span>
        ))}
      </div>
      <Rule width={300} delay={6 + words.length * 4} />
      <div style={{ ...rise(frame, 14 + words.length * 4), fontFamily: cormorant, fontSize: 40, lineHeight: 1.35, textAlign: 'center', color: CREAM, opacity: 0.9, maxWidth: 820 }}>
        {subtitle}
      </div>
    </AbsoluteFill>
  );
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', gap: 34 }}>
      {plates.map((p, i) => {
        ...
        return (
          <div key={i} style={{ ...st, width: 760, border: `1px solid ${CREAM}66`, padding: '30px 40px', textAlign: 'center' }}>
            <div style={{ fontFamily: cormorant, fontSize: 30, letterSpacing: 5, color: CREAM, opacity: 0.7, textTransform: 'uppercase' }}>
              {p.label}
            </div>
            <div style={{ fontFamily: playfair, fontWeight: 700, fontSize: 74, color: CREAM, marginTop: 10 }}>
              {p.value}
            </div>
          </div>
        );
...
const SceneLesson: React.FC<Props> = ({ lesson }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', padding: 120 }}>
      <div style={{ ...rise(frame, 4), fontFamily: cormorant, fontStyle: 'italic', fontSize: 58, lineHeight: 1.4, textAlign: 'center', color: CREAM }}>
        {lesson}
      </div>
    </AbsoluteFill>
  );
...
const SceneClub: React.FC<Props> = ({ club }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', padding: 100 }}>
      <div style={{ ...rise(frame, 2), fontFamily: cormorant, fontSize: 30, letterSpacing: 10, color: CREAM, opacity: 0.72, textTransform: 'uppercase' }}>
        {club.tagline}
      </div>
      <div style={{ ...rise(frame, 10), fontFamily: playfair, fontWeight: 900, fontStyle: 'italic', fontSize: 96, lineHeight: 1.08, textAlign: 'center', color: GOLD, marginTop: 18 }}>
        {club.name}
      </div>
      <Rule width={360} delay={20} />
      <div style={{ ...rise(frame, 26), fontFamily: cormorant, fontSize: 38, lineHeight: 1.35, textAlign: 'center', color: CREAM, maxWidth: 780, opacity: 0.92 }}>
        {club.terms}
      </div>
      <div style={{ ...rise(frame, 36), marginTop: 46, border: `1px solid ${CREAM}88`, padding: '20px 46px', fontFamily: cormorant, fontWeight: 700, fontSize: 40, letterSpacing: 4, color: CREAM, textTransform: 'uppercase' }}>
        {club.cta}
      </div>
    </AbsoluteFill>
  );
...
const SceneColophon: React.FC<Props> = ({ invariant, url, year, lang }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
      <div style={{ ...rise(frame, 4), fontFamily: cormorant, fontSize: 36, letterSpacing: 4, color: CREAM, opacity: 0.9 }}>
        {ribbon}
      </div>
      <Rule width={460} delay={12} />
      <div style={{ ...rise(frame, 18), fontFamily: playfair, fontWeight: 700, fontSize: 62, color: CREAM }}>
        {url}
      </div>
      <div style={{ ...rise(frame, 28), marginTop: 34, fontFamily: cormorant, fontSize: 30, letterSpacing: 8, color: CREAM, opacity: 0.7 }}>
        {year}
      </div>
    </AbsoluteFill>
  );
    ...
    <AbsoluteFill style={{ justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 150 }}>
      ...
      >
        {src.match(/\.(mp4|webm|mov)$/i) ? (
          <OffthreadVideo src={src} muted style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(1) contrast(1.15) brightness(0.95)' }} />
        ) : (
          <Img src={src} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(1) contrast(1.15)' }} />
        )}
      </div>
    </AbsoluteFill>
  );
...
const Caption: React.FC<{ captions: Props['captions'] }> = ({ captions }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 110 }}>
      <div style={{ fontFamily: cormorant, fontWeight: 700, fontSize: 56, letterSpacing: 6, color: CREAM, textTransform: 'uppercase', textShadow: `0 2px 18px ${BG}` }}>
        {cur.text}
      </div>
    </AbsoluteFill>
  );
...
export const TrinityBlogReel: React.FC<Props> = props => {
  ...
  const acts = props.club
    ? [
        ...
        { at: Math.round(d * 0.92), len: d - Math.round(d * 0.92), C: SceneColophon },
      ]
    : [
        ...
        { at: Math.round(d * 0.88), len: d - Math.round(d * 0.88), C: SceneColophon },
      ];
  return (
    <AbsoluteFill>
      <Paper />
      ...
      {props.avatarVideo ? (
        <Sequence from={acts[2].at} durationInFrames={acts[2].len + acts[3].len}>
