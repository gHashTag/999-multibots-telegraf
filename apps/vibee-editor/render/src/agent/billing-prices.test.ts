...
describe('чип длительности не назначает счёт там, где не доходит до провайдера', () => {
  it('длину задаёт человек ТОЛЬКО при `duration` в контракте', () => {
    expect(длинуЗадаётЧеловек(['prompt', 'duration'])).toBe(true)
    // `video_url` — это НЕ «мы задаём длину»: файл присылают готовым.
    expect(длинуЗадаётЧеловек(['video_url'])).toBe(false)
    expect(длинуЗадаётЧеловек(undefined)).toBe(false)
  })

  it('длину задаёт файл при `video_url` или `audio_url`', () => {
    expect(длинуЗадаётФайл(['video_url'])).toBe(true)
    expect(длинуЗадаётФайл(['image_url', 'audio_url'])).toBe(true)
    expect(длинуЗадаётФайл(['prompt', 'duration'])).toBe(false)
  })

  it('topaz считается по замеру, а не по чипу', () => {
    /*
     * Замер прогоном модулей: чип «6с» давал счёт 96, чип «10с» — 160, и в
     * обоих случаях сборщик входа отправлял ровно `{video_url}`. Один и тот
     * же файл стоил по-разному из-за переключателя, не менявшего НИЧЕГО, а
     * KieAI считал нам настоящую длину ролика.
     */
  ...
  it('маршрут видео СЧИТАЕТ длину файла, а не берёт её с экрана', () => {
    // Помощник может быть зелёным и не подключённым — семнадцатое правило.
    const сервер = readFileSync(
      pathJoin(__dirname, '..', '..', 'render-server.ts'),
      'utf8'
    )
...
describe('перегенерация не снимает сторожа', () => {
  it('единица остаётся там, где провайдер её назвал, а цену взять нельзя', () => {
    /*
     * Обе таблицы писались из одного поля, и модель с непереводимой единицей
     * теряла ОБА значения. Единица — это причина отказа продавать, и по ней
     * сторожит тест «модель, чью единицу не перевести в вызов, НЕ ПРОДАЁТСЯ».
     * Обнули её — отбор становится пустым, тест зелен и не проверяет ничего.
     *
     * То есть один запуск генератора молча снимал сторожа, поставленного
     * ради конкретного убытка: Gemini TTS продавался за 280 токенов при
     * работе в доли цента.
     */
    const непереводимые = Object.keys(ЕДИНИЦА_ЦЕНЫ).filter(
      id => ЕДИНИЦА_ЦЕНЫ[id] === 'за млн токенов'
    )
    expect(непереводимые.length).toBeGreaterThan(0)
    // И у каждой из них цены НЕТ: единица без цены — это «не продаём».
    for (const id of непереводимые) {
      expect(priceForKieModel(id), `${id} продаётся`).toBeNull()
    }
  })
})
