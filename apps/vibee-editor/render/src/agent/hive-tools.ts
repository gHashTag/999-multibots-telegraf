  ...
  async handler(a: any, ctx?: ToolContext) {
    ...
    return {
      ...
      how_to_read:
        ...
        'Пустая колонка со счётом 0 — это факт «там никого», а не отсутствие ' +
        'колонки. «pulse» — её собственные счётчики под её же именами: ' +
        'замерено 07.09.2026, что они не сдвинулись за два круга, пока она ' +
        'стояла в ожидании разбора, поэтому это НЕ темп работы сейчас. ' +
        '«bees» внутри pulse — не четыре рабочих места выше: что именно она ' +
        'там считает, по её ответам установить не удалось.',
    }
  },
})
