 ...
 */
/*
 * СПИСОК СМОТРИТЕЛЕЙ ЖИВЁТ В `src/hive/roles.ts`, А НЕ ЗДЕСЬ.
 *
 * Здесь стояла своя копия зашитого id владельца платформы. Вторая копия
 * правила «кому видно всё» — это способ однажды поправить одну и забыть
 * другую, а цена ошибки тут — чужие клиенты на чужом экране.
 */
...
export async function областьВидимости(ctx?: ToolContext): Promise<string[] | null> { // cyrillic-ok: pre-existing name
  ...
  /*
   * Роль спрашивается у ОБЩЕГО модуля, а не вычисляется здесь.
   *
   * Раньше эта функция сама сверялась с зашитым id владельца и сама ходила в
   * `avatars`. Пока правило повторяется в каждом чтении, одна забытая строка
   * означает, что клиент увидел чужих клиентов — а таблица `users` в этой
