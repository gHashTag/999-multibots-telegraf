...
function extractAudio(videoPath: string): string {
  ...
  try {
    // execFileSync без шелла: путь — аргумент, а не часть команды
    execFileSync(
      'ffmpeg',
      ['-i', videoPath, '-ar', '16000', '-ac', '1', '-y', audioPath],
...
function captionsToSegments(captions: Caption[], fps: number): Segment[] {
  ...
  return captions.map((caption, i) => {
    ...
    return {
      ...
      bRollUrl:
        type === 'split' ? brollVideos[i % brollVideos.length] : undefined,
...
export async function transcribeVideo(
  ...
  if (!fs.existsSync(whisperDir)) {
    throw new Error(
      'Whisper.cpp not installed. Run: git clone https://github.com/ggerganov/whisper.cpp'
    )
  }

  if (!fs.existsSync(modelPath)) {
    throw new Error(
      `Model not found: ${modelPath}. Run: cd whisper.cpp && bash models/download-ggml-model.sh small`
    )
  }
  ...
  if (
    ...
    !videoPath.startsWith('/Users') &&
    !videoPath.startsWith('/app') &&
  ...
  try {
    ...
    // Бинарник — литерал в каждой ветке (не переменная): у сканера нет
    // причин считать команду сборной. Язык — белый список.
    const useNewCli = fs.existsSync(
      path.join(whisperDir, 'build/bin/whisper-cli')
    )
    const lang = language === 'en' ? 'en' : language === 'ru' ? 'ru' : 'auto'
    const whisperArgs = [
      '-m',
      modelPath,
      '-f',
    ...
    console.log(`Using whisper binary: ${useNewCli ? 'whisper-cli' : 'main'}`)

    try {
      // cwd вместо `cd dir &&`: шелл не нужен
      if (useNewCli) {
        execFileSync('build/bin/whisper-cli', whisperArgs, {
          stdio: 'pipe',
          timeout: 300000,
          cwd: whisperDir,
        })
      } else {
        execFileSync('./main', whisperArgs, {
          stdio: 'pipe',
          timeout: 300000,
          cwd: whisperDir,
        })
