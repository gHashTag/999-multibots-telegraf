...
struct TimelineView: View {
   ...
   *
   * Было «1 дорожек · 150 кадров». Русский требует три формы, а не одну, и
   * подстановка `\(n) дорожек` даёт согласование только для 5–20. Единица —
   * самый частый случай в этом редакторе (одна текстовая дорожка), то есть
   * ошибка стояла ровно там, где на неё смотрят чаще всего.
   *
   * 11–14 — исключение: одиннадцать, а не «одиннадцать дорожка», поэтому
   * сотня отбрасывается до проверки десятков.
   */
  private func склонение(_ n: Int, _ одна: String, _ две: String, _ пять: String)
    -> String
  {
    let сто = abs(n) % 100
    let десять = сто % 10
