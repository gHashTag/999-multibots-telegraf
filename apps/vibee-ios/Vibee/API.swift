...
enum API {
  ...
  struct Баланс: Decodable {
    ...
    func мера(_ операция: String, модель: String?) -> String? {
      if let м = модель {
        if perThousandCharsModels?.contains(м) == true { return "1000 зн." }
        if perSecondModels?.contains(м) == true { return "с" }
        if perSecondModels?.isEmpty == false || perThousandCharsModels?.isEmpty == false {
          return nil
        }
      }
    ...
    func посекундная(_ операция: String, модель: String? = nil) -> Bool {
      if let м = модель, let список = perSecondModels, !список.isEmpty {
        return список.contains(м)
      }
