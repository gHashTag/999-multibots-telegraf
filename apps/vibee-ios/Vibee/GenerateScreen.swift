  ...
  @ViewBuilder private var форма: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      Button {
        ...
        /**
         * ПОКА ИДЁТ ГЕНЕРАЦИЯ — БЕЗ ЗАТЕМНЕНИЯ.
         *
         * Было `.opacity(нельзяЗапускать ? 0.45 : 1)`, а `нельзяЗапускать`
         * включает и `идёт`. То есть спиннер и «Идёт 32 с» рисовались уже
         * поверх притушенных фона и текста — а `.opacity` на родителе гасит
         * ОБА разом теми же 45%, и тёмный текст на тёмном фоне уходит куда-то
         * в район 20% реальной видимости. Не оттенок, а нечитаемая надпись.
         *
         * Затемнение — это визуальный сигнал «нажать нельзя и не станет
         * можно само», и он верен для «нет описания» или «модель не в
         * строю». Для «идёт запрос» это неверный сигнал: кнопка активна по
         * смыслу, просто защищена от повторного нажатия — вот для этого и
         * достаточно `.disabled`, непрозрачность ему в помощь не нужна.
         */
        .opacity(нельзяЗапускать && !идёт ? 0.45 : 1)
      }
      .buttonStyle(.plain)
