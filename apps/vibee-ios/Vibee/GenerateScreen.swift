...
struct GenerateScreen: View {
  ...
  private var сколькоСпишут: ЦенаНажатия {
    ЦенаНажатия.посчитать(
      единица: ценаНажатия,
      посекундная: посекундная,
      // Длительность человек задаёт только у видео. У липсинка её определяет
      // готовая озвучка, и выдумывать её здесь нельзя.
      секунды: вид == .видео ? секундыИзДлительности(длительность) : nil
    )
  }
