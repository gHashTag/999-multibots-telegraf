...
struct GenerateScreen: View {
  ...
  enum Вид: String, CaseIterable, Identifiable {
     ...
     */
    /// Тот же вид в терминах каталога KieAI.
    var видКаталога: Модель.Вид {
      switch self {
      case .видео, .сценарий: return .видео
      case .картинка: return .картинка
      case .звук: return .звук
    ...
    var неготовность: String? {
       ...
       */
      let естьЖивые = !КаталогKie.поВиду(видКаталога).filter(\.живая).isEmpty
  ...
  @ViewBuilder private var форма: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      VStack(alignment: .leading, spacing: 4) {
        ...
        ForEach(КаталогKie.поВиду(видКаталога)) { м in
      ...
      Button {
        ...
        .opacity(нельзяЗапускать ? 0.45 : 1)
      }
      .buttonStyle(.plain)
