...
struct GenerateScreen: View {
  ...
  private var почемуНельзяЗапускать: String? {
       ...
       б.balance < цена
    {
       ...
       */
      return посекундные.contains(оп)
  ...
  /// Операции, у которых цена умножается на секунды. Список с сервера
  /// (`billedSeconds` в render-server.ts), а не догадка по названию.
  private let посекундные: Set<String> = ["lipsync_generate"]
