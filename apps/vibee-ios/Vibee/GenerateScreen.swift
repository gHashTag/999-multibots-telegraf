...
struct GenerateScreen: View {
  ...
  enum Вид: String, CaseIterable, Identifiable {
     ...
     *
     * У СЦЕНАРИЯ ИХ НЕТ, И ПОДСТАВЛЯТЬ ВИДЕО НЕЛЬЗЯ. Здесь раньше стояло
     * `case .видео, .сценарий: return .видео`, и это было ошибкой, видной
     * на экране: вкладка «Сценарий» показывала Kling и Grok Imagine —
     * модели, которые снимают ролик, а не пишут текст. Хуже того, проверка
     * «есть ли живые модели» считала по этому же чужому списку и снимала
     * с вкладки честное предупреждение про недостающий ключ xAI.
     *
     * Каталог KieAI у нас содержит только видео, картинки, звук и липсинк —
     * ни одной текстовой модели. Пока это так, сценарию нечем работать, и
     * `nil` говорит об этом прямо.
     */
