...
enum Connect {
  ...
  /**
   * Состояние: подключён ли Telegram и знаем ли мы номер.
   *
   * Номер приезжает ВМЕСТЕ со статусом, а не отдельным запросом: экран и так
   * спрашивает статус при открытии, и второй круг сети пришёлся бы ровно на
   * тот миг, когда человек ждёт появления формы.
   */
  static func состояние() async throws -> (подключено: Bool, phone: String?) {
    let о = try await запрос("api/tg/connect/status", "GET", nil)
    let номер = о["phone"] as? String
    return (
      подключено: (о["подключено"] as? Bool) ?? false,
      phone: (номер?.isEmpty ?? true) ? nil : номер
    )
  }
