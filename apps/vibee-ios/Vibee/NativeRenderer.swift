...
final class NativeRenderer {
    ...
    -> AVMutableVideoComposition
  {
    ...
    if mode == .export {
      vc.animationTool = сделатьAnimationTool()
    }
  ...
  /**
   * Дерево слоёв для запекания в файл.
   *
   * ТРИ ЛОВУШКИ, КАЖДАЯ ИЗ КОТОРЫХ МОЛЧАЛИВАЯ:
   *
   * 1. ОДНОРАЗОВОСТЬ. `AVVideoCompositionCoreAnimationTool` забирает слои
   *    себе и на второй экспорт с теми же объектами отдаёт мусор или пустоту.
   *    Поэтому дерево строится ЗАНОВО на каждый вызов — отсюда и требование
   *    к `OverlayLayerBuilding` быть фабрикой, а не хранилищем.
   *
   * 2. ГЕОМЕТРИЯ — И ЗДЕСЬ ЕЁ ТРОГАТЬ НЕ НАДО. Автономное дерево (то, что
   *    рендерится в видеобуфер, а не на экран) считает Y СНИЗУ ВВЕРХ, и
   *    переворот в левый верхний угол делает `isGeometryFlipped = true` на
   *    корне ДЕРЕВА — это забота `OverlayLayerBuilding`, см. контракт
   *    протокола. На РОДИТЕЛЕ такой флаг был бы чистой видимостью работы:
   *    оба его подслоя (`videoLayer` и дерево) занимают его прямоугольник
   *    целиком, а полный прямоугольник симметричен относительно переворота,
   *    то есть флаг не сдвинул бы ни пикселя. Ставить его «на всякий случай»
   *    хуже, чем не ставить: следующий человек решит, что переворот сделан
   *    здесь, и снимет его с корня дерева.
   *    Проверка стороны: отрендерить кадр 0 с меткой в y=0 — метка обязана
   *    быть СВЕРХУ. Если снизу — флаг потерялся в дереве, а не тут.
   *
   * 3. МАСШТАБ РАСТРА. `contentsScale` обязан быть 1.0. Дерево уже задано
   *    в ПИКСЕЛЯХ кадра (1080×1920), и экранный масштаб 2×/3×, который
   *    CALayer наследует по умолчанию, заставил бы `CATextLayer`
