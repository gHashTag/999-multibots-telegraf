...
struct LayerBuilder {
  ...
  func build() async throws -> RenderTree {
    ...
    //
    // ВАЖНО: флаг НЕ НАСЛЕДУЕТСЯ. Каждый контейнер, внутри которого дети стоят
    // не впритык к его границам, должен выставить его сам — см. обёртку клипа.
    // Забыть это дороже всего у текста: он единственный смещается внутри
    // обёртки, и без флага уезжает вертикально ровно на (высота обёртки −
    // высота текста).
  ...
  private func собратьКлип(
    ...
    // Флаг не наследуется от корня — выставляем на каждом контейнере.
    обёртка.isGeometryFlipped = true
