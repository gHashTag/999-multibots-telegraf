...
public func measureSpring(
  ...
  while i < 20 {
    ...
    if разница >= threshold {
       ...
       * ЕДИНИЦА, А НЕ НОЛЬ.
       *
       * В оригинале это `for (let i = 0; i < 20; i++)`, и `i++` из заголовка
       * отрабатывает НА КАЖДОЙ итерации — в том числе сразу после `i = 0`
       * внутри тела. То есть счётчик перезапускается с единицы, и после
       * нарушения проверяется 19 кадров, а не 20.
       *
       * Ноль здесь давал лишний кадр наблюдения. Разница видна только если
       * пружина нарушает порог ровно на двадцатом кадре после прошлого
       * нарушения: тогда Swift его ловил, а Remotion уже нет, и
       * естественная длительность расходилась — а от неё считается
       * масштабирование всего `spring(durationInFrames:)`.
       */
      i = 1
      финальный = frame + 1
    } else {
...
public func spring(
  ...
  // `d != 0` — УСЛОВИЕ ВЕТКИ, а не проверка внутри неё.
  //
  // Прошлая версия этого комментария объясняла ноль «truthy-проверкой» в JS.
  // Механизм другой: у Remotion есть validateSpringDuration, и на нуле он
  // БРОСАЕТ, а не тихо считает значение незаданным. Мы ноль принимаем и
  // трактуем как «не задано» — расхождение сознательное и вот почему.
  //
