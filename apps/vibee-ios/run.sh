...
# 2-бис. МЕСТО НА ДИСКЕ — ДО СБОРКИ, А НЕ ПОСЛЕ.
#
# 07.09.2026 автономный цикл встал целиком: том заполнился, и оболочка команд
# перестала запускать ЧТО-ЛИБО -- она создаёт файл под вывод до запуска, а
# значит при полном диске не стартует даже та команда, которая освободила бы
# место. `rm`, `df`, `du`, `tri disk` -- все получали ENOSPC. Тупик, из
# которого автономная работа выйти не может.
#
# Сборка iOS -- крупнейший разовый едок места в этом репозитории. Отказать
# ЗАРАНЕЕ с внятным словом дешевле, чем оставить машину в состоянии, где не
# работает ничего.
# ИМЯ ЛАТИНИЦЕЙ. Кириллическое имя переменной bash принимает при разборе
# (`bash -n` молчит!) и падает на запуске: "command not found". За этот цикл я
# наступил сюда четырежды, и трижды -- уже ПОСЛЕ того, как записал урок.
# Проверка синтаксиса тут не помощник: помогает только запуск.
FREE_MB=$(df -m . | awk 'NR==2 {print $4}')
if [ "${FREE_MB:-0}" -lt 2048 ]; then
  echo "❌ мало места: свободно ${FREE_MB} МБ, нужно хотя бы 2048"
  echo "   Сборка iOS -- самый крупный едок места здесь."
  echo "   Освободить:  rm -rf \"$PWD/build2\"   (и см. tri disk)"
  exit 1
fi

# 3. Сборка. Цель + явный SDK, см. ловушку выше.
#
# УБОРКА ПРЕДЫДУЩЕЙ СБОРКИ -- ЧАСТЬ САМОЙ СБОРКИ.
#
# `build2` не чистился никогда: каждая сборка добавляла к нему. Знание об
# уборке было записано в `tri ios` (там свой каталог и своя чистилка), но в эту
# ночь я четыре раза звал `run.sh` напрямую -- и защита, стоящая в другой
# двери, не сработала ни разу.
#
# Поэтому уборка стоит ЗДЕСЬ, на пути, по которому реально ходят. Полная
# пересборка стоит минуту; полный диск стоит остановки всей работы.
rm -rf "$PWD/build2"
