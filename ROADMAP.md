...
*   [ ] ❓ **Проблема с созданием PENDING статуса при инициации платежа**
    ...
    *   ✅ Найти причину, почему не создается запись `payments_v2` со статусом `PENDING` (Неверный Enum `MONEY_INCOME`).
...
*   [ ] 📜 **Проверить и обновить SQL-функции (`get_user_balance`, `get_user_balance_stats`)**
    *   [ ] Убедиться, что они корректно обрабатывают типы операций (`MONEY_INCOME`, `MONEY_OUTCOME`, `MONEY_INCOME` и т.д.) согласно `payments-subscriptions-logic.mdc`.
