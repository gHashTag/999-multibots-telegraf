...
*   **Ключевые Модули/Файлы:**
    *   📝 `src/registerCommands.ts` (регистрация `/start`)
    *   ✅ `src/scenes/startScene/index.ts` (логика сцены) - Покрытие: 58.45%, Тесты проходят
    *   ✅ `src/core/supabase/createUser.ts` (создание/поиск пользователя) - Покрытие: ~74.66%, Тесты проходят
    *   📝 `src/core/supabase/getUserDetailsSubscription.ts` (проверка существования/подписки)
    *   📝 `src/core/supabase/referral.ts` (базовая проверка рефералов)
    *   📝 `src/utils/localization.ts` (`getTranslation`)
    *   📝 `src/menu/index.ts` (`startMenu`)
    *   📝 `src/menu/mainMenu.ts` (`levels` для туториала)
    *   📝 `src/helpers/language.ts` (`isRussian`) - ✅ Тесты проходят