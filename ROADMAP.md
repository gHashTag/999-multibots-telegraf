...
- ✅ **{current_date}: Успешно удален Jest** из зависимостей и конфигурации проекта.
- ✅ **{current_date}: Существующие базовые тесты (интерфейсы, команды /support, /help, /bot) проходят на Vitest.**
- ⏳ **{current_date}: НАЧАТЬ TDD** для `startScene` ([`@src/scenes/startScene/index.ts`](mdc:src/scenes/startScene/index.ts)) согласно `user-start-flow.mdc`.
  - Создать файл `src/scenes/startScene/startScene.test.ts`.
  - Написать тесты (🔴 -> ✅ -> ♻️) для основного потока: вход, проверка/создание пользователя, рефералы, приветствие, туториал, меню.
- ❌ **{current_date}: Тесты для `startScene` блокированы** из-за проблем с мокированием асинхронных шагов сцены (вызовы `vi.fn` не отслеживаются). <-- СТАТУС УСТАРЕЛ, требует перепроверки после написания тестов.
- ✅ **{current_date}: Команда /support протестирована** (`__tests__/commands/support.test.ts`)
...
2.  **Тестирование `startScene` (Приоритет 2 - ПОНИЖЕН)** ❌
    *   **Цель:** Написать базовые тесты для `startScene`.
    *   **Статус:** ❌ Блокировано (Проблемы с мокированием `vi.fn` в асинхронных шагах сцены).
    *   **Действия (Отложено):**
        *   Исследовать специфику мокирования Telegraf WizardScene в Vitest.
        *   Попробовать интеграционный подход к тестированию сцен.

3.  **Решение Проблемы с Разрешением Путей в Тестах (Приоритет 3)** ❌
    *   **Проблема:** Ошибка `Cannot find package '@/...'` (Возможно, решена исправлением `vitest.config.ts`, но требует подтверждения на других тестах).
    *   **Статус:** ❌ Требует проверки.
    *   **Действия:**
        *   Проверить на тестах команд.

4.  **Тестирование `helpScene` (Приоритет 3)** ⏸️
    *   **Цель:** Написать тесты для `helpScene`.
    *   **Статус:** ⏸️ Приостановлено. Вернемся после `startScene` и решения проблемы с путями.
    *   **Проблемы:**
        *   Сложность моков для WizardScene.
        *   Ошибка разрешения путей `@/`.
    *   **Действия (Отложено):**
        *   Исправить ошибки в `helpScene.test.ts` (Linter Errors).
        *   Упростить моки.
        *   Решить проблему разрешения путей.

5.  **Унификация Навигации (Справка, Главное Меню)** ✅
    *   **Цель:** Единообразный и глобальный доступ к "❓ Справка" и "🏠 Главное меню".
    *   **Статус:** ✅ Завершено (25.04.2025).
    *   **Действия:**
        *   ✅ Созданы глобальные `hears`/`action` хендлеры (`go_help`, `go_main_menu`).
        *   ✅ Стандартизированы кнопки ReplyKeyboard (`mainMenu.ts`, `getStepSelectionMenu...`, `imageModelMenu.ts`, `videoModelMenu.ts`).
        *   ✅ Удалены локальные `hears` из сцен (`digitalAvatarBodyWizard`, `paymentScene`, `starPaymentScene`, `textToVideoWizard`, `neuroPhotoWizard`).
        *   ✅ Адаптирована `helpScene` (кнопка "Назад", `leave_help` хендлер).
        *   ✅ Исправлен баг в `neuroPhotoWizard` (не обрабатывает кнопки как промпт).
        *   ✅ Добавлены глобальные `hears` для текстовых кнопок в `registerCommands.ts`.
        *   ✅ Добавлен `ctx.scene.leave()` в хендлер `/menu`.

---

## 🛠️ Рефакторинг и Улучшения

*   **Подписочная система (`payments_v2`)** ⏳
    *   **Статус:** ⏳ В процессе (требует проверки SQL функций).
    *   **Цель:** Использовать `payments_v2` как единственный источник истины. Упростить логику и типы (`operation_type`).
    *   **Действия:**
        *   ✅ Код бота обновлен для использования `operation_type`.
        *   ❓ Проверить и обновить SQL функции (`get_user_balance`, `get_user_balance_stats`, `create_system_payment`) в Supabase SQL Editor.
*   **Переход на Fastify (`api-server`)** ✅
    *   **Статус:** ✅ Завершено.
*   **Типизация и Конфигурация** ✅
    *   **Статус:** ✅ В основном завершено. `models.config.ts` типизирован.
*   **Расчет Цен** ✅
    *   **Статус:** ✅ Логика `calculateFinalPrice` проверена и соответствует правилам.
...
*   **Docker & Elestio** ✅
    *   **Статус:** ✅ Настроено. `docker-compose.yml` обновлен. API_SERVER_URL прописан.
*   **CI/CD (GitHub Actions)** 📝
    *   **Статус:** 📝 Запланировано.
*   **PM2 для Production** ✅
    *   **Статус:** ✅ Настроено. Скрипт `deploy-prod.sh` использует PM2.

---

## 🧪 Тестирование (Общее)

*   **Текущее Покрытие:** ~5% (оценка)
*   **Инструменты:** Vitest, ts-mockito (или vi.mock).
*   **Цель:** 100% покрытие.
*   **Статус:** ⏳ Начато.
...
*   **Разрешение Путей в Тестах (`@/`)** ❌ - **Критично** для продолжения тестирования.
*   **Проверка SQL Функций** ❓ - Необходимо проверить и обновить SQL функции для `